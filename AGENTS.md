...
### 「この画面のV4を作って」と依頼されたときの標準自動フロー

- この表現を、既存JSXの部分修正ではなく「友だち属性V3で成功した静的View先行方式」の開始指示として扱ってください。
- 最初に `pnpm create:v4-screen -- --name=<画面id> --title=<画面名> --route=<ルート> --node-id=<Pencil実ノードID>` で、既存から独立したViewとCSS Moduleを作ります。
- 既存JSX・V2/V3表示部品・共通画面CSS・Tailwindの見た目クラスは参照せず、Pen.devの実ノード画像だけから静的Viewを完成させます。
- `apps/web/v4-screens.json` へ、現行画面の全機能を `retained`（残す）・`added`（追加）・`missing`（未移植）・`approved-removal`（削除承認済み）で記録します。初期値は「残す」で、Pen.devに見当たらないだけでは削除しません。
- Pen.devにない現行機能は、勝手に落とさず「V4のどこへ置くか」の確認事項として利用者へ報告します。削除はGitHub Issueまたは仕様書で明示承認された場合だけ許可します。
- 静的Viewの見た目を完成させてから、ダミーデータをpropsへ変えます。現行画面から持ち込むのは、データ取得・状態・操作とprops型に必要な契約だけです。
- `page.tsx` は機能層、`*-view.tsx` は表示層とします。ViewへAPI、状態Hook、ルーター状態、ブラウザ保存処理を入れてはいけません。
- 1画面目と2画面目を完成させた後、両方で位置・寸法・状態・操作が同じ部分だけを共通部品として抽出します。想像で先に共通部品を作りません。
- `pnpm check:v4-architecture` が落ちる状態、または台帳に `missing` / `pending` がある状態をV4完成と報告しません。未移植機能は完了報告に必ず列挙します。
- V4完成後は環境変数の切替1本で新旧を切り替え、確認期限後に旧Viewと旧ルートを削除します。`-v2`・`-v3`ルートを追加し続けません。

- Pen.devのV3/V4を実装するときは、`docs/pendev-v4-implementation-runbook.md` の開始前・画像比較・PRゲートを必ず通してください。
- V4画面の実装は、Pencil MCPで取得した実ノードID、対象状態の一覧、1920pxの設計画像、同じ状態・同じ横幅の実装画像がそろうまで開始・完了扱いにしないでください。画面名や「V4 4-1-A」のような仮名をノードIDの代わりにしてはいけません。
- `data-design`、見出し・ボタン名の文字列テスト、機能テスト、ビルド成功は視覚一致の証拠ではありません。設計画像と実装画像を横に並べ、位置・寸法・文字・色・余白・枠・角丸・影・全状態を目視比較してください。
- V2や既存の共通部品は、V4と見た目が一致すると画像比較で確認できた場合だけ再利用してください。機能を残すことと、V2の表示構造を残すことを混同しないでください。
- PRでV4完了を名乗る場合は、`.github/PULL_REQUEST_TEMPLATE.md` のVisual Parity欄を埋めてください。実ノードや比較証拠が無い画面は `blocked` / `unverified` と記録し、検証済みに変更してはいけません。
