...
- 管理画面のデザインの判断基準は **「使う人にとって分かりやすく使いやすいか」** と、それを形にした **Pencil の ★V7**（`~/Downloads/LINE-Harness-V7.pen`。部品と「画面の決まり」ボード）です。★V6 260画面は最初の土台を作るためのもので、見た目を合わせる対象ではありません（オーナー決定 2026-09-25）。
- V6 に基づく設計・注記・契約テスト（`data-design` のノードID、V6 の文言や見た目を文字で固定するもの）が使いやすさの直しの邪魔になるときは、消すか ★V7 に書き換えてください。動き（保存・送信・権限・失敗時の扱い・読み直し）を守るテストは消さないでください。消したときは PR に「何を・なぜ」を書きます。
- 画面を作る前に `docs/v6-common-rules.md`（骨組みと共通部品）と、★V7「画面の決まり」ボード（説明の帯・緑の使い方・余白・出さないもの・読み込めなかった時・開き先がない）を確認してください。優先順位は「使いやすさ → ★V7 → この文書 → `apps/web/src/components/shared/` → 契約テスト」で、上が勝ちます。
- 機能ごとの動きと合格条件は `docs/v6-requirements/` の要件定義に従います（見た目の指定ではなく、動きの正本です）。要件と実装が食い違うときは、実装ではなく要件を先に直します。
- 見た目を大きく変えるときは Pencil の ★V7 を先に直してください。コードだけ直すと、次に絵を見た人が元に戻します。
- 画面の確認は、本番ビルドを撮影して目で見ることで行います。文字列テスト・ビルド成功は見た目の証拠ではありません。
...
- PCの確認基準は幅1152px（1440pxの125%表示）・1440px・1920pxとし、主要な一覧画面でページ全体または表だけの横スクロールが発生しないことを確認してください。
