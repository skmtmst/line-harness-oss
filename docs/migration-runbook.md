...
### 0. 前提：Migrate D1 が Actions に出ているか

**手で走らせるワークフローは、既定ブランチに入るまで Actions の一覧に出ない。**
このリポジトリの既定ブランチは `codex/development`。

Actions の一覧に **Migrate D1** が無ければ、
[#61](https://github.com/skmtmst/line-harness-oss/pull/61) がまだマージされていない。
先にそれを入れること。中身は CI と文書だけで、データベースには触らない。
