...
3. **カルーセルの編集画面**。検証は入ったので、JSONを書かずに済む画面だけ
4. **二要素認証**。列はあるが、認証の仕組みそのものが要る。単独で見積もる方がよい
5. **サイドバーの並び替え**。`account_settings` の `sidebar.order` は
   APIが受け取るところまで入っている。画面だけ
6. 作成画面の穴埋め（多くは一覧に作る仕組みがあるので、実際に使ってから判断）
