...
| 対象 | 状態 |
|---|---|
| メディアライブラリ | **完了**（#52）。使用箇所スキャンのCronだけ未 |
| 共通情報 | **完了**（#52）。日付切り替えのCronも入れた |
| アクセス解析・クロス・ファネル・URLクリック | **完了**（#53） |
| サイトスクリプト | **未**。収集の入口・埋め込みJS・レート制限 |
| ファネルの作成画面 | **未**。結果は見られるが、作るAPIは画面から呼べていない |

`media_usages` は表とAPIはあるが、**本文を走査して埋める仕組みが無い**ので、
いまは常に0件。「どこでも使われていません（本文の走査が済んだ時点の情報です）」
と画面に書いてある。
...
- 新しいテーブル: 12
- DBヘルパ: 9本
- 新しいWorkerルート: 5本（`friend-fields` `friend-attributes` `feature-settings` `contents` `analytics`）
- 新しい画面: 6（`/tags/fields/new` `/friends/detail` `/settings` `/contents` `/contents/vars/new` `/analytics`）＋ タブ統合7画面
- テスト: worker **1225** / db **195** / web **43**
...
3. **サイトスクリプト**。フェーズ4で残っている唯一の大きいもの。
   収集の入口はレート制限とセットで作る必要がある
4. **メディアの使用箇所スキャン**。表とAPIはあるので、本文を走査して
   埋めるCronを足すだけ。いまは常に0件で、削除前の警告が働かない
5. **ファネルの作成画面**。APIはあるので画面だけ
6. カルーセル編集、ログイン履歴の画面、二要素認証
