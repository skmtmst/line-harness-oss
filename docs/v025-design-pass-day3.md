...
`max-w` が無い。**設計の絵は 1440px 前後で描かれている**ので、
横に広いモニタで開くと表が伸びきって、絵とまったく違う見え方になる。
...
2. **共通の3つを直す**（4節）。1つずつ別のコミットにする
   - `header.tsx` → 65画面が変わる。直したら一通り見る
   - `app-shell.tsx` に `max-w` を入れる
   - `layout.tsx` の `<body>` をトークンに寄せる
