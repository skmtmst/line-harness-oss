...
| 11 | 04 属性 | 4-2-A | 画像・PDFは差し込み不可、メディア参照であることを表示 | 04 §20 | 未反映(2026-09-04 設計画像で確認。`A1ZYeP` に差し込み不可の断りが無い) |
| 12 | 04 属性 | 4-2-B | 切戻し期限、変換不可の個別修正導線を追加 | 04 §20 | 未反映(2026-09-04 設計画像で確認。`KoT6c` に切戻し期限も個別修正の導線も無い) |
| 13 | 04 属性 | 4-3 | 「対応状況とは別の自由分類」と明記 | 04 §20 | 未反映(2026-09-04 設計画像で確認。`rIhbN` に「対応状況とは別」の断りが無い) |
| 14 | 04 属性 | 4-3-B | 置換先を必須選択として追加 | 04 §20 | 一部反映(2026-09-04 確認。置き換え先は出るが必須の選択になっていない) |
| 15 | 04 属性 | 4-4 | 所有者、対象範囲、ライブ/固定、revisionを表示 | 04 §20 | 未反映(2026-09-04 設計画像で確認。`QKx8Q` に所有者・対象範囲・版が無い) |
| 16 | 04 属性 | 4-4-A | 「次回実行から変わる」はライブ参照時だけ表示 | 04 §20 | 一部反映(2026-09-04 確認。「次回実行から変わる」は在るが、ライブ参照のときだけ出す形になっていない) |
| 17 | 09 追加時配信 | 9-1-A/B/C/I | 設定は1枚・削除概念なし・本文はシナリオ側のため、4画面を削除または非表示タグ | `scripts/visual-qa/screens.mjs` gap:'drop' | 未反映(2026-09-04 確認。9-1-A/B/C/I が台帳に残っている) |
| 18 | 10 ウェビナー | 10-1-K | 視聴履歴の物理削除を禁止するため削除確認を削除し「アーカイブ確認」へ | 同上 | 未反映(2026-09-04 確認。`LKuAQ` は「削除の確認」のまま) |
| 19 | 13 回答フォーム | 13-1-B | デザイン設定は作らない方針のため削除 | 同上 | 未反映(2026-09-04 確認。13-1-B が台帳に残っている) |
| 20 | 16 アフィリエイト | 16-1-G | 支払記録の物理削除を禁止するため削除確認を「アーカイブ確認」へ | 同上 | 未反映(2026-09-04 確認。`QX70l` は「削除する確認」のまま) |
| 21 | 全機能 | 権限不足状態 | `forbidden`に対応する設計部品が無い。共通部品`ListState`の権限不足状態を1枚描く | `apps/web/src/components/shared/list-state.tsx` | 未反映(2026-09-04 確認。共通部品に権限不足の状態が無い) |
...
| 27 | 01 ダッシュボード | `NjK9q` | プルダウンの選択印が共通部品`Gfsb4`と違う | `docs/design-qa/dashboard-v6/design-qa.md` | 未反映(2026-09-04 確認。`NjK9q` の選択肢に✓が無い) |
| 28 | 33 アカウント設定 | 新規 | LINEアカウント一覧(L)、登録(E)、詳細・編集(D)、乗り換え・引き継ぎ(E)の4画面を追加 | 33 §5-2 | 未反映(2026-09-04 確認。33 の 4 画面が無い) |
| 29 | 34 はじめの設定と案内 | 新規 | はじめの設定(B)、レシピ一覧(L)、レシピを複製する(E)、マニュアルの正本表(L・運営側)の4画面を追加 | 34 §5 | 未反映(2026-09-04 確認。34 の 4 画面が無い) |
...
| 41 | 06 / 07 / 08 / 05 / 03 | `bPF0s` `PSmHo` `e6iJG` `s6Vvp` `kk8dz` `InCDe` `bV5Vs` `NrBkW` | 追従バーの規則違反(空のバー3枚、並びが機能で逆、右寄せ、同一画面で構成が変わる)。§1-6のとおり削除は左・他は中央に統一し、シナリオ編集に「配信を開始」を置く | 同上 | 未確認(2026-09-04。並びの話なので書き出した文言では判定できない) |
| 42 | 03 / 06 | `YzxU1` `r7eSi` `vtBCu` `I6UAdr` `u6gHt` | 主ボタン色が緑と青で分裂、タブ様式が2通り。緑1系統、タブは`ISA1Q`に統一 | 同上 | 未確認(2026-09-04。色とタブの見た目なので文言では判定できない) |
| 43 | 05 / 06 | `kk8dz` `xfYLn` `XQfMD` | メッセージ種別タブが6種・9種・9種(並びも違う)の3通り。1部品にする | 同上 | 未反映(2026-09-04 確認。`kk8dz` に種別タブが無く、`xfYLn` と `XQfMD` で数が違う) |
| 44 | 05 / 06 / 07 / 08 | `M2b2B` `u6gHt` `GC4St` `t7UtYQ` | CSV書き出しの置き場が3通り(緑主ボタン、追従バー、右上)。「すでにあるものを扱う」は右上の副次ボタンに統一 | 同上 | 未確認(2026-09-04。置き場の話なので文言では判定できない) |
| 45 | 05 / 06 | `M2b2B` `u6gHt` `vW4Es` | 結果画面の数値が入力欄の見た目。`XywGr`の数値カードにし、結果画面から「テスト送信」「配信イメージ」を外す | 同上 | 2026-09-04(設計画像で確認。`M2b2B` `u6gHt` から外れている) |
| 46 | 05 / 06 / 07 / 08 | `TC1b1` `q76C35` `M1EXwB` `cmDfJ` | 一覧の行操作が削除だけ。「結果を見る」「複製」を行に足す(読み上げ名に対象名)。シナリオ一覧から結果`M2b2B`への入口を作る | 同上 | 未反映(2026-09-04 確認。4画面とも行の操作が削除だけ) |
| 47 | 03 | `PhxG6` | 対応チップと文字の二重表示で矛盾(対応済み＋対応中)、`[sticker]`の露出、UID移行タブと右上ボタンの重複。状態は1表現、内部語は「スタンプ」に | 同上 | 未確認(2026-09-04。見た目の話なので文言では判定できない) |
| 48 | 全機能 | 各画面 | 表記ゆれ・内部語(未割当/未割り当て、対応済/対応済み、dry-run、SlackのPRスレッド、Webhook同一ID、Flex、AND/OR、外部サービス、ブレイクダウン/マトリックス、9,110分前は「6日前」)。共通ルールの用語表に載せて全文検索で潰す | 同上 | 一部反映(2026-09-04 確認。表記は「対応済み」「未割り当て」で統一済み。画面名の dry-run は直した。用語表への追加は未) |
| 49 | 07 / 08 | `s7T2dz` `K7vg2` `g46ja` `U9hzqH` | ウィザードの段とボタン名のずれ(「予定を確認」が次工程名と違う、「自動応答ルールを保存」、テスト画面のステッパーがSTEP5)。「{次工程名}へ」に揃え、テストはSTEP4内の位置で描く | 同上 | 未反映(2026-09-04 確認。テスト画面のステッパーが STEP 5 のまま) |
| 50 | 07 / 05 / 03 / 02 / 08 | `J64xI` `bV5Vs` `I6UAdr` `xGLVe` `PSmHo` `e6iJG` | 小さな描画ずれ(本文4行目がtextareaの外、4行目の行操作アイコン、カード外のボタン、ヘッダの担当と右パネルの矛盾、「一時停止」の「?」アイコン) | 同上 | 未確認(2026-09-04。描画ずれなので文言では判定できない) |
