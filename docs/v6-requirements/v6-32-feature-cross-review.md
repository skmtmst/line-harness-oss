...
| 17 | 09 追加時配信 | 9-1-A/B/C/I | 設定は1枚・削除概念なし・本文はシナリオ側のため、4画面を削除または非表示タグ | `scripts/visual-qa/screens.mjs` gap:'drop' | 2026-09-04(Pencil の画面名に「V6では使わない・台帳から外す」を付けた。`screens.mjs` からの削除は所有パス外なので司令塔へ依頼) |
| 18 | 10 ウェビナー | 10-1-K | 視聴履歴の物理削除を禁止するため削除確認を削除し「アーカイブ確認」へ | 同上 | 2026-09-04 |
| 19 | 13 回答フォーム | 13-1-B | デザイン設定は作らない方針のため削除 | 同上 | 2026-09-04(Pencil の画面名に「V6では使わない・台帳から外す」を付けた。`screens.mjs` からの削除は司令塔へ依頼) |
