...
| テナント分離 | この索引 §5-1 | **当面は単一組織として扱う。** `tenants` は将来用として残し、いま実装しない |
| 主ボタンの文字色 | この索引 §5-2 | 白文字が乗る緑は `$accent-deep`(#087A3E・5.44:1)。`$accent`(#06c755) は文字が乗らない用途だけ |

### 5-1. テナント分離（2026-09-03 決定）

**当面は単一組織として扱う。** `tenants` にあたる機能は将来用として残し、いまは実装しない。

- 多テナント運用の顧客がまだいない
- 顧客契約に関わるため、方針が変わるときは人が決め直す
- `tags` をはじめ、所属で絞る必要が出た機能は **LINE公式アカウント単位**で絞る（`accountId`）。
  組織をまたぐ絞り込みは作らない

### 5-2. 主ボタンの文字色（2026-09-03 決定）

**白文字が乗る緑は `$accent-deep`（#087A3E）を使う。**

| 色 | 白文字との比 | 判定 |
|---|---|---|
| `$accent` #06c755 | **2.26:1** | AA 不合格 |
| 検討した #06a94a | 3.10:1 | 大きい文字なら合格。**ボタンのラベルは13px・太字600なので不合格** |
| `$accent-deep` #087A3E | **5.44:1** | 合格 |

AA が求めるのは、18.66px未満の太字・24px未満の並字で **4.5:1**。
ボタンのラベルは `$size-label` = 13px なので、3.10:1 では届かない。
新しい色を作らず、既にある `$accent-deep` を使う。

`$accent`（#06c755）は**文字が乗らない用途**（札の地、選択状態、アイコン、バッジ）に残す。
そこはLINEらしさが要る場所で、文字を乗せていないため基準の対象外。

Pencil側は 2026-09-03 に反映済み（白文字が緑に乗っていた **399箇所**）。
