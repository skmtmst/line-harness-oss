...
- 各区切りの末尾11件に入ると、次の100件用チャンネルを先行作成する。PR #290〜#300では `#line-harness-pr-301-400` を準備し、PR #301から自動で使う
- 次チャンネルの作成や1人の招待に失敗しても、現在のPR報告と残りの招待は止めない。末尾11件の各PRで再試行し、失敗内容はWorkerログへ残す
