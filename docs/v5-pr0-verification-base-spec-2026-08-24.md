...
| 指摘 | 反映 |
|---|---|
| 表示制御の正規表現が全角コロン | 直した。旧実装はASCIIのコロンを消費できないため、`md:hidden`も検知できていなかった。加えて、前置きの固定列挙では`md:max-lg:hidden`や`peer-checked:hidden`も見逃すため、列挙をやめて**最後のコロンより後ろ**で判断する方式に変えた |
