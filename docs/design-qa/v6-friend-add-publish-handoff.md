...
1. `ec9vg` は下書き、4つの確認結果、競合、最後の試験結果を読み合わせてから公開する。
...
5. `quhg6` は公開版、対象人数、二重実行防止、実行結果への導線を公開結果から表示する。
6. 通常・空（404）・失敗（500）・権限不足を別の面にする。空を失敗、失敗を0件にしない。
7. `data-design-node="ec9vg"` / `data-design-node="quhg6"` を付ける。
8. 窓や次の段を開く操作には `data-qa-open="ec9vg"` / `data-qa-open="quhg6"` を付ける。
9. 1440px・1920pxでページと主要領域の横スクロールを0にする。
