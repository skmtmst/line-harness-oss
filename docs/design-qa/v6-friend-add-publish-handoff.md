...
6. `quhg6` は公開版、対象人数、二重実行防止を公開結果から表示する。`monitoringPath` があるときだけ実行結果への導線を出す。`null` のときは `monitoringUnavailableReason` を表示し、存在しない画面へリンクしない。
...
- 通常：下書き、確認4件、dry-run、公開結果（実行結果画面は未接続）
- 空：404「確認する下書きがありません」
