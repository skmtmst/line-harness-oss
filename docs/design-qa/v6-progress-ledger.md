...
比較済み **259** ／ 比較不可 **13** ／ 10%超 **28**。高さは上端を揃えて共通領域を比較し、高さ差を別に記録します。
...
| | **合計** | **272** | **267** | **268** | **0** | **0** | **0** | **0** | **259** | **28** | **13** | **0** | **0** | **5** | |
...
| 3 | `vtBCu` | 3-4 UID移行 | 6.0778%（前回値） | +146px | 中央・中央 | — | [差分画像](friends-v6/vtBCu-diff-1920.png) | **一致** |
...
| 8 | `cmDfJ` | 8-1 自動応答 | 3.7387%（前回値） | 0px | 中央・左 | — | [差分画像](auto-replies-v6/cmDfJ-diff-1920.png) | **一致** |
...
| 8 | `g46ja` | 8-1-E 自動応答テスト | 5.9914%（前回値） | 0px | 中央・右 | — | [差分画像](auto-replies-v6/g46ja-diff-1920.png) | **一致** |
| 8 | `Yj6CQ` | 8-1-F 最終確認 | 5.6140%（前回値） | 0px | 下部・中央 | — | [差分画像](auto-replies-v6/Yj6CQ-diff-1920.png) | **一致** |
| 8 | `e6iJG` | 8-1-G 有効化完了 | 4.5472%（前回値） | +23px | 下部・右 | — | [差分画像](auto-replies-v6/e6iJG-diff-1920.png) | **一致** |
| 8 | `t7UtYQ` | 8-1-H 実行結果 | 4.5475% | +14px | 中央・右 | docs | [差分画像](auto-replies-v6/t7UtYQ-diff-1920.png) | **一致** |
| 8 | `Gy9OK` | 8-1-I 削除確認 | 14.6352% ⚠（前回値） | 0px | 中央・中央 | — | [差分画像](auto-replies-v6/Gy9OK-diff-1920.png) | **一致** |
...
| 11 | `GFlD7` | 11-1-A メッセージを作る | 8.0376%（前回値） | -56px | 中央・右 | — | [差分画像](templates-v6/GFlD7-diff-1920.png) | **一致** |
| 11 | `FRkls` | 11-1-B カルーセルを作る | 6.6401%（前回値） | 0px | 中央・右 | — | [差分画像](templates-v6/FRkls-diff-1920.png) | **一致** |
| 11 | `NNDMR` | 11-1-C 質問を作る | 6.8783% | +160px | 上部・右 | docs | [差分画像](templates-v6/NNDMR-diff-1920.png) | **一致** |
| 11 | `j9ixI` | 11-1-D リッチメッセージを作る | 5.6969%（前回値） | 0px | 中央・右 | — | [差分画像](templates-v6/j9ixI-diff-1920.png) | **一致** |
| 11 | `hsBtl` | 11-1-E クーポンを作る | 6.4859%（前回値） | 0px | 中央・右 | — | [差分画像](templates-v6/hsBtl-diff-1920.png) | **一致** |
| 11 | `J3GxEZ` | 11-1-F リサーチを作る | 7.1590%（前回値） | +1px | 中央・右 | — | [差分画像](templates-v6/J3GxEZ-diff-1920.png) | **一致** |
...
| 12 | `DIUbO` | 12-1-C 切替メニューのつながり | 6.8123%（前回値） | 0px | 上部・右 | — | [差分画像](rich-menus-v6/DIUbO-diff-1920.png) | **一致** |
...
| 17 | `s98Vfw` | 17-1 マイル | 4.7372%（前回値） | 0px | 中央・左 | — | [差分画像](mileage-v6/s98Vfw-diff-1920.png) | **一致** |
...
| 17 | `vz0Ji` | 17-1-F マイルを手で増やす・減らす | 6.1014%（前回値） | 0px | 中央・左 | — | [差分画像](mileage-v6/vz0Ji-diff-1920.png) | **一致** |
| 17 | `p9CcEB` | 17-1-G マイルの使い道をつくる | 6.2012% | +701px | 上部・右 | docs | [差分画像](mileage-v6/p9CcEB-diff-1920.png) | **一致** |
| 17 | `k8VCU` | 17-1-H たまる決めごと・一覧の状態 | 4.0657%（前回値） | +512px | 上部・左 | — | [差分画像](mileage-v6/k8VCU-diff-1920.png) | **一致** |
| 17 | `z3PB2` | 17-2 行動スコア | 5.2937%（前回値） | 0px | 上部・左 | — | [差分画像](mileage-v6/z3PB2-diff-1920.png) | **一致** |
...
| 29 | `ugP5y` | 29-1 イベント予約 | 4.4000%（前回値） | 0px | 中央・左 | — | [差分画像](events-v6/ugP5y-diff-1920.png) | **一致** |
| 29 | `MKrPY` | 29-1-A イベントをつくる | 6.0708%（前回値） | +642px | 中央・右 | — | [差分画像](events-v6/MKrPY-diff-1920.png) | **一致** |
| 29 | `i5SN2j` | 29-1-B 申込者の一覧 | 3.3306%（前回値） | 0px | 上部・左 | — | [差分画像](events-v6/i5SN2j-diff-1920.png) | **一致** |
| 29 | `k5m5Bc` | 29-1-C 一覧の状態（空・読込・エラー） | 3.7226%（前回値） | 0px | 上部・左 | — | [差分画像](events-v6/k5m5Bc-diff-1920.png) | **一致** |
...
| 30 | `I3ZSrU` | 30-1-C 人を招待する | 4.8181%（前回値） | -16px | 中央・左 | — | [差分画像](staff-v6/I3ZSrU-diff-1920.png) | **一致** |
| 31 | `c4R6F` | 31-1 機能設定 | 7.5002% | +81px | 中央・右 | docs | [差分画像](settings-v6/c4R6F-diff-1920.png) | **一致** |
| 32 | `UgonK` | 32-1 運用状態・健全性チェック | 5.1817%（前回値） | +75px | 中央・左 | — | [差分画像](operations-v6/UgonK-diff-1920.png) | **一致** |
| 32 | `b3HfZ` | 32-1-A 緊急コントロール | 5.3686%（前回値） | -56px | 中央・右 | — | [差分画像](operations-v6/b3HfZ-diff-1920.png) | **一致** |
| 32 | `UhC2O` | 32-1-B 更新履歴 | 4.9458%（前回値） | +445px | 中央・左 | — | [差分画像](operations-v6/UhC2O-diff-1920.png) | **一致** |
