...
| 名前 | ID | 用途 |
|---|---|---|
| **Sidebar V4** | `xxgIK` | 実装の共通サイドバー。今後V4へ移す全画面で使用 |
| Sidebar V2 | `khQh9` | 未改修画面の比較用。共通レイアウトの正本にはしない |
| Sidebar（V1） | `UGV7c` | 旧デザイン用。**参照しない** |

共通レイアウト・サイドメニューはV4 `hmBzC` 内の `xxgIK` を正本とする。
V2の選択状態は未改修画面の比較にだけ使い、共通レイアウトへ戻さない。
...
ダッシュボード `/` は、下記V4を実装の正とする。V2のノードは履歴・他画面との
比較用として残す。

| 画面 | ノードID | ルート |
|---|---|---|
| V4 ダッシュボード | `hmBzC` | `/` |
| V4 ダッシュボード編集 | `kuTR2` | `/?edit` |
| V4 友だち追加QRコード | `QEThZ` | `/?qr` |

V4の共通寸法は、1920pxフレーム、サイドバー256px、本体1664px、本体の
左右余白40px（実コンテンツ1584px）。ダッシュボードだけの例外ではなく、
今後V4へ変更する受信箱・友だちなどにも同じ共通レイアウトを使う。

## V4（友だち属性）

友だち属性は、下記8状態を1920pxで書き出し、同じ状態のChrome実装画像と比較する。
`V4 4-1 series` や `V4 4-1-A` は画面名であり、実装・検証では下記の実ノードIDを使う。

| 画面状態 | ノードID | 状態 |
|---|---|---|
| 友だち属性・タグ一覧 | `xn98K` | 比較済み |
| タグを作る・初期状態 | `wdLjf` | 比較済み |
| タグを作る・連動ON | `KYG39` | 比較済み |
| 連動アクション追加ドロワー | `X79IHW` | 比較済み |
| タグを編集・既存設定あり | `t7Mhgl` | 確認済み |
| 遡及反映の確認ダイアログ | `lNGcs` | 確認済み |
| タグ削除の確認ダイアログ | `NU4N2` | 確認済み |
| 属性フォルダを追加・色編集 | `FORVU` | 確認済み |

比較画像は `docs/design-reference/friend-attributes-v4/` と
`docs/design-qa/friend-attributes-v4/` に、同じ番号・状態名で保存する。
...
| V4 4-1 友だち属性・タグ（静的確認用V3） | `xn98K` | `/tags-v3` |
