...
友だち属性は、一覧とタグ作成の実ノードIDが未記録のため、**V4実装完了ではない**。
`V4 4-1 series` や `V4 4-1-A` は画面名であり、ノードIDとして使わない。

| 画面状態 | ノードID | 状態 |
|---|---|---|
| 連動アクション追加ドロワー | `X79IHW` | 確認済み |
| タグを編集・既存設定あり | `t7Mhgl` | 確認済み |
| 遡及反映の確認ダイアログ | `lNGcs` | 確認済み |
| タグ削除の確認ダイアログ | `NU4N2` | 確認済み |
| 属性フォルダを追加・色編集 | `FORVU` | 確認済み |
| 友だち属性4タブの一覧 | 未取得 | 実装開始ゲート未通過 |
| タグを作る | 未取得 | 実装開始ゲート未通過 |

残りの実ノードIDを取得し、`docs/pendev-v4-implementation-runbook.md` の比較を終えるまで、
`/tags` と `/tags/new` をV4検証済みに変更しない。
