...
17枚すべて `design-structure.json` に登録済み。崩れたらテストが止める。

### 骨格すら見ていない画面

`docs/design-node-ids.md` の Tier 3 のうち イベント4（作成3段階＋編集）と
...
|---|---|---|
| #83 | 根っこ C・E | 緑。**マージ待ち** |
| #84 | 予約7画面＋イベント2画面 | #83 の上に積んである。**#83 を先にマージすること** |

配布はまだしていない。#83 → #84 の順にマージしてから、
ロックを取って検証環境へ。マイグレーションはどちらにも無い。

## 次にやること（順番）

### 1. 一覧APIに `visible_tag_id` を足す（小さい）

イベント一覧の「申込条件」がこれだけで出る。列はもうある。
判断待ち 16-1。

### 2. イベントの作成を3段階に分ける（中くらい）

設計 8-3-2 / 8-3-3 / 8-3-4。いまは `event-form.tsx`（1323行）を
new と edit で共有した1枚の長いフォーム。

`create-page.tsx` の `FormSection` / `AsideCard` / `aside` がそのまま使える。
予約の作成画面2枚（8-2-1 / 8-2-2）が見本になる。
