...
2. (決定済み 2026-09-03)tags のテナント分離 → **当面は単一組織**。`tenants` は将来用として残し実装しない。正本は索引 §5-1。
3. **未着手。Lステップ検証用アカウントの契約(月 2,980 円〜)。** 費用が出るため、エージェントでは契約しない。人が契約したうえで、未確認前提 10 件のうち影響「高」3 件を実機で潰す。
4. **未着手。本番 Cron Trigger の実設定確認(Cloudflare ダッシュボード)。** ダッシュボードの権限が無く、エージェントからは確認できない。
5. (決定済み 2026-09-03)Pencil の主ボタン文字色 → **白文字が乗る緑は `$accent-deep`(#087A3E・5.44:1)**。

   検討した #06a94a は 3.10:1 で、**18.66px未満の太字に必要な 4.5:1 に届かない**
   (ボタンのラベルは `$size-label` = 13px・太字600)。新しい色を作らず既存の
   `$accent-deep` を使い、`$accent`(#06c755) は文字が乗らない用途に残した。
   Pencil側は反映済み(白文字が緑に乗っていた **399箇所**)。正本は索引 §5-2。
