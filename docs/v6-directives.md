...
4. Pencil 側: 横断レビュー §7 の 50 件のうち未反映分を反映し、反映した行に日付を書く。設計画像が無い機能(4、9〜32)を書き出す。主ボタンは既存トークン $accent-deep(#087A3E)+ 白文字に変える(決定済み、索引 §5-2)。LINE の緑 #06c755 は選択状態・チップ・有効表示に残す。
...
| 主ボタンの文字色 | 既存トークン `$accent-deep` #087A3E + 白文字(5.44:1)(2026-09-03 更新。初版は #068a3c) | #06a94a(3.10:1)では 13px 太字に要る 4.5:1 に届かない。#068a3c(4.46:1)は届くが Pencil のトークン表に無い新色で「トークンだけを使う」に反するため、既にある `$accent-deep` を採る。LINE の緑 #06c755 は選択状態・チップ・有効表示に残す。Pencil を先に直し、実装は `--color-accent-deep` をそのまま使って同値トークンを増やさない。正本は索引 §5-2 |
