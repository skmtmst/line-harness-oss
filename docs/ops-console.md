...
- チケットの解決と同じDBトランザクションで生成ジョブを登録。運営の問い合わせ詳細を開いている間、書込権限とAI利用可否を確認し、`POST /api/ops/knowledge/tickets/:id/process` でその問い合わせだけを処理する。解決の保存や返信入力とは別通信で、生成結果はナレッジ部分だけに反映する。既存5分ジョブも同じ排他処理を使用するが、検証環境のCronは有効にしない。通知・配信ジョブは呼ばない。
- 生成は45秒上限のHTTP応答を待ち、応答後30秒までの `waitUntil` には載せない。画面を閉じた場合の完了は保証しないが予約は残り、再度対象の詳細を開くと再開する（切断で実行中のまま残った予約は5分のリース満了後）。これは常駐ワーカーではなく、画面を一切開かない状態での処理保証はない。AI失敗は解決操作を取り消さず、最大3回の試行後は画面から再試行できる。3回目の切断もリース満了後に再試行可能。通信・権限エラーでは自動呼出しを止め、既存のエラー欄で知らせる。
...
- 成功確認が取れない問い合わせでも、運営の返信があれば「回答例（お客様の確認なし）」を未承認の下書きとして残す。質問は最初の運営返信より前のお客様発言、答えは運営の全返信を匿名化して原文順に組み立て、追加のAI呼び出しはしない。運営返信が無い場合は質問だけを「要確認」で残し、運営が答えを書ける。回答例は解決実績として扱わず、運営が元のやり取りと照合して承認するまで検索対象にしない。
...
- migration `442_platform_knowledge.sql` と `445_platform_knowledge_answer_examples.sql` は列・表・索引の追加のみ。445 は既存記事を `verified` とし、新しい回答例を `answer_example` で区別する。未適用のままコードを配備しない。新しい環境変数・Webhook・OAuth・Cron設定は不要。既存AI bindingと `OPS_SUPPORT_AI_MODEL` を使用。
...
## 契約先の停止・保管の実効化（OPS-d-1）

- `tenants.status` が `suspended` または `archived` の契約先も、メール・LINE・2要素認証の通常手順を終えると停止中セッションを発行する。停止中セッションは `GET /api/auth/session`、`POST /api/auth/logout`、`/api/hq/support/*`、運営からのお知らせの取得と既読だけを使え、それ以外の管理 API は `TENANT_SUSPENDED` で拒否する。許可経路は `middleware/auth.ts` の一覧だけを正本にする。
- 統括画面は V6 `CXFjb9`（「★ V6 0-1-A ログイン・契約先が停止中（1920）」）に合わせ、通常画面を停止案内と `/hq/support` への導線に差し替える。お問い合わせは V6 `IwfA0`（「★ V6 0-1-B 停止中・お問い合わせ（1920）」）の専用外枠で、メニューを「お問い合わせ」だけにし、LINE アカウント切替を出さない。
- 運営が契約先を `active` に戻すと、次のセッション確認から同じセッションのまま通常画面へ戻る。再ログインは求めない。
- 停止中も LINE の受信と友だち状態は保存するが、自動応答、シナリオ、自動化、リッチメニュー切り替え、その他の外部送信は行わない。LIFF・公開 API の読み書きも 503 で止める。
- 予約済みの配信・リマインド・チャット送信等は終端のスキップ状態へ移し、復帰後に期限切れの送信が自動で流れないようにする。新しい予約と通常操作は `active` へ戻すと再開する。
- 運営マスターの有効な代理ログインだけは停止中の契約先を閲覧できる。書き込み制限は従来の代理ログイン規則を維持する。
- DB migration、新しい環境変数、Webhook・OAuth・Cron 設定の変更はない。
