...
- 正本: V6 `P5egpk`（一覧）、`twq67`（根拠確認）、`MGgKT`（編集）。FAQや公開の操作は置かない。運営限定。
...
- migration `441_platform_knowledge.sql` は列・表・索引の追加のみ。未適用のままコードを配備しない。新しい環境変数・Webhook・OAuth・Cron設定は不要。既存AI bindingと `OPS_SUPPORT_AI_MODEL` を使用。
