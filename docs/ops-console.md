...
- `tenants.status` が `suspended` または `archived` の契約先は、メール・LINE ログインで通常セッションを発行しない。管理 API も `TENANT_SUSPENDED` で拒否する。`/api/auth/session`、ログアウト、お問い合わせ、運営からのお知らせは例外として使える。
- 統括画面は V6 `CXFjb9`（「★ V6 0-1-A ログイン・契約先が停止中（1920）」）に合わせ、停止中はその案内と `/hq/support` への導線に差し替える。
