...
## 4.5 PR を受け取ったらどうするか(列車方式、2026-09-06)

保護ルールで「ブランチが最新」でないとマージできないため、1 本ずつマージすると、1 本入れるたびに残りの PR 全部が最新化と必須ゲートの再実行(約 6 分)に戻る。これを止めるため、司令塔は PR を **列車**にまとめて通す。

**列車を出す条件**: ゲート合格(required-pr-gate SUCCESS)・競合なし(CLEAN か BEHIND)の PR が 2 本以上そろったとき。または 1 本が 60 分待ったとき。docs だけの PR も同じ列車に乗せる(ゲートを別に走らせない)。
...
6. 列車が入ったあと、競合(DIRTY)になった PR にだけ Issue で「取り込み依頼」を書く。

**lane(Codex)の約束**: PR を出したらその PR に触らない(update-branch もマージもしない)。マージを待たず次のチケットへ。取り込み依頼が来たときだけ `git fetch origin && git merge origin/codex/development` → 解消 → push(rebase・force push なし)。題名の (PR #番号) を最新に保つ。

**ゲートそのものの時間**は Codex の台帳 #260(キャッシュ・並列・docs だけの PR の短縮)で縮める。
