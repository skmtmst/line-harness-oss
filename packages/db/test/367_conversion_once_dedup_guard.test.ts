...
/** 367号だけを当てる最小構成。既存重複があっても止まらないことを見る。 */
function setupMinimal(): Database.Database {
  ...
  db.exec(`
    CREATE TABLE conversion_points (
      ...
      count_repeat INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL
    );
...
describe('367号 migration: 既存重複の決定的整理', () => {
  it('重複があっても止まらず承認済み(計上済み)を残す', () => {
    const db = setupMinimal();
    db.exec(`
      INSERT INTO conversion_points (id, name, event_type, count_repeat, created_at)
      VALUES
        ('point-once', '購入完了', 'purchase', 0, '2026-08-01'),
        ('point-repeat', '来店', 'visit', 1, '2026-08-01');
    ...
    // 1人1回地点の重複は承認済み(計上済み)を優先して1件に決定的に整理する。
    expect(onceRows.map((r) => r.id)).toEqual(['event-later-approved']);
    expect(onceRows[0].once_key).toBe('point-once|friend-a');
    const repeatRows = db.prepare(
      `SELECT id, once_key FROM conversion_events WHERE conversion_point_id = 'point-repeat' ORDER BY id`,
    ).all() as Array<{ id: string; once_key: string | null }>;
    // 繰返し数える地点の複数記録は正規なので残し、制約の対象外(NULL)のままにする。
  ...
  it('整理後は同じ1人1回キーで2件目を書けない', () => {
    const db = setupMinimal();
    db.exec(`
      INSERT INTO conversion_points (id, name, event_type, count_repeat, created_at)
      VALUES ('point-once', '購入完了', 'purchase', 0, '2026-08-01');
    `);
    ...
    // 繰返し地点の行(NULLキー)は何件でも書ける。
    db.prepare(
      `INSERT INTO conversion_events (id, conversion_point_id, friend_id, created_at, once_key)
       VALUES ('e3', 'point-once', 'friend-a', '2026-08-03', NULL)`,
    ).run();
