...
describe('common variable usage impact', () => {
  it('returns only the selected account while keeping unscoped forms safe', async () => {
    ...
    raw.exec(`
      ...
      VALUES ('b1','予約中','text','{{var.shop_hours}}です','all','scheduled','a1'),
             ('b2','送信済み','text','{{var.shop_hours}}でした','all','sent','a1');
      ...
      INSERT INTO forms (id, name, fields, on_submit_message_content)
    ...
    expect(impact).toMatchObject({
      ...
      historicalTotal: 1,
      unscopedFormTotal: 1,
      byKind: { template: 1, broadcast: 2, scenario: 1, form: 1 },
    });
    expect(impact.items.map((item) => item.source_id)).toEqual(['t1', 'b1', 'b2', 'ss1']);
