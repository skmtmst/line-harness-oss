...
const migration = readFileSync(
  join(import.meta.dirname, '..', 'migrations', '199_conversion_history_safety.sql'),
  'utf8',
);
...
describe('成果地点と過去実績の安全性', () => {
  ...
  it('同じ外部イベントの冪等キーを重複保存できない', () => {
    const sqlite = setup();
    sqlite.prepare(`
      INSERT INTO conversion_events
        (id, conversion_point_id, friend_id, created_at, idempotency_key)
      VALUES (?, 'point-a', 'friend-a', '2026-08-03', ?)
    `).run('event-b', 'order-1:version-1');
    expect(() => sqlite.prepare(`
      INSERT INTO conversion_events
        (id, conversion_point_id, friend_id, created_at, idempotency_key)
      VALUES (?, 'point-a', 'friend-a', '2026-08-04', ?)
    `).run('event-c', 'order-1:version-1')).toThrow(/UNIQUE constraint failed/);
  });
});
