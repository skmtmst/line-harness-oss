...
export interface ReminderRow {
  ...
  /** 251: 下書き／公開中／停止中を is_active だけに潰さない。 */
  lifecycle_status: 'draft' | 'published' | 'stopped';
  /** 251: 後編集する版。公開版とは別行に固定する。 */
...
/** 予約取消・期限超過など、登録元のできごとが無効になったときに残りの通を止める。 */
export async function cancelReminderEnrollmentsForSource(
  db: D1Database,
  input: { sourceKind: string; sourceId: string; reason: string },
): Promise<void> {
  const now = jstNow();
  // 先に登録を止める。2本目が失敗してもcronの取得条件から外れるため、
  // 取消後に送ってしまう半端な状態にはならない。
  await db.prepare(
      `UPDATE friend_reminders
          SET status = 'cancelled', cancel_reason = ?, lock_version = lock_version + 1,
              updated_at = ?
        WHERE source_kind = ? AND source_id = ? AND status = 'active'
          AND (
            reminder_version_id IS NULL
            OR EXISTS (
              SELECT 1
                FROM reminder_versions rv
               WHERE rv.id = friend_reminders.reminder_version_id
                 AND COALESCE(
                   json_extract(rv.settings_snapshot, '$.stopConditions.bookingCancelled'),
                   1
                 ) = 1
            )
          )`,
    ).bind(input.reason, now, input.sourceKind, input.sourceId).run();
  await db.prepare(
    `UPDATE reminder_delivery_runs
        SET status = 'cancelled', next_retry_at = NULL, lease_expires_at = NULL,
            completed_at = COALESCE(completed_at, ?), updated_at = ?
      WHERE friend_reminder_id IN (
        SELECT id FROM friend_reminders
         WHERE source_kind = ? AND source_id = ? AND status = 'cancelled'
           AND (
             reminder_version_id IS NULL
             OR EXISTS (
               SELECT 1
        ...
        AND status IN ('planned', 'claimed', 'retry_wait')`,
  ).bind(now, now, input.sourceKind, input.sourceId).run();
}

/** 1人の登録を止める。ブロック・期限超過など送信直前の停止条件に使う。 */
export async function cancelReminderEnrollment(
  db: D1Database,
  input: { friendReminderId: string; reason: string; now?: string },
): Promise<void> {
  ...
  await db.prepare(
    ...
    ).bind(input.reason, now, input.friendReminderId).run();
  await db.prepare(
    `UPDATE reminder_delivery_runs
        SET status = 'cancelled', next_retry_at = NULL, lease_expires_at = NULL,
            completed_at = COALESCE(completed_at, ?), updated_at = ?
      WHERE friend_reminder_id = ?
        AND status IN ('planned', 'claimed', 'retry_wait')`,
  ).bind(now, now, input.friendReminderId).run();
}
...
/** 定義と版を1回のD1 batchで作り、版だけ／親だけが残る半端な下書きを作らない。 */
export async function createReminderWithDraftVersion(
  ...
  settings.steps.forEach((step, position) => {
    statements.push(
      db.prepare(
        ...
        crypto.randomUUID(), versionId, step.stableStepId, position,
        step.offsetMinutes, step.messageType, step.messageContent,
        step.offsetDays ?? null, step.sendAtTime ?? null, step.templateId ?? null,
        JSON.stringify(step.targetCondition ?? {}), JSON.stringify(step.action ?? {}), now,
      ),
    );
  });
...
async function replaceReminderVersionSteps(
  db: D1Database,
  versionId: string,
  steps: ReminderDraftStepInput[],
  now: string,
...
/**
 * 公開版は触らず、編集用の版だけを作る／更新する。
 * 公開済みしか無い場合は次の版番号で下書きを作る。
 */
export async function saveReminderDraftVersion(
  ...
  const now = jstNow();
  let draft = await getReminderDraftVersion(db, reminderId);
  ...
  if (!draft) {
    ...
    await db.batch([
      ...
      db.prepare(
        `UPDATE reminders
            SET current_draft_version_id = ?, lifecycle_status = CASE
                  WHEN current_published_version_id IS NULL THEN 'draft'
    ...
    draft = await getReminderVersionById(db, versionId);
  } else {
    await db.prepare(
    ...
    ).bind(JSON.stringify(settings), now, draft.id).run();
  }
  if (!draft) throw new Error('REMINDER_DRAFT_NOT_SAVED');
  await replaceReminderVersionSteps(db, draft.id, settings.steps, now);
  return (await getReminderVersionById(db, draft.id))!;
}
...
/** 公開版を固定し、既存の登録は以前の reminder_version_id のまま残す。 */
export async function publishReminderDraftVersion(
  ...
  const versionSteps = await db.prepare(
    `SELECT * FROM reminder_version_steps
  ...
  // 互換表は削除しない。過去の配信履歴が参照するstable stepを残したまま、
  // 新しい公開版の内容だけをupsertする。
...
export async function getReminderVersionSteps(
  ...
  const result = await db.prepare(
    `SELECT stable_step_id AS id, rv.reminder_id, rvs.offset_minutes,
...
export async function enrollFriendInReminder(
  ...
  await db.prepare(
    ...
    input.friendId,
    input.reminderId,
    reminder.current_published_version_id ?? null,
    input.targetDate,
...
export async function completeReminderIfDone(db: D1Database, friendReminderId: string, reminderId: string): Promise<void> {
  ...
  if (totalSteps && deliveredSteps && deliveredSteps.count >= totalSteps.count) {
    const now = jstNow();
    await db.prepare(
      `UPDATE friend_reminders SET status = 'completed', completed_at = ?, updated_at = ? WHERE id = ?`,
...
export async function retryReminderDeliveryRun(
  ...
  const [changed] = await db.batch([
    db.prepare(
      `UPDATE reminder_delivery_runs
          SET status = 'planned', retry_cycle_attempt_count = 0,
...
export async function getReminderDeliveryRunSummary(
  ...
  const row = await db.prepare(
    `SELECT
       ...
       SUM(CASE WHEN status IN ('planned', 'claimed', 'retry_wait') THEN 1 ELSE 0 END) AS scheduled,
       ...
       MIN(CASE
         ...
         WHEN status IN ('planned', 'claimed') THEN scheduled_at
