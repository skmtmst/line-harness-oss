...
export const SCREENS = [
  ...
  */
  ...
  { ...SCENARIO, node: 'TC1b1', name: '5-1 シナリオ配信', route: '/scenarios',
    ...
    verdictSource: 'scenarios-v6/TC1b1.txt', verdictHead: '2d0ee180',
  },
  { ...SCENARIO, node: 'cCB7r', name: '5-1-A シナリオ作成・配信方式', route: '/scenarios/mode?id=scenario-0',
    ...
    verdictSource: 'scenarios-v6/cCB7r.txt', verdictHead: '2d0ee180',
  },
  ...
  },
  {
    ...
    */
    ...
    steps: [{ click: '再開' }],
    verdict: 'needs_fix',
    verdictNote: '**development `2d0ee180` で撮ったが、確認の窓を開けていない。** ルート `/scenarios`。**撮影の段が窓を開けられず、一覧のまま撮れている**（本文に配信前チェックの4項目が0件、窓の題も出ない）。**「無い」ではなく「確かめられていない」。** **P1 設計の配信前チェック4項目**（開始条件が設定されています／すべてに配信タイミングがあります／テスト送信が完了しています／LINE公式の送信枠を超えていません）は、#521 の窓に入っているかどうかを**まだ見ていない**。 **残作業**：撮影の段を直して窓を開き、4項目の有無を数える。**押し口に `data-qa-open="RUxNf"` が付いていれば、文言に頼らず開ける。** **推奨修正**：4項目のうち「開始条件が設定されています」「すべてに配信タイミングがあります」は**シナリオの設定から導ける**ので画面だけで出せる。「テスト送信が完了しています」「送信枠を超えていません」は**取得元が無いので `—（未取得）`** とする。`s6Vvp`（リマインダの最終確認）が同じ形の公開前チェック5項目を既に持っているので、その部品を写す。取得元：`scenarios-v6/RUxNf.txt`（一覧のまま）',
    verdictSource: 'scenarios-v6/RUxNf.txt', verdictHead: '2d0ee180',
  },
  {
    /*
  ...
  },
  {
    ...
    verdictSource: 'scenarios-v6/q5G45-error.txt', verdictHead: '2d0ee180',
  },
  ...
  */
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix', verdictNote: 'P2 設計の「先に差し替えてから削除する」流れが無い。設計は使用先を3つ挙げ、差し替え画面へ誘い、そのまま消すときは名前を打ち直させる。実装は使用中のテンプレートから削除の導線ごと外す作りで、危険は無い代わりに使用中のものを整理する手段が無い。窓にも使用先が出ない **ルート**：`/templates`（削除の窓）。**取得元**：`templates-v6/design-qa.md`。**推奨修正**：**窓に使用先を出すのが先**。いまは使用中のテンプレートから削除の導線ごと外す作りで危険は無いが、**使用中のものを整理する手段も無い**。`uNBlA`（変える前に影響を見る）が同じ「使用先を数えて出す」形を既に持っているので写す。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix', verdictNote: 'P2 メニューの5項目（名前を変更・色を変える・並び順を上へ・並び順を下へ・フォルダを削除）は設計どおり（folder-panel.tsx:128-157、#493 head 62ddaebe）。ただし設計にある但し書き「削除しても、中のテンプレートは未分類に残ります。」がメニューに無い。開いた状態の撮影は #493 の木でサーバを起こさないとできないため、撮り方だけ先に入れた **ルート**：`/templates`（フォルダのメニュー）。**取得元**：`folder-panel.tsx:128-157`（#493 head `62ddaebe`）。**推奨修正**：但し書き「削除しても、中のテンプレートは未分類に残ります。」をメニューに足す。**`byqIW`（属性フォルダ）と `WuKzU`（共通情報）が同じ文言を既に出している**のでそろえる。**残作業**：#493 の木でサーバを起こして開いた状態を撮る。',
  ...
  */
  ...
  { ...RICH_MENU, node: 'XtfO3', name: '12-1-A メニューを作る・形とボタン', route: '/rich-menus/new', verdict: 'needs_fix', verdictNote: '**#583 `bb1e4dfd` で撮り直した。前に挙げた「優先の説明が無い」は、`kQ1bs` の節が入ったことで**この編集画面からは解消した**。** ルート `/rich-menus/new`。1440・1920とも横スクロール0。**P2 形とボタンが1枚に同居している。** 撮った本文は「リッチメニューを作る／名前と土台のレイアウトを決め…」で、`kQ1bs`（誰に出すか）`UMiJ9`（公開のしかた）と**同じ1枚**。設計は段で分ける。**`kQ1bs` と `UMiJ9` と3つまとめて段へ分ける**のが早い（1枚ずつ直すと同じ画面を3回触ることになる）。**`undefined`・`NaN`・`Invalid Date`・`API error`・内部ID は0件。** 取得元：`rich-menus-v6/XtfO3.txt`。**推奨修正**：段へ分けるのは `PV1Vh` `d3rFGD` `Xjk8q`（ウェビナー編集）と同じ形の作業なので、どちらかで作った段の部品を写す。', verdictSource: 'rich-menus-v6/XtfO3.txt' , verdictHead: 'bb1e4dfd' },
  ...
  { ...RICH_MENU, node: 'TL7tp', name: '12-1-E 管理画面の外のメニューを取り込む', verdict: 'needs_fix', verdictNote: '**P2 管理画面の外のメニューが、設計では別画面だが実装は一覧の中の節。** ルート `/rich-menus`。「LINE 公式アカウントの現状」の節に、LINE 上のメニューが並び、行ごとに **「管理画面に取り込む」「LINE から削除」** が出る。取り込みと削除はできる。**良い点**：「LINE 上に登録されているメニュー 3 個／うち 2 個が管理画面外」と**管理下と管理外を数えて分ける**。名前の無いメニューを「テスト用（名前なし）」と書き、空欄にしない。管理下のものは「管理画面 → 通常メニュー（会員向け）(トップ)」と**どれに対応するか**を出す。**P2 残る差**：設計 12-1-E は取り込む前に「何が上書きされるか」を出すが、実装は押すとすぐ取り込む。**この画面の並び順は #583 より前の絵**（「タップ数が多い順」が既定）。取得元：`rich-menus-v6/TL7tp.txt`。1440・1920とも横スクロール0 **推奨修正**：取り込む前に「何が上書きされるか」を出す。**`DIUbO`（切替のつながり）が既に「入口へ戻れません」を出せている**ので、同じ考え方で取り込み先の重なりを見せる。', verdictSource: 'rich-menus-v6/TL7tp.txt' , verdictHead: 'c275749d' },
  {
    ...
    */
    variants: [
      ...
      { suffix: '-managed-fail', steps: [{ click: '削除', nth: 5 }, { click: '削除する' }, { wait: 1200 }] },
    ],
    verdict: 'needs_fix', verdictNote: '**#575 `ab5750ec` で `ConfirmDialog` につながり、`status: \'unimplemented\'` を外せる状態になった。実際に押して4通りを撮った。** ルート `/rich-menus`。1440・1920とも横スクロール0。 **管理画面のメニューと、LINE上の管理外メニューが別々の窓で開く**（`kind: \'managed\' | \'external\'`）。 **① 管理画面のメニュー**（下書きの「店舗A限定メニュー」で確認）：題「**「店舗A限定メニュー」を削除しますか？**」、説明「管理画面に保存したこのリッチメニューを削除します。**LINEに登録中のメニューは、先に取り下げない限り削除できません。**」、中身は **消えるもの**（このリッチメニューの設定と画像）／**残るもの**（同じフォルダのほかのメニューと、これまでのタップ記録）／**元に戻せません。**、ボタンは「キャンセル」と赤い「削除する」。 **② LINE上の管理外メニュー**：題「「LINE公式マネージャーで作成」を削除しますか？」、説明「この管理画面外で作成されたリッチメニューを、LINE公式アカウントから削除します。」、消えるもの（**LINE公式アカウント上のこの**リッチメニュー）／残るもの（管理画面で作成・編集しているほかのリッチメニュー）／元に戻せません。、ボタンは赤い「**LINEから削除**」。**消す先が違うことが、題・説明・ボタンの3か所で分かる。** **③ 失敗しても窓が閉じない。** 撮影用の口が書き込みを405で返すので、そのまま「削除する」を押すと**窓は開いたまま**、赤帯で「**リッチメニューを削除できませんでした。状態を確認して、もう一度お試しください。**」。**内部の番号（405）も英語も出ない。** `busy`（`deleteBusy`）も窓へ渡っており、押している間の二重送信を止める。 **④ 公開中のメニューは窓を出さずに止める**——`handleDelete` が `status === \'published\'` を先に見て返す（`rich-menus/page.tsx:249`）。**LINEに残ったまま消えることは無い。** **P2 その止め方がブラウザ標準の `alert` のまま。** 「『通常メニュー（会員向け）』は LINE に登録されています。編集画面の「危険な操作」から「LINE から取り下げ」を実行してから、改めて削除してください。」と**中身は正しい**が、窓の見た目がこの画面のほかの窓とそろわない。**削除の道の中にある**ので、取り込みの `confirm` とは別に数える。**推奨修正**：この `alert` も `ConfirmDialog`（またはお知らせ用の窓）へ替える。**P1 は残っていない**——設計 `szXsT` が求めた「消える／残る／戻せない」と、LINE取り下げが先という順番は、文で明示されている。**表示中の人数・次に出るメニュー・切替元・自動応答の参照**という影響の内訳は出ないが、**それを返す口がまだ無い**ため、この画面の担当ではない（`gap` として残す）。 取得元：`rich-menus-v6/szXsT-managed-1440.png` ＋ `-external-1440.png` ＋ `-managed-fail-1440.png` ＋ `rich-menus/page.tsx:249,730`',
    verdictSource: 'rich-menus-v6/szXsT-managed-1440.png + -managed-fail-1440.png + rich-menus/page.tsx:249', verdictHead: 'ab5750ec',
  },
  {
    ...
    verdictSource: 'rich-menus-v6/RW5Tb-error.txt + RW5Tb-empty.txt + rich-menus/page.tsx:324', verdictHead: '7b8df2f4',
  },
  ...
  */
  ...
  { ...FORM, node: 'v9tYhl', name: '13-1-D 集まった回答', steps: [{ click: '来店アンケート' }], verdict: 'needs_fix', verdictNote: '**画面全体は要修正のまま。** P1 情報欄への書き込みが失敗した件数が出ない（設計は「3件は欄が消えていて書けていません」）。**答えは受け取れているのに友だち情報へ入っていない状態が、画面のどこにも出ない。** P1 「1件ずつ見る／まとめて見る」の切り替え、絞り込み、CSVで書き出すが無い ／ **#436 の受入条件は確認済み**（head `35c613a6`）。回答の表は 友だち名・日時・各項目で、**答えの無い項目は `—`**、友だちが分からない行は「不明」、未記入は「（未記入）」と書き分ける。下に **「1〜4件 / 全4件」「20件表示」「前へ 1 次へ」** が出て、APIが返した `page` / `limit` / `total` をそのまま使う。1440・1920とも横スクロール0。`undefined` / `Invalid Date` / `NaN` / 内部IDは0件 **ルート**：`/form-submissions`（集まった回答）。**取得元**：`forms-v6/v9tYhl.txt`（#436 head `35c613a6`）。**推奨修正**：**情報欄への書き込みが失敗した件数を先に出す**——答えは受け取れているのに友だち情報へ入っていない状態が、いま画面のどこにも出ない。**気づけないまま数日たつのがいちばん重い。** 切り替え・絞り込み・CSVはそのあと。', verdictSource: 'forms-v6/v9tYhl-1440.png' , verdictHead: '35c613a6' },
  {
  ...
  */
  ...
  { ...AFFILIATE, node: 'n5VVTb', name: '16-1-B 成果承認', route: '/conversions?tab=approvals', verdict: 'needs_fix', verdictNote: '**P2 成果承認。行ごとの判定はできている。** ルート `/affiliates?tab=approvals`。札は 承認待ち／承認済み／却下済み。列は 日時／友だち／アフィリエイター／案件／CVポイント／金額／**フラグ**／操作（承認・却下）。**良い点**：**注意の要る行に `⚠` が付き、要らない行は `—`**（空欄にしていない）。友だちは表示名と `friend-4…` を並べ、**同名の別人を取り違えないようにしている**。**P1 却下の理由を残す場所が無い。** 押すとすぐ却下になり、あとから「なぜ落としたか」が分からない。**P2 まとめて承認する導線が無い**（1件ずつ）。**推奨修正**：却下に理由の入力を足す。まとめ承認は `hold_days` を超えた行だけに絞ると安全。取得元：`affiliates-v6/n5VVTb.txt`。1440・1920とも横スクロール0', verdictSource: 'affiliates-v6/n5VVTb.txt' , verdictHead: 'c275749d' },
  {
    ...
    */
    ...
    verdictSource: 'affiliates-v6/njLGA-normal.txt + njLGA-error.txt + payment-tab.tsx:74', verdictHead: '3857365b',
  },
  ...
  */
  ...
  { ...AUTOMATION, node: 'xOpDs', name: '25-2 共通アクション', route: '/common-actions', verdict: 'match', verdictNote: '**development `2d0ee180` で撮り直した。前に挙げたP2（帯に「古い版のまま 要確認」が無い）は解決済み。** ルート `/common-actions`。1440・1920とも横スクロール0。 **帯が設計の4つになった**：共通アクション 4「公開中と下書き」／中の処理 12「表示中の合計」／呼び出し場所 15「固定している利用先」／**古い版のまま ＋「要確認」の札**「新しい版へ更新できます」。**数は既存の口が返している**（一覧が `oldVersionBindingCount` `bindingCount` を持つ）ので、新しいAPIは要らなかった。 **「複製して下書きを作る」が行に入った**（数えて4件）。**公開されたものを直接作らず、下書きにする**のが安全。 **タブ帯**（オートメーション／共通アクション 4）も #552 で入っている。表の列は アクション名／状態／中の処理／呼び出し元／版／操作。 **残る差（P2、判定は変えない）**：設計の3つめは「今月2,847回・失敗6」だが、実装は「中の処理」。**実行回数と失敗は集計の口が要る**ので、この画面の担当ではない（`gief7` と同じ根）。取得元：`automations-v6/xOpDs.txt`', verdictSource: 'automations-v6/xOpDs.txt' , verdictHead: '2d0ee180' },
  { ...AUTOMATION, node: 'py5CG', name: '25-2-A 共通アクションをつくる', route: '/common-actions/new', verdict: 'match', verdictNote: '**development `2d0ee180` で撮り直した。前に挙げたP2（「複製して作る」が無い）は解決済み。** ルート `/common-actions/new`。1440・1920とも横スクロール0。 **複製は一覧の行に入った**——`xOpDs` の各行に「**複製して下書きを作る**」があり（数えて4件）、**新規画面を増やさずに済んでいる**。前の推奨修正どおりの直り方。**公開されたものを直接作らず下書きにする**ので、押し間違いで本番のルールが増えない。 **つくる面の作りは設計に近い**：段は 名前と説明 → 順番に動かす処理 → 失敗したとき。処理は12種類（タグを付ける／外す・友だち情報を設定・シナリオを開始/停止/再開・LINEメッセージを送る・外部サービスへ送る・リッチメニューを切り替える/外す・待つ・別の共通アクションを呼ぶ）。「上から順に実行します。**公開後の版は書き換わりません。**」と、版の決めごとを作る前に書く。**タブ帯の位置の差は #552 で解消。** 取得元：`automations-v6/py5CG.txt` ＋ `automations-v6/xOpDs.txt`', verdictSource: 'automations-v6/py5CG.txt + xOpDs.txt' , verdictHead: '2d0ee180' },
  { ...AUTOMATION, node: 'syWp4', name: '25-2-B 共通アクションの版と使われている場所', route: '/common-actions/versions?id=ca-1', verdict: 'needs_fix', verdictNote: '**development `2d0ee180` で撮り直した。** ルート `/common-actions/versions?id=ca-1`。1440・1920とも横スクロール0。 **この画面はよくできている**：帯は 現在の公開版 4「v4を利用できます」／版の数 4「下書きを含む」／使われている場所 2「版を固定した利用先」／**古い版のまま 1「確認して更新します」**。「新版を公開しても、利用先は現在の版を使い続けます。差分を確認した利用先だけ切り替えてください。」「公開した版は書き換えられません。」と、**公開しても勝手に切り替わらないこと**を書く。**古い版のまま呼んでいる先を数えて出せている**のが設計の要。 **P2 残る差**：設計の帯は 共通アクション14／呼び出し元38・**5機能から**／今月2,847回・失敗6／古い版のまま 要確認2 の4つ。実装は**1本ぶんの版の話**に寄っており、**「呼び出し元が何機能からか」と「今月の実行・失敗」が出ない**。**推奨修正**：呼び出し元の機能数は既存の `bindingCount` の内訳から出せる見込みがあるが、**今月の実行・失敗は集計の口が要る**ので `xOpDs` `gief7` と同じ束で扱う。**この画面だけでは終わらない。** 取得元：`automations-v6/syWp4.txt`', verdictSource: 'automations-v6/syWp4.txt' , verdictHead: '2d0ee180' },
  ...
  */
  ...
  { ...BOOKING, node: 'SbuUI', name: '27-1-C 今週の予約', steps: [{ click: '今週' }], verdict: 'needs_fix', verdictNote: 'P1 今週の予約の面が設計とそろわない。時間×担当の格子でないため、空きが面で分からない **ルート**：`/booking/bookings`（今週の予約）。**取得元**：`booking-v6/design-qa.md`（この画面の `.txt` は取れていない）。**推奨修正**：`TV2DI` と同じ束。**電話の予約が台帳へ載るようにするのが先**で、格子はそのあと——載らないまま格子にしても、空きが本当の空きにならない。', verdictSource: 'booking-v6/design-qa.md' , verdictHead: 'c275749d' },
  /*
    代理予約の入力を、実際に通す。
  ...
  */
  ...
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**#459 で登録完了までつながった。実際に登録して撮った。** 予約IDは**返事の `booking_id` をそのまま出す**（固定値ではない）。Googleカレンダーも `calendar_sync` の4つの値を読み分ける。1440・1920とも横スクロール0。P1 **設計が完了画面で見せたかった2つが出ない**。ひとつはリマインダの具体時刻「9/01(月) 19:00 に前日のお知らせ ／ 9/02(火) 8:00 に当日のお知らせ」、もうひとつは「成果地点「予約が入った」を1件 数えました。**電話の予約も同じように数えます。**」。代理で入れた予約がLINEの予約と同じ扱いになる、という設計の要点がここで確かめられない。**ただし断り方は正しい**——実装は `—（完了APIへの接続が必要）` と書き、**取得元のない時刻を作らない** **ルート**：`/booking/bookings/new`（登録完了）。**取得元**：`booking-v6/GfceK.txt`（実際に登録して撮った）。**推奨修正**：リマインダの具体時刻と「成果地点『予約が入った』を1件 数えました。電話の予約も同じように数えます。」を出す。**代理の予約がLINEの予約と同じ扱いになる、という設計の要点がここでしか確かめられない。** 予約IDを返事の `booking_id` からそのまま出す形は維持する。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**#562 `45789965` で、記録していたP1が直った。実際に重なりを起こし、選び直して登録まで通した。** ①**`API error: 409` は出ない**——赤帯は「選んだ時間は、ほかの予約で埋まりました」。②**回復画面へ進む**——「この時間には予約を入れられません／最新の空き時間を読み直して、別の時間を選んでください。入力したお客様・メニュー・担当者・要望は残っています。」と「空いている時間を選び直す」。③**選び直したあと登録を完了できる**——押して10:00を選び直すと「予約を登録しました」まで進み、予約IDが出る。直し方も良い：文字で見分けるのをやめ、`ApiError.code` を足して**機械コードと人へ見せる文を別の契約に分けた**。`extractApiErrorCode` は `^[a-z][a-z0-9_]{0,63}$` に絞るので、SQLや外部APIの文言はコードとしても入らない。1440・1920とも横スクロール0。`API error` / `Failed to fetch` / `undefined` / `NaN` はどこにも無い。P2 設計は重なりを**登録前に**止める（「9/02(火) 11:00 は 佐々木 がふさがっています（2件）」）。実装は登録を試して初めて分かる **ルート**：`/booking/bookings/new`（重なりと入力エラー）。**取得元**：`booking-v6/Lg8ff.txt`（実際に重なりを起こし、選び直して登録まで通した）。**推奨修正**：**`ApiError.code` で機械コードと人へ見せる文を分ける直し方を、ほかの失敗表示へ写す**（`Y0Sn3` の `API error: 405` がまさに同じ問題）。差は残っていない。',
  ...
  */
  ...
  { ...EVENT, node: 'i5SN2j', name: '29-1-B 申込者の一覧', route: '/events/bookings?id=ev-1', verdict: 'needs_fix', verdictNote: '**#467 `6bb950f3` で予約者の一覧が撮れるようになった。** 帯は 申込 0人（定員12・残り12）／承認待ち 0件（対応が必要）／**キャンセル待ち 2人**／キャンセル 0件。状態の札は 承認待ち・確定・拒否・キャンセル・期限切れ・参加済・無断・キャンセル待ち・全件 で、**内部の語（`confirmed` `requested` `waiting` `invited`）は0件**。1440・1920とも横スクロール0。**画面全体は要修正のまま**：P2 設計は「あと少しで満席」「声をかけると埋まります」のように、次にする行動を帯へ出す。実装は数まで。**撮影の断り**：キャンセル待ちの固定データを2人入れているため、帯が「2人／受け付けない設定です」と並ぶ。イベント側の受付設定と別の値なので、実装の食い違いではない **ルート**：`/events/bookings?id=ev-1`。**取得元**：`events-v6/i5SN2j.txt`。**推奨修正**：`ugP5y` と同じで、帯に次にする行動を出す。**内部の語（`confirmed` `requested` `waiting` `invited`）が0件なのは維持する。** 帯の「キャンセル待ち2人／受け付けない設定です」の並びは**撮影用の固定データがイベント側の受付設定と別の値だからで、実装の食い違いではない**。', verdictSource: 'events-v6/i5SN2j.txt' , verdictHead: '6bb950f3' },
  {
    ...
    verdictSource: 'events-v6/k5m5Bc-error.txt + k5m5Bc-empty.txt', verdictHead: 'c9d33d95',
  },
...
export const CAPTURED_AT = {
  ...
  */
  ...
  18: [
    ...
    { pr: 589, head: '3d469740', on: '2026-08-30', screens: ['TEVk8'], note: 'Claudeが直した。タグをフォルダで束ねる。直した本人が比較している' },
  ],
  19: [
