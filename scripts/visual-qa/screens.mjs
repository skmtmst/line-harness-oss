...
export const SCREENS = [
  ...
  */
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '#645（head `6e9ed4d6`）で **/friends の中に一括操作を作った。未実装から外した。** #606 の契約（`friendBulkRuns`）を使い、流れは対象選択→操作選択→サーバーで再計算→最終確認→結果。**選んだ4人がそのまま対象ではない**——確認窓に 選んだ人4人／実際の対象3人／対象外1人、アカウント別内訳、対象外の理由、選択した友だちを出す。未取得では実行させず、対象0人でも走らせない。**owner/admin 以外には押し口を出さない**。取り消せない操作は窓に明記して追加確認、冪等キーは操作ごと、409は「読み直す」。やり直しは失敗した対象だけ、取り消しは取り消せる操作だけ。成功・見送り・一時失敗・恒久失敗を別の言葉にする。通常・操作選択前・取得失敗・権限不足・一部失敗の5状態を撮った（10枚、はみ出し0、壊れ値と内部ID0件）。**残る差**：設計は9つの操作タイルを持つ別ページ。今回は画面だけで組み立てられるタグの付け外しに限り、ほかは口の形が要るので**選べるように見せていない**（契約待ち）',
    verdictSource: 'Claude実装',
    verdictHead: '6e9ed4d6',
  },
  { ...FRIENDS, node: 'I6UAdr', name: '3-1-D 友だち詳細', route: '/friends/detail?id=friend-0', verdictNote: '**P1 設計の「来店回数」「前回の申し送り」「つながる先」が無い。** ルート `/friends/detail?id=friend-1`。実装が出すのは 基本（マイル・対応・個別メモ・名前・タグ・リッチメニュー・友だち情報・フォーム回答）とタイムライン（全件／★のみ／受信／送信／システム通知）。**良い点**：マイルが「2,450mile 利用可能 ・ 確定待ち 300」と**使える分と確定待ちを分けて**出す。空の欄は「メモはありません」「タグはありません」「回答はまだありません」「やり取りはまだありません」と**何が無いかを書く**（空欄にしていない）。「本名 未登録／システム表示名 未登録」も同じ。流入元は「不明」。**P2 差**：設計はタイムラインに 健康記録・リマインダ・アクション・注文/定期便 を混ぜて1本の時系列にするが、実装はタブで切り替える。**推奨修正**：来店回数と前回の申し送りは EC連携（`ec_events`）が要るので `ELayY` と同じ束。取得元：`friends-v6/I6UAdr.txt`。1440・1920とも横スクロール0', verdictHead: 'c275749d',
    verdict: 'needs_fix',
    verdictNote: '**一部は契約待ち。** 設計の「来店回数」「前回の申し送り」は、**EC側の来店記録と申し送りを friend に結ぶ口が無い**ため出せない。タイムラインに健康記録・リマインダ・注文/定期便を混ぜるのも、種別をまたぐ時系列の口が要る。**取得元のない値を設計の数字で作らない。** 良い点は残す：マイルを「利用可能／確定待ち」で分ける、空欄に「メモはありません」など何が無いかを書く',
    verdictSource: 'Claude実装',
    verdictHead: '846be01f',
  },
  {
  ...
  */
  ...
  },
  {
    ...
    steps: [{ qaOpen: 'Ho8z4' }],
    verdict: 'needs_fix',
    verdictNote: '#623で通知の段から開くようにした。**以前は「いつ見られるようにするか」タブを撮っていて、視聴後の話が写っていなかった**。設計（視聴完了／テンプレート／CTAクリック／未視聴の出し分け、実行タイミング、同じ視聴への実行回数、視聴結果を取得できない場合）は実装に無く、通知の6種類で近いことをしている。出し分けと実行条件は契約待ち',
    verdictSource: 'Claude実装',
  ...
  },
  {
    ...
    steps: [{ qaOpen: 'D6yO7e' }],
    verdict: 'needs_fix',
    verdictNote: '#623で公開前の確認（STEP 5）を作った。足りないものを1つずつ言い、タイトル・公開ページ・動画の長さ・配信枠を並べる。**未実装ではなくなった**。残る差：設計の公開前チェック4項目（通知テスト済み・リマインド重複の確認）、最終確認の6行（公開期間・対象・アクション）、右のLINEプレビュー。**「有効化」の押し口は置いていない**——公開は基本設定の公開状態で行う口が既にあり、2つ目を作るとどちらが本当か分からなくなる',
    verdictSource: 'Claude実装',
    verdictHead: '988cc37a',
  },
  {
  ...
  */
  ...
  },
  {
    ...
    */
    ...
    mode: 'viewport', height: 1080,
    steps: [{ click: 'テンプレートを削除', scope: 'main' }], verdictNote: 'P2 設計の「先に差し替えてから削除する」流れが無い。設計は使用先を3つ挙げ、差し替え画面へ誘い、そのまま消すときは名前を打ち直させる。実装は使用中のテンプレートから削除の導線ごと外す作りで、危険は無い代わりに使用中のものを整理する手段が無い。窓にも使用先が出ない **ルート**：`/templates`（削除の窓）。**取得元**：`templates-v6/design-qa.md`。**推奨修正**：**窓に使用先を出すのが先**。いまは使用中のテンプレートから削除の導線ごと外す作りで危険は無いが、**使用中のものを整理する手段も無い**。`uNBlA`（変える前に影響を見る）が同じ「使用先を数えて出す」形を既に持っているので写す。', verdictHead: '62ddaebe',
    verdict: 'needs_fix',
  ...
  */
  ...
  // ここには、設計と並べるために撮るものだけを置く。
  {
    ...
    verdictSource: 'friend-attributes-v6/l25rlp-1920.png', verdictHead: '3b5098a3',
  },
  {
  ...
  },
  {
    ...
    verdictSource: 'friend-attributes-v6/ee0sk-1920.png', verdictHead: '3b5098a3',
  },
  {
