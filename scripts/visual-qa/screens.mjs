...
const FEATURE_19_AUDIT = {
  ZrpKn: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06 Issue #232 / UI HEAD 83be84278を3107/8794で撮り、★V6設計と同じ1920pxで並べて確認。** 30日集計は486件・1,284,000円、直前30日は412件となり、6成果地点の名前・起点・件数・金額も設計の値で表示できた。1440px・1920pxとも横はみ出し0。ただし設計の12件内訳、説明帯、状態絞り込み、期間選択、利用先、詳細、CSVはAPIが無く、実装は6件と未接続の説明を表示しているため一致ではない。**推奨修正：利用先の取得・追加APIを先に接続し、「使う場所を足す」と利用先名を行ごとに出す。次に全件数・状態内訳・詳細・CSVの口を接続する。**',
    verdictSource: 'conversions-v6/ZrpKn.txt + conversions-v6/ZrpKn-1440.png + conversions-v6/ZrpKn-1920.png',
    verdictHead: '83be84278',
  },
  GUxsj: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06 Issue #232 / UI HEAD 83be84278を3107/8794で撮り、★V6設計と同じ1920pxで並べて確認。** 7日・30日・90日の期間選択、現期間486件・1,284,000円、前期間412件・1,092,000円、単価2,642円、成果地点別の増減を実データで表示した。1440px・1920pxとも横はみ出し0。設計の日別積み上げグラフ、最も多い帰属経路、取消・純成果、CSVは集計APIが無く、未接続条件を本文に表示しているため `match` にはしない。**推奨修正：日付・成果地点別の集計APIを先に追加して日別グラフを埋め、次に帰属経路とCSVを接続する。**',
    verdictSource: 'conversions-v6/GUxsj.txt + conversions-v6/GUxsj-1440.png + conversions-v6/GUxsj-1920.png',
    verdictHead: '83be84278',
  },
  GtylA: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06 Issue #232 / UI HEAD 83be84278を3107/8794で撮り、★V6設計と同じ1920pxで並べて確認。** 既存の注文確定を作成画面の「購入」へ読み替え、同種1件・この30日386件・612,400円を実データで表示した。同名保存の防止、過去成果を追加しない説明もある。1440px・1920pxとも横はみ出し0。ただし設計の6起点に対し実装は3分類、同じ人を数える方式は設計3択に対し2択で、除外・取消・利用先・保存前試算もAPIが無い。画面の高さと区切りも設計と大きく異なるため `needs_fix` を維持する。**推奨修正：起点の保存契約と回数条件を拡張し、6起点・30日に1回・除外・取消を先に実装する。その後、利用先と保存前試算APIを接続して設計の4区画へ組み直す。**',
