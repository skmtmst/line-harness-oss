  ...
  */
  ...
  { ...COMMON_VAR, node: 'gBtaK', name: '14-1-A 共通情報を編集', route: '/contents/vars/edit?id=cv-1', verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一。設計画像を撮り直した。**実装との突き合わせはこれから。** **P1 編集画面に「どこで使われているか」が1つも出ない。** ルート `/contents/vars/edit?id=cv-1`。出るのは 共通情報名・フォルダ・差し込み名・種別・値・更新スケジュール だけ。共通情報は**1か所直すと差し込んでいる全部の文が同時に変わる**ので、変える前に使用先が見えないと危ない。**ただし #548 で「保存」を押すと影響確認の面（`uNBlA`）へ進むようになった**ので、「どこが変わるか見えないまま保存する」状態ではなくなった。**良い点**：差し込み名に「あとから変えられません。変えるとテンプレートの差し込みが空になります。」と**理由まで**書く。種別は「※変更できません。」と断る。更新スケジュールに「過去の日時は指定できません。指定した時刻を過ぎると、自動で値が入れ替わります。」と書き、無いときは「スケジュールが設定されていません」と出す。**P2 残る差**：設計は値の下に「保存すると、下の15か所すべてが すぐに変わります」と**常に**出す。実装は保存を押してから出る。**推奨修正**：`uNBlA` と同じ口（使用先の数）を編集画面でも先に読んで、値の下に常に出す。取得元：`common-vars-v6/gBtaK.txt`。1440・1920とも横スクロール0', verdictSource: 'common-vars-v6/gBtaK.txt' , verdictHead: '6a3eb22b' },
  {
    ...
    */
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-04 変更前確認を口へつないだ（board#89）。** #773 で入った `POST /api/common-vars/:id/impact-preview` を web の口につなぎ、**保存後の文・文字数・上限超え**を使用先ごとに出すようにした。前は使用先台帳（`delete-impact`）しか読めず、「変更後の文と文字数の検査」は `—`＋未接続の理由だった。 **4通りを書き分ける**：(1) ふつうに変わる行は「保存後の文」と「文字数 N / 5,000文字」。(2) **送信済みは変わらない**ので保存後の文が同じ。(3) 差し込みの目印を本文から読み取れない行は **`—`＋理由**——空文字で埋めると「保存すると空になる」と読める。(4) 上限を超える行は赤で「この通は送信のときに落ちます」。 **止める理由があるあいだは保存させない。** 5,000文字を超える文を保存すると、その通は送信のときに落ちる——**落ちるのは保存の何日もあとで、原因がこの操作だと結びつかない。** 押せない理由はボタンの脇に言葉で出す（灰色のボタンだけでは何度も押すことになる）。**読めていないときは止めない**——影響を確かめられなかったことを理由に保存を塞ぐと、口が落ちているあいだ誰も値を直せなくなる。 入力のたびには問い合わせない（400ms 待つ）。**打っている途中の文で「空になります」と出ると、消して打ち直しているだけの人を止めてしまう。** 上限が無い使用先に「/ 5,000」と書かない（無い決まりを作ってしまう）。所属を確定できないフォームは**件数だけ**出し、名前と中身は出さない。 撮影用の口 `commonVarChangeImpact` を足したので撮れる。**設計画像との突き合わせはこれから。**',
