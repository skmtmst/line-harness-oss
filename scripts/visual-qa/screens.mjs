...
export const SCREENS = [
  ...
  */
  { ...MILEAGE, node: 's98Vfw', name: '17-1 マイル', verdict: 'match', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** 今月の増減合計とブロンズ886人・シルバー312人・ゴールド86人を固定応答ではなくAPI契約の値として表示した。友だち別の残高・今月増減・失効予定・ランクも維持し、1440・1920pxとも横はみ出し0。', verdictSource: 'mileage-v6/s98Vfw.txt + s98Vfw-1920.png', verdictHead: '1c8055931' },
  ...
  },
  {
    ...
    */
    ...
    verdict: 'match', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** 反映待ち18件、各履歴のLINEアカウント名、変更後残高を表示した。理由・発生元・操作者と合わせて増減の根拠を追え、1440・1920pxとも横はみ出し0。',
    verdictSource: 'mileage-v6/MvZm5.txt + MvZm5-1920.png', verdictHead: '1c8055931',
  ...
  { ...MILEAGE, node: 'BmoGY', name: '17-1-D たまる決めごとをつくる', route: '/mileage/earning-rules/new', verdict: 'needs_fix', verdictNote: '**2026-09-04 撮り直して判定した（S3 第1段）。** 要修正。**実装のほうが進んでいる部分がある**：倍率表（倍率なしの人1.0倍／EC顧客連携済み1.2倍／NEN会員1.5倍）と「倍率はタグ側の設定で決まります。優先度がいちばん高いタグ1枚だけが効きます。」——**これは残し、設計側にも足す**。実装に無いのは「付いてから1年」（有効期限）、**「予約が取り消されたら 300 マイルを引きます。残高が足りなければ 0」**（取り消しの扱い）、「条件を足す（15の軸から選べます）」、**LINEプレビュー**。 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一。設計画像は `docs/design-reference/mileage-v6/` を撮り直した。**実装との突き合わせはこれから。** **development `c275749d` で撮った。** 構造は設計とそろっており、残るのは実データの接続', verdictSource: 'mileage-v6/BmoGY.txt + mileage-v6/design-qa.md' , verdictHead: '31293424' },
  {
    ...
    */
    ...
    verdict: 'match', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** ランク進捗、付与理由別集計、履歴の変更後残高を表示し、ランク名は日本語へ統一した。マイル契約側の表示名を優先して高橋直人を表示し、1440・1920pxとも横はみ出し0。',
    verdictSource: 'mileage-v6/HIU5O.txt + HIU5O-1920.png', verdictHead: '1c8055931',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'match', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計の確認段階と実装を同じ1920pxで並べて再判定。** 2,000マイルは変更前・変更量・変更後を見せる確認段階へ進み、承認境界ちょうどの10,000マイルは別オーナー承認が必要として実行前に停止する変種も両幅で撮影した。自動通知なので手動返信ヘッダーは付けない。全4枚で横はみ出し0。', verdictSource: 'mileage-v6/vz0Ji.txt + vz0Ji-1920.png + vz0Ji-high-value.txt + vz0Ji-high-value-1920.png',
  ...
  */
  ...
  { ...NEN, node: 'VLMGH', name: '21-1 NEN配信',
    ...
    verdict: 'structure_match_data_pending', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** 30日の関連成果142件・612,400円と配信別成果金額を表示した。通常・読込・空・失敗を両幅で撮影し横はみ出し0。残る差はLINEが個人の開封・押下を提供しないため反応率を作れないことだけで、理由を表示して構造一致・データ待ちとする。', verdictSource: 'nen-v6/VLMGH.txt + VLMGH-1440.png + VLMGH-1920.png + VLMGH-loading/empty/error screenshots', verdictHead: '1c8055931' },
  { ...NEN, node: 'DEX0k', name: '21-1-A NENコラム', route: '/nen-campaigns?tab=columns', verdict: 'match', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** 成果金額、記事の読了人数、コラム複製、テスト送信を一覧から確認・実行できるようにした。24本の集計・絞り込み・ページ情報も維持し、1440・1920pxとも横はみ出し0。', verdictSource: 'nen-v6/DEX0k.txt + DEX0k-1440.png + DEX0k-1920.png', verdictHead: '1c8055931' },
  ...
  { ...NEN, node: 'WeXbL', name: '21-1-C NEN配信の履歴', route: '/nen-campaigns?tab=history', verdict: 'structure_match_data_pending', verdictNote: '**2026-09-07 Issue #418 / HEAD 1c8055931 で実APIへ接続し、設計画像と実装画像を同じ1920pxで並べて再判定。** 未達6通をブロック4・退会2・その他0に分け、待機中をまとめて今すぐ送る操作を接続した。1440・1920pxとも横はみ出し0。残る差は個人の反応をLINEから取得できないことだけで、取得不可と明示して構造一致・データ待ちとする。', verdictSource: 'nen-v6/WeXbL.txt + WeXbL-1440.png + WeXbL-1920.png', verdictHead: '1c8055931' },
  {
