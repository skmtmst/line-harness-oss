...
export const SCREENS = [
  // ── 機能1 ダッシュボード ────────────────────────────────
  {
    ...
    verdictNote: '**2026-09-03 第2ラウンド第1段で撮り直して判定した。** ルート `/`。1440・1920とも横スクロール0。 **構造は設計どおり**——今日やること4枚・出荷予定・受信一覧・推移表と、右の 今月の送信枠／運用アラート／接続状態／対応状況／友だちの状態。文言も一致した（「残り 197 / 上限 200通」「残り 98.5%」まで同じ）。 **P2 要修正は1点**：接続状態の LINE Webhook が設計は「3 / 3 正常」で**何本中何本かを出す**が、実装は「正常」だけ。何本落ちているかが読めない。 **設計側が古い点が1つ**：設計は「現在の対応マーク」、実装は「現在の対応状況」。§7 #1 が「対応マーク→対応状況」と決めているので**実装が正しく Pencil が未反映**（board#101）。 残りの差（運用アラート2件↔0件、二段階認証 0/6人↔0/0人、未対応360人↔5人）は撮影用の固定データの違いで実装の問題ではない。',
    verdictSource: 'dashboard-v6/vUXKb.txt',
  ...
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/`（「ダッシュボード編集」）。1440・1920とも横スクロール0。 カードの並び（今日やること／メイン／右サイド）と札の構成は設計どおり。 **P2 文言が2か所ちがう**：枠数の注記が設計「「今日やること」は4枠までです」に対し実装「最大4枚まで・ドラッグで順番変更」。流入経路TOP3の副題が設計「右サイド｜直近7日の上位経路」に対し実装「右サイド｜友だち追加経路」。 **実装のほうが厚い点**：設計は右サイドの札に副題がほとんど無いが、実装は「右サイド｜使用数と残り」のように全札に付けている。**消さずに設計へ足す向き**で揃えたい（Pencil 側）。 内部語1件：「右サイド｜Webhook・自動処理」（`components/dashboard/dashboard-editor.tsx:78`）。',
    verdictSource: 'dashboard-v6/ZN0ov.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: 'QRを表示' }],
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/`（「QRを表示」）。1440・1920とも横スクロール0。 追加URL・コピー・ヒント3行は設計と一致。 **P2 要修正は2点**：(1) 設計は「ダウンロード形式 PNG / JPG / SVG」を選ばせるが、実装は「PNGをダウンロード」の1つだけで**形式が選べない**。(2) 設計の「画像の大きさ」は選択口で「大（1200px）」が選ばれた形だが、実装は大・中・小の3つを並べて出す。 「PDFで印刷」は実装にもある。',
    verdictSource: 'dashboard-v6/JN6mQ.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: '表示件数' }],
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/`（「表示件数」）。1440・1920とも横スクロール0。 表示件数の選び口（5/10/15/20件表示）と開いたときの位置は設計と一致。 **設計側が古い**：設計は「対応が必要な受信」カードの**値**が「5件表示」になっている（プルダウンの文言が値に混ざった §7 #23）。実装は「5件」で**正しい**。#23 は §7 で未反映のままなので、Pencil を直すまで設計画像がこの誤りを持つ（board#101）。 **一致と数えないのは設計側の未反映が残るため**で、実装に直すところは見つからなかった。',
    verdictSource: 'dashboard-v6/NjK9q.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: '通知' }],
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/`（「通知」）。1440・1920とも横スクロール0。 パネルの骨（見出し「通知」／「すべて既読にする」／すべて・エラー・アップデートの3タブ）は設計どおり。 **中身がデータ未接続**：3タブの件数がどれも `—` で、本文は「通知を読み込めませんでした。もう一度お試しください。」。設計は「LINE Webhook の応答遅延を検知しました」など3件が並ぶ。 **口はある**（`apps/worker` の `/api/notifications/center`）が、**撮影用のモックがこの口を持っていない**ため中身が撮れない。実装の不具合ではない。モックは撮影ハーネス側（S0）なので Issue で渡す。 数が取れないときに0と書かず `—` にしているのは規則どおり。',
    verdictSource: 'dashboard-v6/Alekb.txt',
    verdictHead: '49e1341c',
  },
  ...
  // ── 機能2 受信箱 ────────────────────────────────────────
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 第2ラウンド第1段で撮り直して判定。** ルート `/chats`。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 一覧・トーク・顧客情報の3カラムと、見出しの担当／対応の選び口は設計どおり。**文字の差は固定データだけ**だった。 P2 送信欄の設定ボタンが設計「送信設定」、実装「⚙ 送信の設定」。',
    verdictSource: 'inbox-v6/xGLVe.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（トーク→「テンプレートを選択」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 フォルダの選び口（件数つき）・分類の札・名前と本文の検索の3通りの絞り込み、差し込みプレビューと「入力欄へ挿入」まで設計どおり。**送信ではなく挿入だと押す前に分かる。** 文字の差はひな形名の固定データだけ。',
    verdictSource: 'inbox-v6/NfgOs.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（顧客情報パネルを閉じた形）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 パネルを閉じた形は設計どおりで、文字の差は固定データだけだった。 **前の判定にあった「閉じたときのトーク欄の広がり方が設計と違う」は、文字では確かめられない。**寸法の突き合わせは第2段で絵を重ねて見る。',
    verdictSource: 'inbox-v6/H3lAOB.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（「表示項目」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 項目ごとの出し入れ・「上へ／下へ」の並べ替え・「初期状態に戻す」が在り、選んだ形は端末に残る。 P2 設計はスイッチと掴んで動かす形、実装はチェックと「上へ／下へ」。出し入れの中身は同じで、残るのは操作の見た目の差だけ。設計にある「完了」は無い（押さなくてもその場で効くため要らない）。',
    verdictSource: 'inbox-v6/Xi4x9.txt',
    verdictHead: '49e1341c',
  },
  /*
  ...
  */
  { ...INBOX, node: 'f0zn6', name: '2-5 新着・担当者別未読', steps: [...OPEN_CHAT],
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 帯6つ（要返信／最長待ち／自分が担当／今日の受信／メール／期限超過）は設計と一致。 **設計も実装も1本の一覧**で、担当は行の末尾に「担当：◯◯」として出る。**担当者ごとに束ねる形は設計にも無い**（前の判定の「設計は担当ごとの束」は誤りだったので取り消す）。 **P1 足りないのは担当者ごとの未読数**——画面名が「新着・担当者別未読」なのに、担当者の絞り込みに未読数が付かない。台帳 kentavndng/line-harness-board#33 で直す。 **設計側が古い点**：設計は「担当：未割当」、実装は「担当：未割り当て」。§7 #48 は「未割り当て」に統一と決めているので**実装が正しく Pencil が未反映**（board#101）。',
    verdictSource: 'inbox-v6/f0zn6.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 フォルダの選び口を開くと すべてのフォルダ／お問い合わせ／予約／EC／未分類 が件数つきで並ぶ。**取得できた0件も0のまま出る。** フォルダ名での検索も在る。文字の差はひな形名の固定データだけ。',
    verdictSource: 'inbox-v6/NWbuF.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（トークを開いて「内部メモ」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 「内部メモ」「内部メモを追加」「スタッフのみ」は設計どおり。実装はさらに「対応方針や引き継ぎ内容を入力してください。顧客には表示・送信されません。」を添えている。 **前の判定「メモに書いた人と時刻が残らない」は取り消す**——設計にも書き手と時刻は無く、設計と実装の差ではない。足すなら Pencil が先。',
    verdictSource: 'inbox-v6/B7CER8.txt',
    verdictHead: '49e1341c',
  },
  /*
  ...
  */
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（「担当者で絞り込む」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 通常の形は撮れて、選び口の並びは設計どおり。 **P1 選択中の状態（`-selected`）が撮れなかった。**「Kenta 3」のように**担当者ごとの未読数**を出す面だが、`GET /api/chats/stats` の返事に担当者ごとの数が無い。 **前の判定は「`InboxStats.assigneeUnread` を読むようにした」と書いていたが、その実装は codex/development に入っていない**（`assigneeUnread` はいま木のどこにも無く、契約枝のローカルcommitのままだった）。判定を取り消して未実装に戻す。board#33 で実装する。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（トーク見出しの「担当：」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 担当の変更はその場で選べる（設計の「担当者を変える」ボタンは無く、選び口へ寄せてある）。 P2 設計は名前と顔を並べた専用の選び口。実装は名前だけの選び口で、顔が出ない。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（対応の選び口）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 未対応・対応中・保留・対応済みの並び、色の丸、色付きの札、選択中の✓まで設計どおりで、**文字の差は固定データだけ**だった。',
    verdictSource: 'inbox-v6/IYjvu.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り方を直して撮れた。** ルート `/chats`（テンプレートのフォルダで「予約」を選ぶ）。1440・1920とも横スクロール0。 予約フォルダを選ぶと、そのフォルダのひな形だけに絞られる。**文字の差はひな形名の固定データだけ**だった。 **撮れなかった原因は撮り方**——`template-folder-select.tsx` がフォルダ名の前を文字から絵の印（`aria-hidden`）へ変えたので、行の読み上げ名が「フォルダ 予約」ではなく「予約 5」になっていた。この台帳の `steps` を直した。',
    verdictSource: 'inbox-v6/TUveA.txt',
    verdictHead: '49e1341c',
  },
  { ...INBOX, node: 'w72a2', name: '2-12 絞り込みを開く', steps: [{ click: '絞り込み' }],
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（「絞り込み」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 設計の6項目（対応マーク・担当者・受信経路・期限・表示するメッセージ種別・未読だけ表示）と「リセット」「この条件で絞り込む」がすべて在る。 **押せない2項目に理由が本文で出ている**（「期限はまだ記録していないため、絞り込めません」「種別で絞る読み口がまだ無いため、選んでも一覧は変わりません」）。**これが束3の手本。**',
    verdictSource: 'inbox-v6/w72a2.txt',
    verdictHead: '49e1341c',
  },
  { ...INBOX, node: 'ASsb3', name: '2-13 保存した検索を開く', steps: [{ click: '保存した検索' }],
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。前の判定は取り消す。** ルート `/chats`（「保存した検索」）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 **P1 設計にある「条件の要約」と「該当件数」が出ていない。** 設計は1件ごとに 名前／条件の要約（例「受信経路：LINE／未読のみ」）／件数 の3段だが、実装は 名前＋「削除」だけ。**何を絞る条件なのかが名前からしか分からない。** **前の判定は「条件の要約を出した」「削除を『…』へ畳んだ」と書いていたが、その実装は codex/development に入っていない**（`normalizeSavedViewConditions` はいま木のどこにも無く、#604 のローカルcommitのままだった）。 **開いた状態（`-menu`）は撮れなかった**——実装に `data-qa-open` の印（ASsb3） の押しどころが無い。第2段で足す。 board#43（古い保存条件で受信箱が落ちる）も同じ面。',
  ...
  */
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（保存した検索に名前を入力）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 名前の入力欄と保存の導線は設計どおり。文字の差は保存済みの名前（固定データ）と、`ASsb3` と同じ**条件の要約が出ていない**点。要約は `ASsb3` でまとめて直す。',
    verdictSource: 'inbox-v6/ANgda.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（保存完了）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 保存できたことの知らせは設計どおり。差は保存済みの名前（固定データ）と、`ASsb3` と同じ条件の要約の欠落。',
    verdictSource: 'inbox-v6/tBlkL.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（名前が空のまま保存）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 未入力のときの断りは出る。 **board#50「保存名未入力を最初から保存不可にする」と board#57「未入力エラーを明確にする」がこの面**。第2段で直す。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/chats`（同じ名前で保存）。1440・1920とも横スクロール0。設計との差のうち 人名・ひな形名・件数・「NENマイル」は撮影用の固定データの違いで、実装の問題ではない。**「Enter で改行」は設計にもある**（設計は「Enterで改行」で空白の有無だけの差）。 重複のときの断りは出る。 P2 §7 #39 は「重複エラー中の保存ボタンが通常表示」を挙げている。押せない状態の見た目は第2段で部品に寄せる。',
    verdictSource: 'inbox-v6/LHjwD.txt',
    verdictHead: '49e1341c',
  },
  ...
  { ...FRIENDS, node: 'PhxG6', name: '3-1 友だち',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 第2ラウンド第1段で撮り直して判定。** ルート `/friends`。1440・1920とも横スクロール0。 一覧の列（友だち／対応・担当／シナリオ／最新メッセージ／タグ・属性／最終接触）と帯は設計どおり。 **P1 内部語がそのまま出る**：最新メッセージの欄に `[sticker]`。§7 #47 は「スタンプ」と決めている。 **P2 検索欄の説明が無い**：設計は「名前・LINE名・タグ・メモで検索」と何で探せるかを書くが、実装は「検索」だけ。 P2 表記の空白差：設計「相手から 12・自分から 5」に対し実装は中黒の前後に空白（「相手から 12 ・ 自分から 5」）。',
    verdictSource: 'friends-v6/PhxG6.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends`（表示件数）。1440・1920とも横スクロール0。 表示件数の選び口は設計どおり。 差は `PhxG6` と同じ（`[sticker]`／検索欄の説明／中黒の空白）に加え、担当の書き方が設計「担当 Masato」に対し実装「担当：Masato」。**どちらかに決めて全画面でそろえる**。',
    verdictSource: 'friends-v6/LT8RS.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: '詳細条件' }],
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends`（詳細検索）。1440・1920とも横スクロール0。 14軸の絞り込みは設計どおり並ぶ。 **できないことを本文で断っているのが良い**：設計は「LINE登録名・本名・システム表示名」と3つ探せるように書くが、実装は「LINE登録名から探します。本名とシステム表示名は、まだ検索の対象にできません。」と**理由を本文に出している**（共通ルール §5-5 の形）。設計を実装に合わせるか、口を足すかは第2段で決める。 残る差はタグ名の固定データ。',
    verdictSource: 'friends-v6/Igi72.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    */
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 撮れなかった。判定できない。** ルート `/friends`（一括操作）。通常・取得失敗・権限不足・選択・結果の5状態すべて。 **理由は撮影用の固定データに権限が無いこと。** `friends/page.tsx:393` は `canRunBulk(selectedAccount?.role)`（`owner` か `admin`）のときだけ「操作を選ぶ」を出すが、モックの `/api/line-accounts` が返すアカウントに `role` が無い。そのため押しどころ（`data-qa-open` の印）が描かれない。 **実装の不具合ではない。** モックは撮影ハーネス側（S0）なので Issue で渡す。',
    verdictSource: 'friends-v6/（撮れず）',
    verdictHead: '49e1341c',
  },
  { ...FRIENDS, node: 'I6UAdr', name: '3-1-D 友だち詳細', route: '/friends/detail?id=friend-0',
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends/detail?id=friend-0`。1440・1920とも横スクロール0。 基本／マイル／対応の段組みと「タブを編集（友だち情報欄）」「個別トークを開く」は設計どおり。 **P2 動かないボタンが2つ置いてある**：「非表示」と「ブロック」。どちらも押せず、理由は吹き出しにしか無い（「一覧から隠す操作は準備中です」「ブロックはLINE側の操作です。管理画面からは変えられません」）。**理由は本文に出す**のが共通ルール §5-5。第2段で `BlockedAction` に寄せる。 設計にある「現在は1アカウントのみ」が実装に無い。残りの差は人名・タグの固定データ。',
    verdictSource: 'friends-v6/I6UAdr.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    */
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends`。空・読込中・取得失敗の3状態を1440・1920で撮れた（6枚、はみ出し0）。 §7 #38 が求める「失敗状態に再読み込みがある」「読込中・空でKPIとページ送りを消す」を、この画面で確かめる。**部品側（`ListState`）は S0 の担当**なので、直しが要るときは渡す。 通常の絵との差は固定データ。',
    verdictSource: 'friends-v6/bzDn6-empty.txt',
    verdictHead: '49e1341c',
  },
  { ...FRIENDS, node: 'YzxU1', name: '3-2 重複検出', route: '/friends?tab=duplicates',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends?tab=duplicates`。1440・1920とも横スクロール0。 帯4つ（友だち総数／ユニーク人数／重複している行／重複による配信コスト）と、アカウント別・アカウント間の2つの表は設計と同じ考え方。**配信コストは `—` ＋「まだ繋がっていません。配信実績が接続されると表示されます。」で、0で埋めていない**のは規則どおり。 **P1 内部語が2つ見出しに出る**：「アカウント別ブレイクダウン」「アカウント間 重複マトリックス」（`app/duplicates/page.tsx:161,198`）。§7 #48 の範囲。 **P2 設計にある「CSVで書き出す」が無い**——実装の `duplicates/page.tsx` に書き出しの口は1つも無い（`CSV` の語が0件）。',
    verdictSource: 'friends-v6/YzxU1.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends/identity-candidates`。1440・1920とも横スクロール0。 **実装は「判定根拠」を丁寧に出す**（確からしさ／決め手になる／参考／`—（未取得）`／「表示名やプロフィール画像だけの一致は、決め手にはしません。」）。取れない項目を0で埋めていない。 **P1 設計と作りが違う**：設計 3-2-A は候補A・候補Bを左右に並べ、項目ごとに「候補Aを採用」「候補Bを採用」を選ばせる。実装にその選択が無く、**どちらの値を残すかを決められない**。 **どちらを正本にするかは人の判断**——実装の「根拠を見せて結び付ける」ほうが安全側だが、設計は値の選択を求めている。第2段の前に Pencil と突き合わせる。',
    verdictSource: 'friends-v6/InCDe.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends?tab=merged`。1440・1920とも横スクロール0。 帯4つ（統合ユーザー／紐付く友だち／重複している行／重複率）と、統合ユーザー・連絡先・紐付くアカウント・UID・最終接触・重複配信・操作 の表は設計どおり。 **P2 設計にある3つが実装に無い**：「名前・メール・電話・UIDで検索」の検索欄、「UID：すべて」の絞り込み、「CSVで書き出す」。**「CSVで書き出す」は友だち一覧タブにしか出ない**（`friends/page.tsx:603` が `tab === \'list\'` で囲っている）。',
    verdictSource: 'friends-v6/r7eSi.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/friends?tab=merged`（行を開く）。1440・1920とも横スクロール0。 通常の形は撮れた。 **編集（`-edit`）と衝突（`-conflict`）の2状態は撮れなかった**——押しどころ（`data-qa-open` の印、`components/users/user-row.tsx:162`）が固定データの行に描かれない。 **P2 設計にある「根拠 連携UID・電話番号」「表示名 候補Aから採用」「MAIL 候補Bから採用」が実装に無い**——どの値をどちらから採ったかが読めない。`InCDe` と同じ話なので一緒に決める。',
    verdictSource: 'friends-v6/w8W4Eh.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 状況だけ確かめた。** 3-4 UID移行は `status: \'unimplemented\'`（`gap: \'api\'`）のままで、撮らない・合格にもしない。 **設計の文字が書き出されていない**——`docs/design-reference/friends-v6/` で `vtBCu.png` だけ `.txt` が無く、236枚の書き出しから漏れている。文字の突き合わせができないので、書き出し直しを lane:pen へ渡す。',
    verdictSource: 'friends-v6/vtBCu.png',
    verdictHead: '49e1341c',
  },
  ...
  { ...SCENARIO, node: 'TC1b1', name: '5-1 シナリオ配信', route: '/scenarios',
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 第2ラウンド第1段で撮り直して判定。** ルート `/scenarios`。1440・1920とも横スクロール0。 一覧の列と絞り込みは設計どおりで、**文字の差は固定データだけ**だった。 P2 「マニュアル」ボタンが押せず、理由が吹き出しにしか無い（`app/scenarios/page.tsx:233`）。第2段で本文に出すか、描かない。',
    verdictSource: 'scenarios-v6/TC1b1.txt',
    verdictHead: '49e1341c',
  },
  { ...SCENARIO, node: 'cCB7r', name: '5-1-A シナリオ作成・配信方式', route: '/scenarios/mode?id=scenario-0',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/mode?id=scenario-0`。1440・1920とも横スクロール0。 配信方式の選択そのものは設計どおり。 **P1 設計にある「STEP 1／STEP 2／STEP 3」の工程表示が実装に無い。** いま何段目で、あと何が残るかが分からない。§7 #49 も同じ指摘（ウィザードの段とボタン名のずれ）。 P2 作成できたときの知らせが、設計「「新規登録7日間フォロー」の下書きを作成しました。」に対し実装「シナリオ「新規登録7日間フォロー」を作成しました。…フォルダ：未分類」。',
    verdictSource: 'scenarios-v6/cCB7r.txt',
    verdictHead: '49e1341c',
  },
  { ...SCENARIO, node: 'kk8dz', name: '5-1-B シナリオ作成・1通目設定', route: '/scenarios/first-step?id=scenario-0',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/first-step?id=scenario-0`。1440・1920とも横スクロール0。 **P1 `cCB7r` と同じく「STEP 1／2／3」が無い。** **P1 「LINEプレビュー」が無い**——設計は書いた本文がLINEでどう見えるかを右に出すが、実装に無い。送る前に見た目を確かめられない。 P2 設計は上に「配信方式：時刻で指定　・　シナリオ：新規登録7日間フォロー」といまの前提を出すが、実装に無い。',
    verdictSource: 'scenarios-v6/kk8dz.txt',
    verdictHead: '49e1341c',
  },
  { ...SCENARIO, node: 'bV5Vs', name: '5-1-C シナリオ編集', route: EDIT,
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`。1440・1920とも横スクロール0。 ステップの並び・追従バー・「配信結果を見る」は設計どおり。実装が足している「同時に購読できるシナリオは 1つ」は設計に無いが**先に知りたい情報**なので残す向きで Pencil へ。 P2 「マニュアル」が押せず理由が吹き出しだけ（`scenario-detail-client.tsx:1348`）。',
    verdictSource: 'scenarios-v6/bV5Vs.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（ステップ編集）。1440・1920とも横スクロール0。 **P1 「LINEプレビュー」が無い**（`kk8dz` と同じ）。実装は「LINEへ届く中身です。」と書くだけで、絵にならない。 **P1 内部語**：メッセージ種別に「Flex（カードタイプ）」（`scenario-detail-client.tsx:76`）、失敗時に「Flex メッセージの JSON が不正です」「画像 JSON パースエラー」（`:203,694,695`）。§7 #48 の範囲。 実装が足している「ⓘ 配信時刻は5分単位で処理するため、指定時刻から最大5分遅れる場合があります」は良いので残す。',
    verdictSource: 'scenarios-v6/xfYLn.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（配信条件）。1440・1920とも横スクロール0。 AND・OR で条件を組む形は設計と同じ考え方。 **P2 設計にある軸の分類が実装に無い**：「標準互換（15軸）」「この画面だけの軸（6軸）」。**どの条件がほかの画面でも使えるかが読めない。** P2 言い回しの差：設計「複数条件は「すべて一致（AND）」または「いずれか一致（OR）」で結合できます。」に対し実装「「すべて満たす」必要がある条件(and条件)」。**§7 #48 が AND/OR を内部語に挙げている**ので、そろえ先は設計の反映を待つ（board#101）。',
    verdictSource: 'scenarios-v6/r6Gzsu.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（送信後アクション）。1440・1920とも横スクロール0。 **P1 設計との差が大きい。** 設計は「外部サービスの8動作をすべて扱い、条件分岐と実行順をこの画面だけで組み立てます」と書き、各動作に「条件ON／条件OFF」を置いて15軸の条件ビルダーを開けるが、**実装に条件分岐が無い**。動作を並べるだけで、分岐できない。 「外部サービス」は §7 #48 が内部語に挙げているので、言い換えは設計の反映を待つ。',
    verdictSource: 'scenarios-v6/hz9ti.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（ステップ削除確認）。1440・1920とも横スクロール0。 確認の窓は設計どおり。文字の差は固定データと、`xfYLn` と同じ「LINEプレビュー」の欠落だけ。',
    verdictSource: 'scenarios-v6/dqFft.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（開始条件）。1440・1920とも横スクロール0。 開始のきっかけを選ぶ形は設計どおり。 **P2 設計にある開始条件の例（「流入経路「LINE公式」かつ タグ「初回案内未実施」」）と「API・Webhook」の選択肢が実装に無い。** 「API・Webhook」は §7 #48 の内部語なので、言い換えてから足す。',
    verdictSource: 'scenarios-v6/EvVO5.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios`（配信開始の確認）。1440・1920とも横スクロール0。 開始前の確認は設計どおり。 **設計側に内部語が残っている**：「・開始・停止・編集は監査履歴とSlackのPRスレッドへ記録します」。§7 #48 が「SlackのPRスレッド」を内部語に挙げており、**実装はこの文を持っていないので実装が正しい**。Pencil の反映待ち（board#101）。',
    verdictSource: 'scenarios-v6/RUxNf.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0&started=1`。1440・1920とも横スクロール0。 開始できたことの知らせは設計どおりで、文字の差は固定データだけだった。',
    verdictSource: 'scenarios-v6/NrBkW.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: '一括テスト送信' }],
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/detail?id=scenario-0`（テスト送信）。1440・1920とも横スクロール0。 **実装の断りは設計より強くて良い**：「本物のLINEメッセージが届きます。相手を間違えないでください。下書きの通もテストでは送ります。」 **P1 設計にある「何を・どれだけ送るか」が実装に無い**：「ステップ1から」「全7ステップ」「待機時間はテスト用に10秒へ短縮」「約2分」。**何通届くか分からないまま押すことになる。** **P2 設計の「実際の配信を開始せず、自分のLINEで確認します。」に当たる一文と、確認用プレビューが無い。**',
    verdictSource: 'scenarios-v6/g2UNV.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios/results?id=scenario-1`。1440・1920とも横スクロール0。 参加中・完了・一時停止の数と、失敗数を `—`＋理由（「配信失敗数は、この集計からは取得できません。」）にしているのは規則どおり。 **P2 設計にある見出しと期間の断りが無い**：「シナリオ結果：新規登録7日間フォロー」「直近30日の開始・完了・離脱を確認します。」「直近30日の配信結果です。」**いつからいつまでの数なのかが画面から読めない。** P2 設計の確認用プレビューが無い。',
    verdictSource: 'scenarios-v6/M2b2B.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/scenarios`。空・読込中・取得失敗の3状態を1440・1920で撮れた（6枚、はみ出し0）。 §7 #38 の「失敗に再読み込み」「読込中・空でKPIとページ送りを消す」をこの画面で確かめる。部品（`ListState`）は S0 の担当。 通常の絵との差は固定データ。',
    verdictSource: 'scenarios-v6/q5G45-empty.txt',
    verdictHead: '49e1341c',
  },

  // ── 機能6 一斉配信 ──────────────────────────────────────
  { ...BROADCAST, node: 'q76C35', name: '6-1 一斉配信', route: '/broadcasts',
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#32 結果画面から作成中の操作を外し「設定サマリー」→「配信した設定」（`u6gHt`）／#35 絞り込みチップ「予約中のみ」が選択中なのに条件外の行が並んでいたので未選択に（`q76C35`）／#39 「配信を予約」が押せる見た目だったので無効に（`vW4Es`）、「テスト送信」も同様（`zZ9fA`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを一覧へ・内容を見るに（`bPF0s`）／#43 メッセージ種別タブを9種・同じ並びに（`XQfMD`。不足1種を追加）／#44 CSV書き出しを副次に統一（`u6gHt`）／#46 行に「その他操作（…）」（5行）／**5段タブの幅が機能6の11枚だけ1300固定で `sqFXf` がはみ出していた**ので `fill_container` に統一。設計画像は `docs/design-reference/broadcasts-v6/` を撮り直した。**実装との突き合わせはこれから。** **#602 `d02be6d8` で帯が設計どおりになった。** ルート `/broadcasts`。1440・1920とも横スクロール0。 **帯の4枚を設計の順にした**（予約中 4件・下書き `—`・今月の配信 12件／1,842人へ到達・平均開封率 69.4%）。前は「今月の配信・到達・平均開封率・失敗」で、V2 の 4-2 を写したままだった。 **下書きの件数は作らず `—（未取得）`。** `/api/broadcasts/stats` は返さず、一覧は全件返るので数えられそうに見えるが、**一覧はLINEアカウントで絞れるのに集計は絞らない**（`getBroadcastStats` はテナント全体を数える）。基準の違う数を同じ帯に並べると、足しても合わない4枚になる。「今日の予約」も同じ理由。 **未取得の半角ハイフンを全角の `—` にそろえた**（表の配信日時と開封（率））。 P2 **列の並びが設計と違う**——設計は タイトル・内容／状態／配信条件／配信日時／配信・開封・クリック／操作 の6列、実装は タイトル／配信日時／配信条件／内容／配信数／開封（率）／状態／削除 の8列。表の作り直しになるので別PRにする。 推奨修正：列を設計の6列へまとめる。あわせて `/api/broadcasts/stats` に `draft` と `scheduledToday` を足せば、帯が4枚とも実値になる（Codex側・P2）',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#32 結果画面から作成中の操作を外し「設定サマリー」→「配信した設定」（`u6gHt`）／#35 絞り込みチップ「予約中のみ」が選択中なのに条件外の行が並んでいたので未選択に（`q76C35`）／#39 「配信を予約」が押せる見た目だったので無効に（`vW4Es`）、「テスト送信」も同様（`zZ9fA`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを一覧へ・内容を見るに（`bPF0s`）／#43 メッセージ種別タブを9種・同じ並びに（`XQfMD`。不足1種を追加）／#44 CSV書き出しを副次に統一（`u6gHt`）／#46 行に「その他操作（…）」（5行）／**5段タブの幅が機能6の11枚だけ1300固定で `sqFXf` がはみ出していた**ので `fill_container` に統一。設計画像は `docs/design-reference/broadcasts-v6/` を撮り直した。**実装との突き合わせはこれから。** **#603 `c86d7242` で解けた。** ルート `/broadcasts/new`（テンプレート選択）。1440・1920とも横スクロール0。 **カードの補足に内部の語 `text` が出ていたのを直した。** 知らない種類のとき `?? template.messageType` で元の値へ落としていたので、`text` や `carousel` がそのまま出ていた。`typeLabel()` を通し、知らない種類は「その他」にする。撮った絵では札が「テキスト」になっている。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#32 結果画面から作成中の操作を外し「設定サマリー」→「配信した設定」（`u6gHt`）／#35 絞り込みチップ「予約中のみ」が選択中なのに条件外の行が並んでいたので未選択に（`q76C35`）／#39 「配信を予約」が押せる見た目だったので無効に（`vW4Es`）、「テスト送信」も同様（`zZ9fA`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを一覧へ・内容を見るに（`bPF0s`）／#43 メッセージ種別タブを9種・同じ並びに（`XQfMD`。不足1種を追加）／#44 CSV書き出しを副次に統一（`u6gHt`）／#46 行に「その他操作（…）」（5行）／**5段タブの幅が機能6の11枚だけ1300固定で `sqFXf` がはみ出していた**ので `fill_container` に統一。設計画像は `docs/design-reference/broadcasts-v6/` を撮り直した。**実装との突き合わせはこれから。** 一致', verdictSource: 'broadcasts-v6/design-qa-final-confirm-497.md', verdictHead: '7b509106',
  ...
  },
  {
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#32 結果画面から作成中の操作を外し「設定サマリー」→「配信した設定」（`u6gHt`）／#35 絞り込みチップ「予約中のみ」が選択中なのに条件外の行が並んでいたので未選択に（`q76C35`）／#39 「配信を予約」が押せる見た目だったので無効に（`vW4Es`）、「テスト送信」も同様（`zZ9fA`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを一覧へ・内容を見るに（`bPF0s`）／#43 メッセージ種別タブを9種・同じ並びに（`XQfMD`。不足1種を追加）／#44 CSV書き出しを副次に統一（`u6gHt`）／#46 行に「その他操作（…）」（5行）／**5段タブの幅が機能6の11枚だけ1300固定で `sqFXf` がはみ出していた**ので `fill_container` に統一。設計画像は `docs/design-reference/broadcasts-v6/` を撮り直した。**実装との突き合わせはこれから。** **#602 `d02be6d8` で撮り直し、差は無い。** ルート `/broadcasts`（削除の窓）。1440・1920とも横スクロール0。 どの配信かを名前で言う（「「8月キャンペーンのお知らせ」を削除しますか？」）、本文は消えるもの・止まるもの・戻せないことの3つが揃う、ボタンは「キャンセル」と赤い「削除する」。削除の口は `if (!result.success) throw` で成否を見る。 **前に挙げた唯一のP2（日付欄が `mm/dd/yyyy`）は撮影側のブラウザの癖で、実装の不具合ではない。** 実装側に差が無いので一致とする。',
  ...
  */
  ...
  },
  {
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#35 「有効のみ」が選択中なのに下書き・停止中が並んでいたので未選択に（`M1EXwB`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを「一覧へ／内容を見る」に（`PSmHo`）／#44 CSV書き出しの置き場／#46 一覧の行に「その他操作（…）」（5行）／#49 「予定を確認」→「配信予定へ」（次工程名にそろえた）／#48 表記統一。設計画像は `docs/design-reference/reminders-v6/` を撮り直した。**実装との突き合わせはこれから。** **#551 `44692a37` で配信予定プレビューが入り、#613 で撮り直して差が無いことを確かめた。** ルート `/reminders/edit?id=reminder-3&stage=preview`。1440・1920とも横スクロール0。 `POST /api/reminders/:id/preview` の返事をそのまま描く。**ブラウザで時刻を再計算していない**——これが長く未実装だった理由で、画面とWorkerで別々に計算すると「画面では届くのに送られない」が起きる。この作りは崩さない。 取得元：`reminders-v6/JCz6J.txt`',
  ...
  },
  {
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#35 「有効のみ」が選択中なのに下書き・停止中が並んでいたので未選択に（`M1EXwB`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを「一覧へ／内容を見る」に（`PSmHo`）／#44 CSV書き出しの置き場／#46 一覧の行に「その他操作（…）」（5行）／#49 「予定を確認」→「配信予定へ」（次工程名にそろえた）／#48 表記統一。設計画像は `docs/design-reference/reminders-v6/` を撮り直した。**実装との突き合わせはこれから。** **#551 `44692a37` でテスト送信の段が入り、#613 で撮り直して差が無いことを確かめた。** ルート `/reminders/edit?id=reminder-3&stage=test`。1440・1920とも横スクロール0。 画面が「本番の登録 増えません／配信予定 作りません」と、実登録も予定も動かさないことを書いている。口も `POST /api/reminders/:id/test-send` の専用口。**一斉配信やシナリオの口を借りると、リマインダの基準日・版・予定時刻を確かめられない。**この専用口は残す。 取得元：`reminders-v6/W98zZQ.txt`',
  ...
  },
  {
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#35 「有効のみ」が選択中なのに下書き・停止中が並んでいたので未選択に（`M1EXwB`）／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを「一覧へ／内容を見る」に（`PSmHo`）／#44 CSV書き出しの置き場／#46 一覧の行に「その他操作（…）」（5行）／#49 「予定を確認」→「配信予定へ」（次工程名にそろえた）／#48 表記統一。設計画像は `docs/design-reference/reminders-v6/` を撮り直した。**実装との突き合わせはこれから。** **撮り直し待ち。判定・head とも動かしていない。** `PSmHo-1440.png` は **8/31 08:26** のままで、今夜の11本が入る前の絵。今日の撮り直し（固定base `7b509106`）はこの Node にまだ届いていない。**中身を見ずに判定を動かさない。** 撮れ次第、設計（STEP 5「有効化完了」）と見比べる。押し口は「この内容で公開」を押したあとの絵なので、**モックが生きていることを確かめてから撮る**。',
  ...
  */
  { ...AUTO_REPLY, node: 'cmDfJ', name: '8-1 自動応答', verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#34 フォルダの合計が16で「すべて14」と食い違っていたので14に合わせた（`cmDfJ`）／#35 「有効のみ」が選択中なのに条件外の行が並んでいたので未選択に／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを「一覧へ／内容を見る」に（`e6iJG`）／#46 一覧の行に「その他操作（…）」（5行）／#49 「自動応答ルールを保存」→「反応条件へ」（次工程名にそろえた）。`g46ja` は**進む押し口が1つも無かった**ので「最終確認へ」を新設し、テスト実行を副次にした。設計画像は `docs/design-reference/auto-replies-v6/` を撮り直した。**実装との突き合わせはこれから。** **#566 `d0680774` で束3の完了条件を満たした。** `silent rule` `automation rule` `line_account_id` `inline` `TEMPLATE` `keyword` `match` `silent` `flex` `image` を数えて**すべて0件**。**#540（実行結果側）では直らない**——#540 と #566 は同じ `auto-reply-runs` の上の兄弟で、一覧の言葉を直しているのは #566。実際に #540 `857cebc5` で撮ると7語とも残っていた。1440・1920とも横スクロール0。**画面全体は要修正のまま**：P2 設計の一覧の作り（ルールごとの当たり方の推移）はこの直しの外 **ルート**：`/auto-replies`。**取得元**：`auto-replies-v6/cmDfJ.txt`（#540 `857cebc5` でも撮って比べた）。**推奨修正**：取り込むのは **#566**。#540 では一覧の言葉は直らない。', verdictSource: 'auto-replies-v6/cmDfJ.txt' , verdictHead: '7b509106' },
  {
  ...
  },
  {
    ...
    */
    ...
    steps: [{ click: '削除' }],
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#34 フォルダの合計が16で「すべて14」と食い違っていたので14に合わせた（`cmDfJ`）／#35 「有効のみ」が選択中なのに条件外の行が並んでいたので未選択に／#40 主ボタンの緑を `$accent-deep` へ／#41 完了画面の追従バーを「一覧へ／内容を見る」に（`e6iJG`）／#46 一覧の行に「その他操作（…）」（5行）／#49 「自動応答ルールを保存」→「反応条件へ」（次工程名にそろえた）。`g46ja` は**進む押し口が1つも無かった**ので「最終確認へ」を新設し、テスト実行を副次にした。設計画像は `docs/design-reference/auto-replies-v6/` を撮り直した。**実装との突き合わせはこれから。** **`7b509106` の1440・1920を設計と見比べて、一致に変えた。** 削除の確認窓は 見出し／取り消せないことを書いた本文／キャンセル・削除 の並びまで設計どおり。設計は「「旧キーワードルール」を削除しますか？」「削除すると新しい受信への自動返信とアクション実行が停止します。過去の実行履歴は監査記録として残り、この操作は取り消せません。」、実装は「自動応答「営業時間外の自動返信」を削除しますか？」「新しく届くメッセージへの自動返信と、タグ付けなどの後続処理が止まります。過去の実行履歴は削除されません。この操作は元に戻せません。」。**言葉の違いは2か所だけ**——「監査記録」という内部語を使わない言い方に直したこと（今夜の内部語撤去どおり）と、押し口が「削除する」→「自動応答を削除」と**何を消すかを書いている**こと。どちらも設計の意図から外れていないので差として数えない。**壊れ値・内部IDは0件。**',
  ...
  */
  ...
  {
    ...WEBINAR, node: 'd3rFGD', name: '10-1-C CTA・フォーム', route: WEBINAR_EDIT, verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** #623で段の3番目になり、head `96ed41b6` で右にLINEプレビュー（ボタン文言をそのまま見せる）を足した。**保存の押し口が2つある**ことは画面で先に断っている。残る差：設計は入力項目・完了アクションを1行ずつたたんで見せる', verdictSource: 'Claude実装',
    steps: [{ qaOpen: 'd3rFGD' }],
    verdict: 'unjudged',
  ...
  },
  {
    ...
    steps: [{ qaOpen: 'Ho8z4' }],
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** **撮り直し待ち。判定・head とも動かしていない。** `Ho8z4-1440.png` は **8/31 23:30** のままで、今夜の11本が入る前の絵。今日の撮り直し（固定base `7b509106`）は機能10のこの Node にまだ届いていない（同じ機能でも `GB0NR` と `yxyzQ` は 9/2 12:09 に撮り直せている）。**中身を見ずに判定を動かさない。** 撮れ次第、設計（STEP 4「通知・リマインド」）と見比べる。',
    verdictSource: 'Claude実装',
    verdictHead: '96ed41b6',
  },
  {
    ...WEBINAR, node: 'Xjk8q', name: '10-1-E 視聴後アクション', route: WEBINAR_EDIT, verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** **P2 視聴後アクションが `PV1Vh` と同じ1枚の中にある。** ルート `/webinars/edit?id=wb-1`。設計は別の段。取得元：`webinars-v6/Xjk8q.txt`。1440・1920とも横スクロール0 **推奨修正**：`PV1Vh` `d3rFGD` と同じ束。**3つまとめて段へ分ける。**', verdictSource: 'webinars-v6/Xjk8q.txt',
    steps: [{ qaOpen: 'Ho8z4' }],
    verdict: 'unjudged',
  ...
  },
  {
    ...
    steps: [{ qaOpen: 'D6yO7e' }],
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** **撮り直し待ち。判定・head とも動かしていない。** `D6yO7e-1440.png` は **8/31 11:37** のままで、今夜の11本が入る前の絵。今日の撮り直し（固定base `7b509106`）は機能10のこの Node にまだ届いていない（同じ機能でも `GB0NR` と `yxyzQ` は 9/2 12:09 に撮り直せている）。**中身を見ずに判定を動かさない。** 撮れ次第、設計（STEP 5 手前の「公開前確認」）と見比べる。',
    verdictSource: 'Claude実装',
    verdictHead: '988cc37a',
  },
  {
  ...
  {
    ...WEBINAR, node: 'Q8sHa', name: '10-1-I 参加者管理', route: WEBINAR_EDIT, verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** head `96ed41b6` で**参加者管理と分析を別の画面に分けた**（設計どおり）。同じ1枚に混ぜると、どちらを見ているのか分からなくなる。口も数も増やさず、出し分けだけを変えた。残る差：設計の参加者管理は絞り込みと並び替えを持つ', verdictSource: 'Claude実装',
    steps: [{ click: '参加者' }],
    verdict: 'unjudged',
  ...
  },
  {
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しを副次ボタンに統一（`yxyzQ` が緑の主ボタンだった）／#48 表記統一。設計画像は `docs/design-reference/webinars-v6/` を撮り直した。**実装との突き合わせはこれから。** **#524 `a6c35ee0` で束4の完了条件を満たした。** 失敗のとき帯は **ウェビナー `—件`／公開中 `—`／申込 `—`** で、ウェビナーだけ `0件` と数えていた差は解消した。読込・空・失敗が分かれる。1440・1920とも横スクロール0。**画面全体は要修正のまま**：P2 設計の一覧の作り（回ごとの申込の推移、視聴後のフォロー配信の状態）はこの直しの外 **ルート**：`/webinars`（空・読込・失敗）。**取得元**：`webinars-v6/zCQXe.txt`。**推奨修正**：一覧の作り（回ごとの申込の推移、視聴後のフォロー配信の状態）は `ZC13r` と同じ束なので一緒に。',
  ...
  */
  ...
  },
  {
    ...
    */
    ...
    mode: 'viewport', height: 1080,
    steps: [{ click: '削除', scope: 'main' }], verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 表記統一。設計画像は `docs/design-reference/templates-v6/` を撮り直した。**実装との突き合わせはこれから。** P2 設計の「先に差し替えてから削除する」流れが無い。設計は使用先を3つ挙げ、差し替え画面へ誘い、そのまま消すときは名前を打ち直させる。実装は使用中のテンプレートから削除の導線ごと外す作りで、危険は無い代わりに使用中のものを整理する手段が無い。窓にも使用先が出ない **ルート**：`/templates`（削除の窓）。**取得元**：`templates-v6/design-qa.md`。**推奨修正**：**窓に使用先を出すのが先**。いまは使用中のテンプレートから削除の導線ごと外す作りで危険は無いが、**使用中のものを整理する手段も無い**。`uNBlA`（変える前に影響を見る）が同じ「使用先を数えて出す」形を既に持っているので写す。', verdictHead: '7b509106',
    verdict: 'unjudged',
  ...
  */
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 表記統一。設計画像は `docs/design-reference/rich-menus-v6/` を撮り直した。**実装との突き合わせはこれから。** **#583 `bb1e4dfd` で、前に挙げたP1（出し分けの条件を決める画面なのに、当てはまったときどれが優先されるかが出ない）が解消した。** ルート `/rich-menus/edit?id=rmg-1`。1440・1920とも横スクロール0。 **「出す順番」の節が入った。** 「**一覧で上にあるメニューが優先されます。現在は4番目です。**」と出て、その下に順番を直す数値欄がある。**#583 で一覧側（`GO8RQ`）に入った断りと、同じ言い回しでそろっている。** **表示は1番始まり、保存値は0始まりのまま。** 口が返した `rmg-1` の `targetingPriority` は **3**、画面は **4番目** と出す（`edit/page.tsx:794` の `targetingPriority + 1`）。直すときも `(入力値 || 1) - 1` で0始まりへ戻すので、**保存の形は変わらない**（`:313` がそのまま `targetingPriority` を送る）。**人が数える番号と、機械が持つ番号を取り違えない作り。** **出し分けを入れたときだけ出る。** 既定は切れており、「条件で出し分ける」を入れると現れる（`:781` の `targetingEnabled && (`）。**使わない人には見せない**判断で、条件を決める場面と対になっている。切るときの断り（「切ると、このメニューは条件で配られなくなります。すでに見えている人からは すぐには消えません。」）も出る。 **未登録のときの断りもある**：「このメニューはまだ LINE に登録されていません。登録するまで、条件に…」。**条件が空のときも止める**：「条件が空です。このままだと誰にも出しません。条件を1つ以上足してください。」 **`undefined`・`NaN`・`Invalid Date`・`API error`・内部ID（`rmg-` `rmp-`）は0件。** 取得元：`rich-menus-v6/kQ1bs-targeting.txt` ＋ `edit/page.tsx:781,794,313`', verdictSource: 'rich-menus-v6/kQ1bs-targeting.txt + edit/page.tsx:794' , verdictHead: '7b509106' },
  {
    ...
    */
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 表記統一。設計画像は `docs/design-reference/rich-menus-v6/` を撮り直した。**実装との突き合わせはこれから。** **#509 `4cf82bd9` で撮り直した。設計 12-1-C の要点をすべて満たす。** ルート `/rich-menus/connections?id=rmg-1`。1440・1920とも横スクロール0。 **設計がいちばん心配していた「戻れない」を実際に見つけている。** 帯は ページ3枚／切替ボタン3件／**確認事項1件**／公開状態 公開中。上に「**1件の確認事項があります。公開する前に、入口からの行き先と戻り道を確認してください。**」。つながりは トップ（入口のページ）→ 商品を見る／予約する、商品を見る → トップへ戻る（**入口へ戻れます**）、予約する → 「別ページへ切り替えるボタンはありません。**・入口へ戻れません**」。 **内部IDを出さないことを画面で宣言している**——「矢印は保存済みの切替ボタンです。**番号や内部IDは表示しません。**」。実際に本文を数えて `rmg-` `rmp-` はいずれも0件。 **切替先が下書きのままかも分かる**——「LINEへ公開済み」「**LINEへ未公開**」。公開前に気づける。 **P2 設計の「よくある事故」3つのうち、切替先だけ『誰に出すか』が違う場合は出ない。** ただし**この画面の担当ではない**——出し分けの条件はメニューごとに持つもので、`kQ1bs` の担当。**推奨修正**：`kQ1bs` に出し分けの条件が出るようになったら、切替先の条件もここへ並べる。 取得元：`rich-menus-v6/DIUbO.txt`',
  ...
  { ...RICH_MENU, node: 'TL7tp', name: '12-1-E 管理画面の外のメニューを取り込む', verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 表記統一。設計画像は `docs/design-reference/rich-menus-v6/` を撮り直した。**実装との突き合わせはこれから。** **P2 管理画面の外のメニューが、設計では別画面だが実装は一覧の中の節。** ルート `/rich-menus`。「LINE 公式アカウントの現状」の節に、LINE 上のメニューが並び、行ごとに **「管理画面に取り込む」「LINE から削除」** が出る。取り込みと削除はできる。**良い点**：「LINE 上に登録されているメニュー 3 個／うち 2 個が管理画面外」と**管理下と管理外を数えて分ける**。名前の無いメニューを「テスト用（名前なし）」と書き、空欄にしない。管理下のものは「管理画面 → 通常メニュー（会員向け）(トップ)」と**どれに対応するか**を出す。**P2 残る差**：設計 12-1-E は取り込む前に「何が上書きされるか」を出すが、実装は押すとすぐ取り込む。**この画面の並び順は #583 より前の絵**（「タップ数が多い順」が既定）。取得元：`rich-menus-v6/TL7tp.txt`。1440・1920とも横スクロール0 **推奨修正**：取り込む前に「何が上書きされるか」を出す。**`DIUbO`（切替のつながり）が既に「入口へ戻れません」を出せている**ので、同じ考え方で取り込み先の重なりを見せる。', verdictSource: 'rich-menus-v6/TL7tp.txt' , verdictHead: '7b509106' },
  {
    ...
    */
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 表記統一。設計画像は `docs/design-reference/rich-menus-v6/` を撮り直した。**実装との突き合わせはこれから。** **#616 `0a11c9e8`（#608 `4923de9d` の上）で一致。** ルート `/rich-menus`（削除の窓）。1440・1920とも横スクロール0。いま表示している人数／次に出るメニュー／切替元／使っている自動処理を表示。人数の `null` は `—（未取得）`、実値0は0人。内部のblockerを日本語にし、塞がれている・影響が読めない場合は確認ボタン自体を出さない。再監査で返した2件も解消した：DELETE 409 の `ApiError.data` から最新impactを型確認して再描画し、影響取得は対象groupIdと照合して別メニューの遅延応答を捨てる。対象21件・required-pr-gate SUCCESS・CLEAN。再監査：[#616 コメント](https://github.com/skmtmst/line-harness-oss/pull/616#issuecomment-5473175349)',
    verdictSource: 'rich-menus-v6/szXsT.txt + rich-menus/page.tsx（-managed は6番目の削除ボタンが無く未撮影）', verdictHead: '7b509106',
  },
  {
  ...
  */
  ...
  { ...FORM, node: 'v9tYhl', name: '13-1-D 集まった回答', steps: [{ click: '来店アンケート' }], verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一。設計画像を撮り直した。**実装との突き合わせはこれから。** **画面全体は要修正のまま。** P1 情報欄への書き込みが失敗した件数が出ない（設計は「3件は欄が消えていて書けていません」）。**答えは受け取れているのに友だち情報へ入っていない状態が、画面のどこにも出ない。** P1 「1件ずつ見る／まとめて見る」の切り替え、絞り込み、CSVで書き出すが無い ／ **#436 の受入条件は確認済み**（head `35c613a6`）。回答の表は 友だち名・日時・各項目で、**答えの無い項目は `—`**、友だちが分からない行は「不明」、未記入は「（未記入）」と書き分ける。下に **「1〜4件 / 全4件」「20件表示」「前へ 1 次へ」** が出て、APIが返した `page` / `limit` / `total` をそのまま使う。1440・1920とも横スクロール0。`undefined` / `Invalid Date` / `NaN` / 内部IDは0件 **ルート**：`/form-submissions`（集まった回答）。**取得元**：`forms-v6/v9tYhl.txt`（#436 head `35c613a6`）。**推奨修正**：**情報欄への書き込みが失敗した件数を先に出す**——答えは受け取れているのに友だち情報へ入っていない状態が、いま画面のどこにも出ない。**気づけないまま数日たつのがいちばん重い。** 切り替え・絞り込み・CSVはそのあと。', verdictSource: 'forms-v6/v9tYhl.txt（絵は「来店アンケート」の行が無く未撮影）' , verdictHead: '7b509106' },
  {
  ...
  */
  { node: 'hqrOv', feature: 4, name: '4-1 友だち属性・タグ', dir: 'friend-attributes-v6', route: '/tags', mode: 'page',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 第2ラウンド第1段で撮り直して判定。** ルート `/tags`。1440・1920とも横スクロール0。 フォルダ分けとタグの並びは設計どおり。 **P1 設計にある「使用先」が出ない**——設計はタグ・項目・条件ごとに「配信3・フォーム1」「シナリオ2」「保存検索2」のように**どこで使われているか**を書くが、実装は出さない。消してよいかを画面から判断できない。機能4で5画面（`hqrOv` `dKlkz` `QKx8Q` `KoT6c` `XBkiQ`）に共通する同じ口の不足で、**1つ足せば5画面が直る**。 残りの差はタグ名の固定データ。',
    verdictSource: 'friend-attributes-v6/hqrOv.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    steps: [{ click: '削除', scope: 'main' }],
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags`（タグ削除の確認）。1440・1920とも横スクロール0。 確認の窓は出る。 **P1 設計にある影響の内訳（「シナリオ2・自動応答1・保存検索1」）が出ない。** 何が壊れるか分からないまま消すことになる。`hqrOv` と同じ「使用先」の口。 §7 #9 は既定を「アーカイブ」に変える依頼を出しているが、**§7 は未反映**（board#101）なので設計側の判断待ち。',
    verdictSource: 'friend-attributes-v6/dKlkz.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags`（CSV一括登録）。この画面は `status: \'elsewhere\'` で `capture.spec.mjs` が基準画像を持つ。 **P1 設計の入口の文言が実装に無い**：「CSVでタグを一括登録」「登録するCSV」「CSVファイルを選択」。 §7 #10 は解析結果・重複・要修正・実行結果の状態を足す依頼を出している（未反映）。',
    verdictSource: 'friend-attributes-v6/H374MR.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags`（CSV取り込みの確認）。 **P1 設計にある取り込み前の内訳が出ない**：「新規 412／飛ばす 73／エラー 15」の3分類、「使えない文字が入っています（3文字目）」のような行ごとの理由、「エラーの15行は登録されません」の断り。**何件入って何件落ちるか分からないまま実行することになる。** なお「dry-run」は §7 #48 が内部語として挙げているので、画面の言葉は「取り込み前の確認」に寄せる。',
    verdictSource: 'friend-attributes-v6/sfTEW.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags`（CSV取り込み・完了）。 完了の知らせは設計どおりで、文字の差はタグ名の固定データと `hqrOv` と同じ「使用先」の欠落だけだった。',
    verdictSource: 'friend-attributes-v6/op1rh.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags`（CSV取り込み・一部失敗）。 **P1 落ちた行の扱いが設計より薄い**：設計は行ごとの理由（「使えない文字が入っています（3文字目）」「タグ名が長すぎます（40文字まで）」）と**「入らなかった10行をCSVで出す」**を出すが、実装に無い。**入らなかった行を直して入れ直す道が画面に無い。**',
    verdictSource: 'friend-attributes-v6/QzRsJ.txt',
    verdictHead: '49e1341c',
  },
  { node: 'HBTk0', feature: 4, name: '4-2 友だち情報欄', dir: 'friend-attributes-v6', route: '/tags?tab=fields', mode: 'page',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags?tab=fields`。1440・1920とも横スクロール0。 項目の一覧と差し込みキー（`{{field.birthday}}` など）、表示先の列は設計どおり。 **P2 設計にある使用先の数（「回答フォーム 3個」）が出ない。**実装は表示先の種類だけで、何個で使われているかが無い。`hqrOv` と同じ口。',
    verdictSource: 'friend-attributes-v6/HBTk0.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags?tab=fields`。空・読込中・取得失敗の3状態を1440・1920で撮れた（6枚、はみ出し0）。 状態の出し分けは設計どおり。差は差し込みキーの固定データ。',
    verdictSource: 'friend-attributes-v6/yKEdO-empty.txt',
    verdictHead: '49e1341c',
  },
  { node: 'rIhbN', feature: 4, name: '4-3 対応マーク', dir: 'friend-attributes-v6', route: '/tags?tab=marks', mode: 'page',
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags?tab=marks`。1440・1920とも横スクロール0。 対応マークの一覧は設計どおりで、文字の差は見つからなかった。 §7 #13 が「対応状況とは別の自由分類」と明記する依頼を出している（未反映）。',
    verdictSource: 'friend-attributes-v6/rIhbN.txt',
    verdictHead: '49e1341c',
  },
  {
    node: 'QKx8Q', feature: 4, name: '4-4 保存した検索', dir: 'friend-attributes-v6', route: '/tags?tab=searches', mode: 'page',
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags?tab=searches`。1440・1920とも横スクロール0。 帯4つのうち3つが `—`＋理由で、**取れないものを0で埋めていない**のは規則どおり。 **P1 設計にある条件の要約と使用先が出ない**：設計は1件ごとに「タグ VIP／未契約・AND」「誕生日が今日から30日以内」の要約と「一斉配信2・自動処理1」の使用先を並べるが、実装は名前と削除だけ。**消してよい条件か画面から判断できない。** 受信箱の `ASsb3` と**まったく同じ不足**なので、口は1つで足りる。',
    verdictSource: 'friend-attributes-v6/QKx8Q.txt',
    verdictHead: '49e1341c',
  },
  ...
  // ここには、設計と並べるために撮るものだけを置く。
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/new`。1440・1920とも横スクロール0。 マイル連動の3項目（本人／紹介者／倍率）は設計と同じ中身で、実装は「● 本人へのマイル付与」のように見出しを足している。 P2 設計は `a.` `b.` `c.` の記号で段を振るが、実装は記号なし。どちらかにそろえる。',
    verdictSource: 'friend-attributes-v6/l25rlp.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/new`（連動ON）。1440・1920とも横スクロール0。 **P2 設計にある説明が実装に無い**：「d. 複数の倍率を持つときの優先度」「複数のタグを持つ場合、いちばん高い優先度の1枚だけが効きます。」「e. タグを外して付け直したときの扱い」。実装は優先度1〜5を選ばせるが、**何が起きるかの説明が無い**ので選べない。 実装が足している断り（「OFFのままでも、タグの手動付与・配信の絞り込み・シナリオ条件には使えます。」）は良いので残す。',
    verdictSource: 'friend-attributes-v6/tP0RW.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/new`（連動アクション追加）。1440・1920とも横スクロール0。 **P2 ドロワーの説明が設計より薄い**：設計は「「NEN会員（定期）」が付いたときに実行する処理を選びます。」と対象のタグ名を出し、選んだ処理の中身（「画像1枚 ＋ テキスト…＋ ボタン2つ」）まで書くが、実装に無い。 `tP0RW` と同じ優先度の説明不足も同じ面に出る。',
  ...
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/edit?id=tag-0`。1440・1920とも横スクロール0。 実装が足している断り（「一覧・チャット・CSVから、このタグを手で付けられます。」「OFFに戻すと、これ以降このタグが付いても連動は動きません。 すでに積んだマイルは取り消されません。」）は設計より親切で良い。 **P2 設計にある「f. 変更した内容を、いまこのタグが付いている人にもさかのぼって適用します。」の段が実装に無い**——遡って反映するかどうかを編集画面で選べない（確認だけ `VjXGX` に出る）。',
    verdictSource: 'friend-attributes-v6/ee0sk.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/edit?id=tag-0`（遡及反映の確認）。1440・1920とも横スクロール0。 確認の窓は出て、**倍率はさかのぼらない**という区別も出ている。 **P2 設計にある付与量の内訳（「+1,280 mile」「+170 mile」「+1,450 mile」）が実装に無い**——何マイル積まれるか分からないまま「適用」を押すことになる。取り消せない操作なので、数は出したい。',
    verdictSource: 'friend-attributes-v6/VjXGX.txt',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/folders/new`。1440・1920とも横スクロール0。 フォルダの名前と色を決める形は設計どおり。 文字の差は**背面のタグ一覧が窓に隠れて写らない**ぶんで、この画面そのものの差ではない。',
  ...
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/fields/new`。1440・1920とも横スクロール0。 項目の種類・差し込みキー・表示先の選び方は設計どおり。 **P2 設計にある3つが実装に無い**：種類の「画像」「PDF」、既定値の説明「友だち情報が空欄のときに、この値が代わりに送信されます。200字まで。」、ON/OFF の切り替え。 §7 #11 は「画像・PDFは差し込み不可、メディア参照であることを表示」と決めているので、**種類を足すより先に設計の反映待ち**（board#101）。',
    verdictSource: 'friend-attributes-v6/A1ZYeP.txt',
    verdictHead: '49e1341c',
  },
  {
    /*
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/fields/migrate?id=field-birthday`。1440・1920とも横スクロール0。 移行元と移行先を並べる形は設計どおり。 **P1 設計にある2つが実装に無い**：「使用先：テンプレート4・フォーム2・予約1」（`hqrOv` と同じ口）と「元の項目：移行後30日間は読み取り専用で保持」。**切り戻せる期限が画面に出ない**ので、いつまでに気づけばよいか分からない。§7 #12 も同じ指摘。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 撮れなかった。判定できない。** ルート `/tags?tab=marks`（対応マークを追加・編集）。通常・読込中・空・取得失敗・権限不足・衝突の6状態すべて。 撮る前に押す「保留」の行が固定データに無い。**撮影用のモックが返す対応マークに「保留」が含まれていない。** 実装の不具合ではない。モックは撮影ハーネス側（S0）なので Issue で渡す。',
    verdictSource: 'friend-attributes-v6/（撮れず）',
    verdictHead: '49e1341c',
  },
  {
    ...
    */
    ...
    verdict: 'unjudged',
    verdictNote: '**2026-09-03 撮れなかった。判定できない。** ルート `/tags?tab=marks`（対応マーク削除の確認）。 押す「対応中を保管」が固定データに無い。`GMvBd` と同じモックの不足。',
    verdictSource: 'friend-attributes-v6/（撮れず）',
    verdictHead: '49e1341c',
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-03 撮り直して判定。** ルート `/tags/searches/edit?id=ss-1`。1440・1920とも横スクロール0。 内部の識別子（`ss-` `support_mark` `scenario_id`）は0件で、選ぶ形になっている。 **P1 設計にある条件の組み立てが実装に無い**：「すべて満たす（AND）」「＋ AND条件を追加」「いずれか1つ以上満たす（OR）」「＋ OR条件を追加」の入れ子と、使用先（「一斉配信「VIP未契約案内」」「オートメーション「3日後フォロー」」）。**AND と OR を混ぜた条件をこの画面で作れない。** `QKx8Q` の使用先と同じ口。',
    verdictSource: 'friend-attributes-v6/XBkiQ.txt',
    verdictHead: '49e1341c',
  },
]
...
export const CAPTURED_AT = {
  4: [
    { pr: 0, head: '49e1341c', on: '2026-09-03',
      screens: ['hqrOv', 'dKlkz', 'H374MR', 'sfTEW', 'op1rh', 'QzRsJ', 'HBTk0', 'yKEdO', 'rIhbN', 'QKx8Q', 'l25rlp', 'tP0RW', 'LfrQs', 'ee0sk', 'VjXGX', 'byqIW', 'A1ZYeP', 'KoT6c', 'XBkiQ'],
      note: '第2ラウンド第1段。19画面44枚、はみ出し0。'
        + '**`GMvBd`（対応マークを追加・編集、6状態）と `zGZMA`（削除確認）は撮れていない**——'
        + 'モックが返す対応マークに「保留」「対応中」の行が無く、押しどころが描かれないため' },
  ...
  5: [
    { pr: 0, head: '49e1341c', on: '2026-09-03',
      ...
      note: '第2ラウンド第1段。14画面38枚（状態ぶんを含む）、はみ出し0。撮れなかった画面は無い' },
  ...
  2: [
    { pr: 0, head: '49e1341c', on: '2026-09-03',
      screens: ['xGLVe', 'NfgOs', 'H3lAOB', 'Xi4x9', 'f0zn6', 'NWbuF', 'B7CER8', 'YZaDK', 'L35UOV', 'IYjvu', 'TUveA', 'w72a2', 'ASsb3', 'ANgda', 'tBlkL', 'AuSDY', 'LHjwD'],
      note: '第2ラウンド第1段。17画面38枚、はみ出し0。'
        + '**`YZaDK-selected`（担当者ごとの未読）は撮れていない**——`GET /api/chats/stats` に担当者ごとの数が無いため（board#33）。'
        + '**`ASsb3-menu` も撮れていない**——実装に `data-qa-open` の印が無い。'
        + '`TUveA` はフォルダの読み上げ名が変わっていて撮れず、この台帳の `steps` を直してから撮った' },
  ...
  3: [
    { pr: 0, head: '49e1341c', on: '2026-09-03',
      screens: ['PhxG6', 'LT8RS', 'Igi72', 'I6UAdr', 'bzDn6', 'YzxU1', 'InCDe', 'r7eSi', 'w8W4Eh'],
      note: '第2ラウンド第1段。9画面48枚（状態ぶんを含む）、はみ出し0。'
        + '**`IAf7j`（一括操作）の5状態は撮れていない**——モックの `/api/line-accounts` が `role` を返さず、'
        + '`canRunBulk`（owner か admin）が false になって「操作を選ぶ」が描かれないため。'
        + '**`w8W4Eh-edit` `w8W4Eh-conflict` も撮れていない**（固定データの行に押しどころが出ない）' },
  ...
  1: [
    { pr: 0, head: '49e1341c', on: '2026-09-03', screens: ['vUXKb', 'ZN0ov', 'JN6mQ', 'NjK9q', 'Alekb'],
      note: '第2ラウンド第1段。**S0 の #719（共通部品の「準備中」全廃）を取り込んだあとの codex/development で撮り直した。**'
        + '5画面10枚、はみ出し0。撮影は内蔵SSDの作業ツリー、モックは 8798 番（8788 番は別セッションが握っていたため）' },
