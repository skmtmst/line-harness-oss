...
const FEATURE_32_REVIEW = {
  UgonK: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、`62fe5fdbb` を1440・1920で設計と目視比較。** 6列（確認する項目／結果／いまの数字／目安／最後の確認／操作）、次回確認時刻、判定の見方4種を追加し、横はみ出し0。画面を開いている間は5分ごとに再取得する。**残るもの**：サーバー側の定期実行結果を保存して共有する専用の健全性チェック口が無く、端末を閉じた状態の自動確認・異常履歴は未接続。',
    verdictSource: 'operations-v6/UgonK.txt',
    verdictHead: '62fe5fdbb',
  },
  b3HfZ: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、`62fe5fdbb` を通常・読込・失敗の各状態で1440・1920撮影。** 停止前の件数・人数、オートメーションと自動応答を同時に止める対象、サーバー共通の停止状態、権限、版競合を表示・送信する。取得失敗時は0件にせず停止ボタンを無効化。横はみ出し0。**残るもの**：設計の右側にある「止めるとどうなるか／止めたあとにすること」と下部の固定操作帯が無く、ログインユーザーへのLINE・メール通知も未接続。',
    verdictSource: 'operations-v6/b3HfZ-normal.txt + operations-v6/b3HfZ-loading.txt + operations-v6/b3HfZ-error.txt',
    verdictHead: '62fe5fdbb',
  },
  UhC2O: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、`62fe5fdbb` を1440・1920で設計と目視比較。** 端末内保存と公開管理鍵の読出しを廃止し、停止・復旧履歴をサーバーから取得。期間、CSV、4つの概要、設計と同じ5列、直近10件のシステム更新を表示し、横はみ出し0。**残るもの**：設計の各行「中身を見る」と右側の監査説明・関連画面への導線が無い。システム更新も反映時間・停止時間まで記録する配備台帳とは未接続。',
    verdictSource: 'operations-v6/UhC2O.txt',
    verdictHead: '62fe5fdbb',
  },
  U0BwS: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、`62fe5fdbb` の最終確認を1440・1920で設計と目視比較。** 下見と同じ件数・人数を再計算せず表示し、選ばなかった自動処理・手動返信・予約受付が止まらないこと、LINEへ渡した配信は取り消せないことを明示。「停止」入力と版番号つきサーバー保存で誤操作・同時更新を防ぐ。横はみ出し0。**残るもの**：操作専用の段階認証と、ログインユーザー全員へのLINE・メール通知が未接続。',
    verdictSource: 'operations-v6/U0BwS.txt',
    verdictHead: '62fe5fdbb',
  },
}
...
const FEATURE_15_REVIEW = {
  g89Tc: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #228 / PR #TBD / HEAD 3eae16770 で再撮影し、構造一致・データ未接続。** 1440px・1920pxとも横はみ出し0。設計と同じ10件を、3つの名前付きフォルダと未分類へ分け、検索、種類・未使用の絞り込み、表示切替、並び順、表示件数、カード一覧、一括削除まで目視比較した。保存容量APIが無いため使用量・上限と「上限に近い」絞り込みは `—` と接続条件のまま。カード内の操作密度と設計の件数186件にも差が残る。**接続条件:** 容量集計APIを接続し、実件数・使用量・上限・上限付近を実値で出して再撮影する。',
    verdictSource: 'media-v6/g89Tc.txt + g89Tc-1440.png + g89Tc-1920.png',
  },
  voJtX: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、Issue #228 / PR #TBD / HEAD 3eae16770 で再撮影し、要修正。** 1440px・1920pxとも横はみ出し0。夏の定番セットのプレビュー、容量・寸法・登録者・フォルダ、名前付き使用先3件、ダウンロード、一括差し替えを確認した。ただし設計は大きなプレビューと右欄を持つ詳細ページで、実装は一覧上の小さな確認窓となり、位置・寸法・情報量が一致しない。名前とURLを保つ版追加APIも未接続。**推奨修正:** 設計の詳細ページ構成へそろえ、版追加API接続後に「差し替える」の選択済み・失敗状態も再撮影する。',
    verdictSource: 'media-v6/voJtX.txt + voJtX-1440.png + voJtX-1920.png',
  },
  eXAJP: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、Issue #228 / PR #TBD / HEAD 3eae16770 で再撮影し、要修正。** 1440px・1920pxとも横はみ出し0。「LINEで送れる大きさ」の文言、20件までの選択、形式・容量・公開リンクの注意、フォルダ選択、登録件数と閉じる操作を確認した。ただし設計は完了2件・上限超過1件の進捗状態を示す一方、撮影は未選択の初期状態だけで、1件ごとの待機・登録中・完了・失敗を画像で確認できない。現行はWorker経由のbase64で動画90MBまで、設計の200MBとR2直接アップロードも未接続。**推奨修正:** 撮影用ファイル選択状態を追加し、全進捗状態と200MB上限を同じ2幅で再撮影する。',
    verdictSource: 'media-v6/eXAJP.txt + eXAJP-1440.png + eXAJP-1920.png',
  },
  YfTfJ: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #228 / PR #TBD / HEAD 3eae16770 で使用中・削除可能を再撮影し、構造一致・データ未接続。** 両状態の1440px・1920pxで横はみ出し0。夏の定番セット、使用先3件と安全な導線、使用中は削除させない案内、別メディアへの一括差し替え、閉じる操作を確認した。設計より窓が横長で、画像プレビューと緑の差し替え説明が無く、名前とURLを保つ版追加APIも未接続。**接続条件:** 版追加APIを接続し、プレビューを含む設計寸法へそろえ、409再取得・差し替え失敗も再撮影する。',
  ...
  h8pBZr: {
    verdict: 'match',
    verdictNote: '**2026-09-06、Issue #228 / PR #TBD / HEAD 3eae16770 で通常・読込中・0件・取得失敗を再撮影し、一致。** 全4状態の1440px・1920pxで横はみ出し0。3つの名前付きフォルダと未分類を残したまま、読込中は待機、0件は登録導線、失敗は再読込を混同せず表示する。状態ごとの文言・色・操作と通常時の共通枠を設計画像と同じ比較入力で目視確認した。保存容量APIの未接続表示は親一覧 `g89Tc` の接続条件として判定を残す。',
...
export const CAPTURED_AT = {
  ...
  32: [
    { pr: 0, head: '31293424', on: '2026-09-04', screens: ['UgonK','b3HfZ','UhC2O','U0BwS'],
      note: 'S3 第1段。**土台を直してから撮り直した。** 撮影ハーネスの押し口とルートが入れ替え前の固定データを指していたのと、モックに口が無くて画面が落ちていたのを直した（台帳の直しはこの枝、モックの直しは #728）。実装は `codex/development` そのもの。**絵は版に残さない**（#730 の決めごと）ので、証拠は `.txt` と判定の注記。' },
    { pr: 0, head: '62fe5fdbb', on: '2026-09-06', screens: ['UgonK', 'b3HfZ', 'UhC2O', 'U0BwS'],
