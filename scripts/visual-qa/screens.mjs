...
export const SCREENS = [
  ...
  */
  ...
  { ...INBOX, node: 'ASsb3', name: '2-13 保存した検索を開く', steps: [{ click: '保存した検索' }],
    /*
      **変種の手順は基本の手順の後ろに足される。** 基本で「保存した検索」を
      開いてあるので、ここでもう一度押すと閉じてしまう。行の「…」を開くだけにする。
    */
    variants: [{ suffix: '-menu', steps: [{ qaOpen: 'ASsb3', after: 600 }] }],
    verdict: 'needs_fix',
    verdictNote: '**2026-09-04 古い形の保存条件で落ちるのを直し、条件の要約を出して撮り直した（board#43）。** ルート `/chats`（「保存した検索」）。通常と「…」を開いた形を1440・1920で撮った（4枚、はみ出し0）。 **① 受信箱より前に作られた保存を開くと受信箱ごと落ちていた。** 古い行は `{ all: [], any: [] }` の形で入っているのに、画面は `InboxSavedViewConditions` と決めつけて `conditions.statuses.length` を読んでいた。`normalizeSavedViewConditions()` で知っている値だけ拾い、分からない形は「何も絞っていない」として扱う。**撮影の固定データが3件とも古い形だったのでこの道を一度も通っていなかった**ので、2件を受信箱がいま保存する形にし、1件だけ古い形のまま残した。 **② 名前の下に条件の要約を出した**：「未対応・保留 ／ LINE」「未対応 ／ 担当 未割り当て」、古い形の1件は「すべての会話」。担当者のidは名前へ直し、分からないidは「不明な担当者」。 **③ 赤字の「削除」を名前の隣から「…」へ畳んだ**（名前を変える／消す）。選ぶつもりで押し間違える並びだった。消す前に「この保存した検索だけが消えます…この操作は取り消せません。」と言う。 **残る差（P2）**：設計は1件ごとに**該当件数**（「1件」「3件」）を出すが実装は出さない——**いまの読み口では一覧を丸ごと数え直さないと出せないので、数を作らないほうを選んだ**。`QKx8Q`（4-4）と同じ口が要る。設計の要約は「対応マーク：未対応／期限：超過」と項目名つき、実装は値だけ。保存ボタンが設計「現在の条件を保存」／実装「この条件を保存」。',
    verdictSource: 'inbox-v6/ASsb3.txt',
    verdictHead: '68a57abd',
  },
  /*
