  ...
  */
  {
    ...
    */
    ...
    verdict: 'unjudged', verdictNote: '**2026-09-03 Pencilを直したので未判定に戻した。** 横断レビュー §7 の反映：#31 サイドメニューの選択状態を受信箱に／#36 開いたプルダウンが欄から11px浮いていたのを直す（`YZaDK` `L35UOV` `IYjvu`。`L35UOV` は横も8pxずれ）／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#48 対応済→対応済み・未割当→未割り当てほかの表記統一。設計画像は `docs/design-reference/inbox-v6/` を撮り直した。**実装との突き合わせはこれから。** 契約枝のローカルcommit `4b97fab1` が足した `InboxStats.assigneeUnread` を読むようにした。**前の判定（担当者ごとの未読が出ない）は解消。** 「未割り当て 2」「Kenta 3」のように未読数を添え、**配列に出てこない担当者は実値0**として「Masato 0」と描く（0件は配列に載らない契約）。**集計の失敗は0件と扱わず、数だけ `—`** にして担当者一覧そのものは残す（別の口）。**画面に見えている行から数えない**——一覧はページ送りされるので2ページ目の未読が落ちる。通常・0件・失敗・選択中の4状態を撮った（8枚、はみ出し0）。`GET /api/chats/stats` はアカウント引数を取らない作りなので、**新しい契約は足さず**切り替えのたびに読み直すだけにした。残る差：設計は担当者ごとの未読を左の縦帯にも出すが、実装は絞り込みの選択肢まで',
