  ...
  */
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-04 画面を作って初めて撮れた（board#135）。前は `unimplemented`（`gap: pending`）で、判定は未マージ枝で書かれたものだった。** ルート `/mileage/rewards/edit`（`?id=` があれば編集）。1440・1920で撮った（はみ出し0）。 **API は既に本流にあった**（`POST /api/mileage/rewards`・`/draft`・`/publish`・`/stop`）。無かったのは画面と web 側の口だけ。 **一覧の行き止まりを解いた**：`qlVLJ` に「使い道をつくる」と行ごとの「内容を編集」を置いた。前は**押した先が無いので置けなかった**。 **「渡すもの」は並べたタイル**（クーポン／タグ／シナリオ／メッセージ／先にお知らせ／ランク）で、それぞれ何が起きるかを添えた。**設計にある「回答フォームへ」「品もの」は出していない**——`MileageRewardKind` に無く、出すと**選べるように見えて保存できない**。 **「限りなし」と「品切れ」を混ぜない**：数の限りは空欄が限りなし、0 が品切れ。「空欄なら限りなし。0 と書くと品切れ（交換できません）」と欄に書いた。 **渡せなかったときの決めごと**（もう一度試す／マイルを返す／担当者が手で対応する）を画面から選べる。既定は「もう一度試す」——**黙って消えるより取り返しがつく**。 **公開中のものを直しても、その場ではお客様の見え方が変わらない**ことを帯で断る（保存は下書き、「出す」で公開）。 **残る差（P2）**：設計の「だれが交換できますか」の条件、「交換されたときにすること」の後続処理、右の「使い道のつくりかた」「つながる先」。交換後に渡すものは共通アクションの版を**文字で入れる**形で、選び口になっていない。 **設計 `p9CcEB` の文字が書き出されていない**ので、`qlVLJ` と Worker の契約を根拠にした（board#125）。',
    verdictSource: 'mileage-v6/p9CcEB.txt',
    verdictHead: 'e1aabda80',
  },
  {
