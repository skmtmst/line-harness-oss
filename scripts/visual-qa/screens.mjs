...
const FEATURE_18_AUDIT = {
  Q4bkTg: {
    route: '/inflow-links?tab=links',
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 流入元24本、今月312人（経路が分かる289人）、クリック8,420回、平均追加率6.4%を接続。タブ件数、フォルダ、検索・絞り込み、CSV、まとめて操作、一覧も設計の役割と順序にそろえた。1440px・1920pxとも横はみ出し0。',
  ...
  IhSBB: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 主欄＋右欄、貼るコード、受信確認、3ドメイン、不明ドメイン警告、貼り方・つながる先・注意を同時に表示した。1440px・1920pxとも横はみ出し0。',
  ...
  v0HaI: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 広告費・友だち単価・成果単価、Google・Meta・Yahoo!の接続状態、5件の広告内訳と未接続媒体の「つなぐ」を表示した。個人情報を送らない説明も維持。1440px・1920pxとも横はみ出し0。',
  ...
  TEVk8: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 入力、通常URL・短いURL・QR、追加時の動き、追加先を主欄へ、流れと注意を右欄へ設計順で配置した。入力済み状態を1440px・1920pxで撮影し、横はみ出し0。',
  ...
  JupxW: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 上部4指標、横方向の5段階、最初に見たページ・状態・成果・マイルを含む友だち表、右欄の流入元と追加時の動きを1画面にそろえた。1440px・1920pxとも横はみ出し0。',
  ...
  UIaM7: {
    ...
    gapNote: '',
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 対象名、使用中URL、現在の流入86人、分析と過去記録への影響を示し、受付停止・別リンクへ転送・削除の3択を確認窓へ配置した。通常・確認・失敗の全状態を2幅で撮影し、横はみ出し0。',
  ...
  BMmxU: {
    route: '/inflow-links?tab=links',
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 通常・読込中・0件・取得失敗で共通の外枠と操作位置を保ち、未取得を0と誤表示せず、次の操作を状態ごとに示した。全4状態の1440px・1920pxで横はみ出し0。',
  ...
  BuVDB: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 4媒体の接続状態・操作、クリック目印、成果名の5行対応表、返した件数、操作、送信履歴への導線を実データで表示した。1440px・1920pxとも横はみ出し0。',
  ...
  Im2b1: {
    ...
    verdict: 'match',
    verdictNote: '**2026-09-06 Issue #231 / UI HEAD 43b3aae50で設計画像と実装画像を同じ幅で並べて再確認し、一致。** 4指標、説明、検索・状態絞り込み、CSV、まとめて再試行、成功・待機・失敗・対象外を含む6行と行操作を表示した。1440px・1920pxとも横はみ出し0。',
