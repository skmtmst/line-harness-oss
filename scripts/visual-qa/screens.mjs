...
const FEATURE_17_REVIEW = {
  s98Vfw: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で再撮影。** 1440・1920とも横スクロール0。4つの指標、CSV、最終行動を追加した。残る差は、ランク・今月の増減・失効予定・ランク別絞り込みを返すAPIが無いこと。取得できない値は `— 未取得` と理由を表示する。',
    verdictSource: 'mileage-v6/s98Vfw.txt',
    verdictHead: 'TBD',
  },
  N46cQ: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で再撮影。** 1440・1920とも横スクロール0。指標、説明、検索、並び替え、CSVを設計順にそろえた。残る差は30日間の付与数と利用範囲を返すAPI、並び替え保存API。',
    verdictSource: 'mileage-v6/N46cQ.txt',
    verdictHead: 'TBD',
  },
  qlVLJ: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で通常・読込中・0件・取得失敗を再撮影。** 1440・1920とも横スクロール0。ランク3段と行ごとの確認・公開停止操作を追加した。残る差はランク人数・特典名・交換割合を返すAPIと、使い道作成画面 `p9CcEB` の取り込み待ち。',
    verdictSource: 'mileage-v6/qlVLJ-normal.txt + qlVLJ-error.txt',
    verdictHead: 'TBD',
  },
  MvZm5: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で再撮影。** 1440・1920とも横スクロール0。4つの指標、CSV、残高、操作者を追加し、撮影データも履歴として表示できる形へ正規化した。残る差は期間別の集計値と操作者を返すAPI。',
    verdictSource: 'mileage-v6/MvZm5.txt',
    verdictHead: 'TBD',
  },
  BmoGY: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で再撮影。** 1440・1920とも横スクロール0。設計の2列構成、LINEプレビュー、追従する状態と保存操作をそろえた。残る差は有効期限・予約取消・15軸条件を保存するAPIが無いこと。未接続項目は選べるように見せず明記した。',
    verdictSource: 'mileage-v6/BmoGY.txt',
    verdictHead: 'TBD',
  },
  HIU5O: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で再撮影。** 1440・1920とも横スクロール0。英語単位 `mile` を運用者向けの `マイル` に統一した。残る差はランク進捗・失効予定・つながる先・付与理由別集計を返すAPI。',
    verdictSource: 'mileage-v6/HIU5O.txt',
    verdictHead: 'TBD',
  },
  k8VCU: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で通常・読込中・0件・取得失敗を再撮影。** 1440・1920とも横スクロール0。各状態を混同せず、未取得と実値0を分けて表示する。残る差は N46cQ と同じ集計・利用範囲・並び替え保存API。',
    verdictSource: 'mileage-v6/k8VCU.txt + k8VCU-error.txt',
    verdictHead: 'TBD',
  },
  z3PB2: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で通常・読込中・0件・取得失敗を再撮影。** 1440・1920とも横スクロール0。撮影データの得点と最終行動を正しく読み、スコアルールへの入口を追加した。残る差は帯ごとの理由を返すAPIと設計見本と同量の固定データ。',
    verdictSource: 'mileage-v6/z3PB2.txt + z3PB2-error.txt',
    verdictHead: 'TBD',
  },
  s6MBc: {
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06、Issue #230 / PR #TBD / HEAD TBD で通常・読込中・0件・取得失敗を再撮影。** 1440・1920とも横スクロール0。パンくず、1人で試す入口、つながる先を追加した。残る差は、設計が読む画面と直す画面を分ける一方、実装は編集表を最初から出すことと、過去の版を返すAPIが無いこと。',
...
export const CAPTURED_AT = {
  ...
  17: [
    ...
    { pr: 0, head: 'TBD', on: '2026-09-06',
      screens: ['s98Vfw', 'N46cQ', 'qlVLJ', 'MvZm5', 'BmoGY', 'HIU5O', 'k8VCU', 'z3PB2', 's6MBc'],
      note: 'Issue #230 のS3第2段。Pencilの実ノードと同じ状態を1440・1920で横に並べて確認した。全対象で横スクロール0。絵は版に残さず `.txt` と判定注記を証拠にする。PR番号と撮影元HEADは採番後に更新する。' },
  ],
  7: [
