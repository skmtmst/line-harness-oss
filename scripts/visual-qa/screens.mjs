  ...
  */
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `ff26e0f1f` を3105/8792で再撮影。構造一致・顧客連携データ待ち。** 左にお客様・予約内容・要望・通知予定、右にLINEプレビュー・注意・関連先・空き再確認を配置し、実際の枠を使って10:00〜11:45の105分を表示した。1440px・1920pxとも横はみ出し0。友だち一覧APIは電話・ペット・店舗・来店回数を返さず、撮影時計では前日と2時間前の通知時刻が過去になるため、存在しない値や過ぎた送信予定を作らず一致扱いにしない。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `ff26e0f1f` を3105/8792で再撮影。構造一致・通知実績データ待ち。** PR #1088の固定応答で実際に登録完了まで進み、返された予約ID・カレンダー状態、お客様・日時・メニュー・担当・受付方法、LINE処理開始、同じ予約台帳への記録、次の操作、LINEプレビューと関連先を表示した。1440px・1920pxとも横はみ出し0。登録結果APIは送信・開封実績、登録者、予定通知の実行結果、成果計上結果を返さないため、送信済みと作らず一致扱いにしない。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `ff26e0f1f` を3105/8792で再撮影。構造一致・重複詳細データ待ち。** 実際のHTTP 409から赤い要約、保持した入力、選択日だけの空き候補、別担当の確認案内、注意、関連先を表示し、選び直した後は登録完了まで到達した。競合・回復を各1440px・1920pxで撮影し、全4枚とも横はみ出し0。エラーAPIは機械コードだけで、重複時間の範囲・件数・別担当の候補を返さないため、入力へ戻って確認と表示し一致扱いにしない。',
