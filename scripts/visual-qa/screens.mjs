...
export const SCREENS = [
  ...
  */
  ...
  },
  {
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06 Issue #237 head `1a0a71ba` で設計と再比較。** 届かなかった／送信対象外／メールで届いた／まだ連絡できていないの帯、名前・注文番号検索、状態絞り込み、その日のうちに代替連絡する案内を追加。再試行は安全なAPIが無いため出さず、理由も維持。1440・1920の通常・読込・空・失敗で横はみ出し0。メール結果・対応済みを返す口と撮影用の通常データが無く、設計の通常行を比較できないため要修正を維持。',
  ...
  },
  {
    ...
    */
    ...
    states: { apis: ['**/api/ec-commerce/notification-runs?**'], kinds: ['normal', 'loading', 'empty', 'error'] },
    verdict: 'needs_fix',
    verdictNote: '**2026-09-06 Issue #237 head `1a0a71ba` で設計と再比較。** 記録数／LINE API受付／押された／送れなかったの帯、名前・注文番号検索、すべて／クリック記録あり／送れなかったの絞り込みを追加。過去の所属不明行と個人既読は出さず、押下は自社短縮URLだけと明記。data-list-state で通常・読込・空・失敗・絞込0件を区別し、1440・1920で横はみ出し0。期間集計・CSV出力権限の口と撮影用の通常データが無いため、要修正を維持。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06 Issue #237 head `1a0a71ba` で設計と再比較。** 帯・作成導線・検索・絞り込み・表・CSV未接続の断りを維持。読込・通常・空・失敗・権限不足・絞込0件を data-list-state で名前付きにし、取得不能な数を0にしない。1440・1920の5状態、全12枚で横はみ出し0。送信処理と実行記録が未接続のため、構造一致・データ未接続を維持。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-06 Issue #237 head `1a0a71ba` で設計と再比較。** 「LINEログインを済ませた人にだけ届く」「担当が未設定だと届かない」「LINEにログインしていない人はメールへ切り替える」を先に明記。宛先・きっかけ・重要度・重複防止・注意・関連導線と、下書きだけ保存する安全な動きを1440・1920で確認し、横はみ出し0。スタッフ人数と送信プレビューは送信処理接続後のため、構造一致・データ未接続へ更新。',
