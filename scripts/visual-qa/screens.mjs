...
export const SCREENS = [
  ...
  */
  ...
  { ...INBOX, node: 'ASsb3', name: '2-13 保存した検索を開く', steps: [...OPEN_CHAT, { click: '保存した検索' }],
    ...
    verdictSource: 'inbox-v6/ASsb3.txt + inbox-v6/ASsb3-1440.png + inbox-v6/ASsb3-1920.png',
    verdictHead: '70fac89c4',
  },
  /*
  ...
  */
  ...
  { ...COMMON_VAR, node: 'gBtaK', name: '14-1-A 共通情報を編集', route: '/contents/vars/edit?id=common-var-delete-target', verdict: 'needs_fix', verdictNote: '**2026-09-04 撮り直して判定した（S3 第1段）。** 要修正。**台帳のルートが古い固定データのIDを指していて、前回は「この共通情報は見つかりませんでした」を撮っていた。**ルートを `?id=common-var-delete-target` に直して撮り直したところ、画面は出た。**#548 の影響確認は入っている**：「保存すると、◯◯ を差し込んでいる 2か所がすぐ変わります。」「送信済みの1か所は変わりません。すでに届いた文は書き換わりません。」——設計の言いたいことと同じ。残る差は **差し込みキーの生表記 `{{var.shop_hours}}`** と、設計が値の下に**常に**出す「保存すると、下の15か所すべてが すぐに変わります」。 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一。設計画像を撮り直した。**実装との突き合わせはこれから。** **P1 編集画面に「どこで使われているか」が1つも出ない。** ルート `/contents/vars/edit?id=cv-1`。出るのは 共通情報名・フォルダ・差し込み名・種別・値・更新スケジュール だけ。共通情報は**1か所直すと差し込んでいる全部の文が同時に変わる**ので、変える前に使用先が見えないと危ない。**ただし #548 で「保存」を押すと影響確認の面（`uNBlA`）へ進むようになった**ので、「どこが変わるか見えないまま保存する」状態ではなくなった。**良い点**：差し込み名に「あとから変えられません。変えるとテンプレートの差し込みが空になります。」と**理由まで**書く。種別は「※変更できません。」と断る。更新スケジュールに「過去の日時は指定できません。指定した時刻を過ぎると、自動で値が入れ替わります。」と書き、無いときは「スケジュールが設定されていません」と出す。**P2 残る差**：設計は値の下に「保存すると、下の15か所すべてが すぐに変わります」と**常に**出す。実装は保存を押してから出る。**推奨修正**：`uNBlA` と同じ口（使用先の数）を編集画面でも先に読んで、値の下に常に出す。取得元：`common-vars-v6/gBtaK.txt`。1440・1920とも横スクロール0', verdictSource: 'common-vars-v6/gBtaK.txt' , verdictHead: '31293424' },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-04 撮って判定した（1440・1920、はみ出し0）。要修正（P1）。** 変更前確認を #773 の口（`POST /api/common-vars/:id/impact-preview`）へつないだ（board#89 / PR #854）。前は使用先台帳しか読めず、「変更後の文と文字数の検査」は `—`＋未接続の理由だった。 **撮り方も直した**——前は「値を変えて保存を押す」手順で、押すと保存が走ってしまい狙った絵が撮れなかった。いまは**値を変えるだけ**で出る（400ms 待つ）。`id` も固定データにある `common-var-delete-target` にした（`cv-1` は無く、一覧の1件目に落ちていた）。 **4通りを書き分けている（設計より細かい。戻さない）**：(1) ふつうに変わる行は「保存後の文」と「文字数 15 / 5,000文字」。(2) **送信済みは変わらない**（「送信済みの1か所は変わりません。すでに届いた文は書き換わりません。」）。(3) 差し込みの目印を読み取れない行は **`—`＋理由**——空文字で埋めると「保存すると空になる」と読める。(4) 上限を超える行は赤で「5,010 / 5,000文字（上限を超えています。この通は送信のときに落ちます）」。 **止める理由があるあいだは保存させない。** 押せない理由はボタンの脇に言葉で出す。**読めていないときは止めない**——影響を確かめられなかったことを理由に保存を塞ぐと、口が落ちているあいだ誰も値を直せなくなる。 **設計との差**：**P1 帯4つが無い**（設計は 変わる場所 15か所（テンプレート12・回答フォーム3）／すぐ効くもの 4件（予約中の配信・公開中のフォーム）／文字数が上限を超えるもの 1件／送信済みの文 変わりません）。実装は同じことを**文章で**言っている（「3か所がすぐ変わります」「送信済みの1か所は変わりません」）。**数える経路はすべて口が返しているので、帯にするのは画面だけの差。** 種類ごとの内訳（テンプレート12・回答フォーム3）は `byKind` が返っている。**P1 「すぐ効くもの」の内訳が無い**——設計は「予約中の配信・公開中のフォーム」を分けて数え、**予約中の配信は保存した瞬間に文が変わって飛ぶ**ので、いちばん急ぐ数。いまは `status`（「使われています」）に混ざっていて拾えない。**P2 表になっていない**。設計は どこ／種類／いまの文／変わったあとの文／状態／操作 の6列。実装は行ごとの縦積みで、**15か所あると縦に長くなり見比べられない。** **P2 状態の書き方が粗い**。設計は「予約中 8/26 10:00」「本文が 66 / 60 字」「公開中」「下書き」と**いつ・どれだけ**まで出す。**P2 「この一覧をCSVで」が無い**（§7 #44 の置き場に合わせて足す）。**P2 行ごとの「操作」列が無い。** **良い点（設計に無いが残す）**：所属を確定できない回答フォームを**件数だけ**出し、名前と中身は出さない（「名前を確認できない使用先：回答フォーム1件（所属するLINEアカウントを確認できないため、名前と内容は表示しません）」）。「2026/09/04 10:00 時点で確かめました。」と**いつ確かめた数か**を書く。 内部語・壊れ値0件。 **ルート**：`/contents/vars/edit?id=common-var-delete-target`（値を変えると出る）。**取得元**：`common-vars-v6/uNBlA.txt` ＋ `uNBlA-1920.png`。**推奨修正**：**帯4つを先に足す**——数はすべて口が返しており、画面だけの差。とくに「すぐ効くもの（予約中の配信・公開中のフォーム）」は、**保存した瞬間に文が変わって飛ぶ**ぶんなので、いちばん急いで見せる。表への作り替えはそのあと。 **前の判定から引き継ぐ点**：この画面は「**分からないものを黙って落とさない**」書き方の見本で、束9のほかの「変える前・消す前」（`M9cij` `voJtX` `YfTfJ`）へ写す価値がある。所属を確定できない使用先を、**内容は見せずに件数だけ数に含める**——落とすと「影響なし」に見え、消してよいと読み違える。',
    verdictSource: 'common-vars-v6/uNBlA.txt + uNBlA-1920.png',
    verdictHead: '2b12d0fc',
  },
  {
  ...
  */
  ...
  { ...AFFILIATE, node: 'xqT1Z', name: '16-1-D アフィリエイターを登録する', route: '/affiliates/new', verdict: 'needs_fix', verdictNote: '**2026-09-04 撮り直して判定した（S3 第1段）。** 要修正。設計に在って実装に無いのは、紹介コードの但し書き「登録したあとは変えられません。半角の英数字と - _ だけ」、メール、**「LINEの友だちと結びつける（任意）」と候補**、結びつけの説明、「EC連携で売上が取れている案件だけ使えます」「この人のLINEに知らせる」。実装の「紹介コードを空欄のままにすると、保存したときに自動で決まります。決まる前のURLはコピーできません。」は**設計より親切なので残す**。 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一（マトリックス→対応表、ブレイクダウン→内訳）。設計画像を撮り直した。**実装との突き合わせはこれから。** **P2 アフィリエイターを追加する。#558 で報酬の決め方が直り、作りは設計に近い。** ルート `/affiliates/new`。段は 1 どなたを登録するか（名前・屋号／連絡先メール／紹介コード）→ 報酬の決め方。**#558 で「報酬が売上×率でしか出ない」が直った**——**成果1件ごとに定額／売上に対する割合／報酬なし（計測のみ）** から選べる。定額のときは「1件あたりの報酬」に「金額は案件ごとに決めます。」と添える。**良い点**：紹介コードに「空欄にすると、推測されにくいコードを自動で作ります（そのほうが、他の人にコードを当てられにくくなります）」と**なぜ自動が良いか**まで書く。連絡先メールに「報酬の確定連絡に使います。」と用途を書く。**P2 残る差**：設計 16-1-D は 支払い条件（締め日・支払サイクル・振込先）もこの段で決めるが、実装は名前・連絡先・コード・報酬まで。**振込先は `njLGA` の決めごと（銀行・支店・種別・末尾4桁だけ）に沿って足す。** 取得元：`affiliates-v6/xqT1Z.txt`。1440・1920とも横スクロール0 **推奨修正**：支払い条件（締め日・支払サイクル・振込先）をこの段に足す。**振込先は `njLGA` の決めごと（銀行・支店・種別・末尾4桁だけ）に沿う**——口座番号の全桁と名義は画面に出さない。', verdictSource: 'affiliates-v6/xqT1Z.txt' , verdictHead: '31293424' },
  {
    ...
    */
    ...
    /*
      2026-09-04 の再照合。上に残した第1段の記録は、固定データが入る前の
      経緯として残す。後勝ちのこの3項目が現在の判定である。
    */
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-04 S3 第2段で再照合。** 構造一致・データ未接続。`node scripts/visual-qa/compare-text.mjs --feature 16` と、1920px の設計本文・実装本文、既存の1440/1920確認記録を突き合わせた。画面にはクリック・友だち追加・成果・確定報酬、案件別・成果地点別・リンク別の内訳、支払いの取り決めがそろい、内部DB名 `ref_tracking` / `ref_code` は見出しに出ていない（`ref_code` の値は運用者が使うリンクコードとして表示）。一方、設計の「気になる点」、リンク別の友だち追加・成果、振込先、次の支払いは未接続。正本要件 §8〜§10・§19 でも支払台帳・銀行口座・次回支払いは未完了と明記されており、画面だけで値を作らない。既存確認では1440/1920とも横スクロール0。今回の再撮影は実行環境が Chromium の Mach port を拒否したため、新しい画像は作らず、既存本文と現在の実装・要件で判定した。**接続条件**：支払台帳・銀行口座・リンク別成果APIが入ったら、同じ2幅で撮り直して一致判定へ進める。',
    verdictSource: 'affiliates-v6/jwrbf.txt + apps/web/src/app/affiliates/affiliate-display.ts + docs/v6-requirements/v6-16-affiliate-requirements-draft.md',
    verdictHead: '933220a47',
  },
  ...
  */
  ...
  { ...MILEAGE, node: 'N46cQ', name: '17-1-A たまる決めごと', route: '/mileage?tab=earning-rules', verdict: 'structure_match_data_pending', verdictNote: '**2026-09-04 撮り直して判定した（S3 第1段）。** 構造一致・データ未接続。「まだ決めごとがありません」の空の絵。枠で見ると、帯（止めているもの2つ／この30日で付いたマイル）、札（すべて9／動いている7／止めている2／ほかの機能から呼ばれている3）、「決めごと 9つのうち 8つを表示」の件数表示、CSVが無い。 横断レビュー §7 の反映：#31 サイドメニューの選択状態／#40 主ボタンの緑を `$accent-deep` へ（白文字 2.26:1 → 5.44:1）／#44 CSV書き出しの置き場／#48 表記統一。設計画像は `docs/design-reference/mileage-v6/` を撮り直した。**実装との突き合わせはこれから。** **P2 たまる決めごと。作りは設計に近い。** ルート `/mileage?tab=earning-rules`。決めごとごとに 名前・説明・停止/再開・稼働（動いています／止めています）・付与数・上限（対象ごとに1回／1日1回まで／紹介された人1人につき1回）が並ぶ。**良い点**：「付与数を変更すると、変更後に発生した行動から新しい値が使われます。」と**いつから効くかを書く**（さかのぼらないことが分かる）。**撮った本文で付与数が空に見えるのは撮影の都合**——付与数は `<input type="number">`（`mileage/page.tsx:339`）で、本文の書き出しに入力欄の値は入らない。**実装の不具合ではない。** **P2 残る差**：設計は決めごとの並び替えと、効いている範囲（どのアカウント・どの友だち）をその場で出す。実装は一覧まで。取得元：`mileage-v6/N46cQ.txt`。1440・1920とも横スクロール0 **推奨修正**：決めごとの並び替えと、効いている範囲（どのアカウント・どの友だち）を出す。**「付与数を変更すると、変更後に発生した行動から新しい値が使われます。」の断りはそのまま残す**（さかのぼらないことが分かる）。', verdictSource: 'mileage-v6/N46cQ.txt' , verdictHead: '31293424' },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**2026-09-04 撮って判定した（1440・1920、通常/読込中/0件/取得失敗の5枚ずつ、はみ出し0）。要修正（P1）。** 使い道のタブは skmtmst/line-harness-oss#835 で入った。それまで `/mileage` のタブに `rewards` の鍵が無く、`?tab=rewards` は「友だちの残高」へ黙って落ちていた——**画面からは「無い」ことすら分からなかった。** **実装のほうが正しい点（設計へ戻さない）**：(1) 「1回も使っていない人」を**設計は 786人と数で出すが、口が返さない**（`getMileageRewardAdminOverview` は `neverRedeemedFriendCount` を固定で null）。**`—`＋理由**にしてある。0人と書くと「全員が使っている」に読め、**声をかける相手を取り違える。** (2) 設計は下書きを**「準備中 1つ」**と呼ぶが、実装は**「まだ出していないもの 1つ」**。下書きは「まだ出していない」であって、こちらの都合で作業中という意味ではない（`v6-common-rules` §5-5）。 **P1 設計のランクの帯3つが無い**（ブロンズ 0マイルから 886人／シルバー 2,000マイルから 312人／ゴールド 5,000マイルから 86人と、それぞれの特典）。**使い道はランクと対でしか意味を持たない**ので、次に足すならここ。 **P1 「使い道をつくる」を出していない。** つくる面（`p9CcEB` = `/mileage/rewards/edit`）が development に無く、押した先が無いボタンは行き止まりになるため。**S1 が PR #851 で作っている**ので、入り次第この面にボタンを足す（台帳 #135）。 **P1 「使い道を友だちに知らせる」「並び順を保存」が無い。** 前者は配信（`XQfMD`）へつなぐだけで済む。 **P1 行ごとの「中身を見る」「出す・止める」が無い。** 設計は各行に2つ置く。止める口（`POST /api/mileage/rewards/:id/stop`）は #772 で入っているので、画面だけの差。 **P2 帯の添え字が設計より薄い。** 設計は「58回。たまった分の 58%」「マイルを持っている人の 61%」「32回・16,000 マイル」と割合や内訳まで出すが、実装は回数だけ。**割合の分母を返す口が無い。** **P2 タブに件数が付かない**（設計は「友だちの残高 1,284／たまる決めごと 9／使い道 5」）。 **P2 交換すると渡るものの書き方が違う。** 設計は「クーポン「送料無料」」「タグ「先行案内」を付ける」と**中身の名前まで**出すが、実装は「クーポンを渡す」「タグを付ける」と種類だけ。`rewardKind` からは種類しか分からず、**渡すものの名前を返す口が無い。** **壊れ値0件・内部語0件。** `coupon` `early_access` などは日本語に言い換えてから出している。 **0件の絵で見つけて直したもの**：(1) 撮影ツールが**入れ子の中まで空にしていなかった**ので、表が空なのに帯だけ「4つ出しています」と出る、実際には起きない絵が撮れていた（`capture-screens.mjs` の `emptyLike`）。(2) **交換0回なのに「いちばん使われた 送料無料」と名指ししていた。** 名前を出すと「これがよく使われている」と読め、**伸ばす先を取り違える。** 0回のときは `—`＋「まだ交換されていません」にした。 **ルート**：`/mileage?tab=rewards`。**取得元**：`mileage-v6/qlVLJ-normal-1920.png` ＋ `qlVLJ.txt`。**推奨修正**：ランクの帯3つが先。使い道はランクと対で意味を持つ。**未取得と0の分け方は設計より正しいので変えない。**',
    verdictSource: 'mileage-v6/qlVLJ-normal-1920.png + qlVLJ.txt',
    verdictHead: '81a01047',
  },
  {
    /*
 ...
 * 8788/8790 は他レーンが使用中だったため触れていない。
 */
const ISSUE_210_REVIEW = {
  xGLVe: '一覧・会話・顧客情報の列幅、件数、情報量が設計と異なる。**推奨修正**：3列の幅と表示項目を設計へそろえ、同じ固定データで再撮影する。',
  NfgOs: 'テンプレート選択窓の大きさ、フォルダ、一覧、プレビューの配置が設計と異なる。**推奨修正**：窓の寸法と左右構成を設計へそろえる。',
  H3lAOB: '右パネル非表示の状態は撮れたが、一覧幅・会話ヘッダー・件数と行データが設計と異なる。**推奨修正**：右パネルを閉じた際の2列幅と会話上部の操作をそろえる。',
  Xi4x9: '設計は6項目の小さな表示設定ポップオーバーだが、実装は9項目の右パネル全幅表示。**推奨修正**：項目数、寸法、表示位置を設計へそろえる。',
  f0zn6: '未読の強調位置と件数、一覧行、会話側の情報が設計と異なる。**推奨修正**：未読バッジと担当者別件数を同じ固定データ・位置で表示する。',
  NWbuF: 'フォルダ展開はできるが、窓の寸法、フォルダ名・件数、テンプレート内容が設計と異なる。**推奨修正**：全フォルダ展開時の階層と左右幅をそろえる。',
  B7CER8: '内部メモ窓の幅・高さ・位置と操作ボタンの並びが設計と異なる。**推奨修正**：設計のダイアログ寸法とフッター配置へそろえる。',
  IYjvu: '対応状況の選択肢は開くが、位置・寸法・背景の会話状態が設計と異なる。**推奨修正**：同じ会話を選択した状態でドロップダウンの寸法と位置をそろえる。',
  TUveA: '「フォルダ 予約」が0件で、1440/1920pxとも撮影操作に失敗した。**推奨修正**：予約フォルダを固定データへ戻し、実際に開ける押し口で再撮影する。',
  w72a2: '設計は会話上の小さな絞り込み面だが、実装は右側の全高ドロワーで、背景の選択会話も異なる。**推奨修正**：寸法・位置をそろえ、会話選択後に開く撮影手順へ直す。',
  ANgda: '設計の条件カードと「よく使うに追加」が無く、実装は空欄エラーの小さな保存窓になっている。**推奨修正**：保存対象の条件要約と切替を窓内に表示する。',
  tBlkL: '設計は保存完了トーストと保存済み検索一覧だが、実装は重複名エラー窓のまま。**推奨修正**：重複しない固定名で保存を完了させ、完了状態を撮る。',
  LHjwD: '重複エラーは出るが、設計にある詳細条件と「よく使うに追加」が実装窓に無い。**推奨修正**：入力エラーを残しつつ条件構成と切替を設計へそろえる。',

  PhxG6: '一覧の骨組みは近いが、行データ、列幅、操作位置、ページ送りが設計と一致しない。**推奨修正**：設計と同じ固定データで列幅・行高・操作を詰める。',
  LT8RS: '設計は表示件数メニューが開いているが、実装画像は通常一覧のまま。**推奨修正**：表示件数を開く撮影手順または安定した押し口を追加する。',
  Igi72: '詳細検索の構造は近いが、14軸の入力内容、候補チップ、人数、窓の高さが設計と異なる。**推奨修正**：14軸すべての固定候補と選択状態をそろえる。',
  IAf7j: '設計は専用の一括操作画面だが、実装は一覧上の小さな確認窓で、選択操作もクリック失敗した。**推奨修正**：一括操作の選択・実行内容・対象一覧を設計の全画面構成へそろえ、押し口を安定させる。',
  I6UAdr: '設計の顧客情報・配信状況・履歴を横断する詳細画面に対し、実装は旧構成のマイル中心画面。**推奨修正**：設計の左プロフィール＋中央履歴＋右判断情報の構成へ組み直す。',
  bzDn6: '読込・空・失敗は個別に撮れたが、設計の3状態を同一面で示す内容・余白・案内と一致しない。**推奨修正**：各状態の文言と次の行動を設計へそろえる。',
  YzxU1: '設計の重複候補一覧・判断操作が実装に無く、集計と対応表だけになっている。**推奨修正**：候補行と詳細確認導線を追加する。',
  InCDe: '候補2件と判定根拠はあるが、採用値の比較表と除外・保留・統合操作が設計より不足する。**推奨修正**：項目ごとの採用値選択と3つの判断操作をそろえる。',
  r7eSi: '統合ユーザー一覧の骨組みは近いが、帯の意味、絞り込み、列、行数、主操作が設計と異なる。**推奨修正**：設計の4指標と検索・CSV・行操作をそろえる。',
  w8W4Eh: '詳細のカード構成は近いが、プロフィール編集、解除操作、属性・履歴の項目と値が設計と異なる。**推奨修正**：上段3カードと下段2領域の情報・操作を設計へそろえる。',

  hqrOv: '一覧の骨組みは近いが、フォルダ幅、指標値、行数、列幅、ページ送りが設計と異なる。**推奨修正**：同じ固定データで一覧密度と列構成をそろえる。',
  dKlkz: '削除確認窓は表示できるが、対象情報、警告文、余白、ボタン配置が設計と異なる。**推奨修正**：影響情報と警告の順序を設計へそろえる。',
  H374MR: '設計のCSV一括登録窓が実装画像に開いていない。**推奨修正**：CSV操作の安定した押し口を用意し、窓を開いた同一状態で再撮影する。',
  sfTEW: '設計のdry-run確認窓が実装画像に開いていない。**推奨修正**：取込前の件数・差分・エラー表を表示する状態まで撮影手順を進める。',
  op1rh: '設計の取込完了窓が実装画像に開いていない。**推奨修正**：完了結果と一覧へ戻る導線が出る固定応答・撮影手順を用意する。',
  QzRsJ: '設計の一部失敗窓が実装画像に開いていない。**推奨修正**：成功・失敗件数と失敗行を表示する固定応答・撮影手順を用意する。',
  HBTk0: '一覧構造は近いが、指標値、項目、利用数、表の情報密度が設計と異なる。**推奨修正**：設計の固定件数と列をそろえる。',
  yKEdO: '読込・空・失敗は撮れたが、設計の案内文・操作と同じ状態構成になっていない。**推奨修正**：状態ごとの文言、再試行、作成導線を設計へそろえる。',
  rIhbN: '一覧の骨組みは近いが、4指標、人数、ルール内容、操作が設計と異なる。**推奨修正**：人数を返す口を接続し、設計の行情報を表示する。',
  QKx8Q: '設計は保存済み検索5件の表だが、実装は0件の空状態。**推奨修正**：固定データを接続し、条件・人数・更新者・操作を表示する。',
  l25rlp: '初期画面の段組みは近いが、説明、選択肢、右側の要約、下部操作位置が設計と異なる。**推奨修正**：3段の入力と右側説明を設計へそろえる。',
  tP0RW: '連動ONは撮れたが、条件・アクションの行構成、右側要約、画面高が設計と異なる。**推奨修正**：連動設定の行と説明を設計順にそろえる。',
  LfrQs: '追加ドロワーは開くが、種類一覧、条件入力、幅、フッター位置が設計と異なる。**推奨修正**：ドロワーの選択肢と段構成を設計へそろえる。',
  ee0sk: '既存設定は表示できるが、カード数、補足、連動アクション、遡及設定が設計と異なる。**推奨修正**：設計の補足と既存ルール一覧をそろえる。',
  VjXGX: '遡及確認窓は表示できるが、件数、警告、選択肢、ボタン配置が設計と異なる。**推奨修正**：対象人数と戻せない影響を設計順に示す。',
  A1ZYeP: '基本構成は近いが、項目種類、差し込み名、既定値、右側の使われ方が設計と異なる。**推奨修正**：不足する日時・画像・PDF型をAPIと合わせて追加し、入力順をそろえる。',
  KoT6c: '移行元・先の2列構成は近いが、種類、影響確認、件数、下部操作が設計と異なる。**推奨修正**：不足型と事前確認件数を接続して設計へそろえる。',
  zGZMA: '「対応中を保管」が0件で、1440/1920pxとも撮影操作に失敗した。**推奨修正**：削除と保管の正本を確定し、対象人数と移行先を返す口を接続して再撮影する。',
  XBkiQ: '設計の条件編集画面に対し、実装は「保存した検索が見つかりません」の空表示。**推奨修正**：固定IDを接続し、入れ子のAND/OR条件と人数確認を表示する。',

  TC1b1: '一覧の骨組みは近いが、フォルダ、列幅、行データ、操作、余白が設計と異なる。**推奨修正**：同じ固定データで一覧密度と列をそろえる。',
  cCB7r: '配信方式2択の構造は近いが、見出し、段階表示、カード寸法、説明と操作位置が設計と異なる。**推奨修正**：作成ウィザードの上部と2カードを設計へそろえる。',
  kk8dz: '設計のLINEプレビュー付き1通目設定に対し、実装は入力欄と右側要約が簡略化されている。**推奨修正**：プレビュー、配信時刻、対象、送信後設定を同じ配置にする。',
  bV5Vs: '設計のステップ表中心の編集画面に対し、実装は指標カードと小さな一覧の別構成。**推奨修正**：編集時の状態カードとステップ表の列・操作を設計へそろえる。',
  xfYLn: '設計の個別ステップ編集に対し、実装はシナリオ全体の編集面が残っている。**推奨修正**：対象ステップだけを編集する画面・プレビュー・送信後設定を表示する。',
  r6Gzsu: '配信条件窓は開くが、設計のAND/OR条件編集に対し実装は小さな絞り込み窓。**推奨修正**：条件行、対象人数、除外条件、フッターを設計へそろえる。',
  hz9ti: '送信後アクション窓は開くが、設計の連鎖アクション一覧・条件・順序が不足する。**推奨修正**：複数アクションと実行条件を設計の段構成で表示する。',
  dqFft: '削除確認窓は表示できるが、対象ステップの情報量、警告、寸法、背景状態が設計と異なる。**推奨修正**：同じステップ編集背景で影響と取り消し不可をそろえる。',
  EvVO5: '設計は開始条件の大きなオーバーレイだが、実装は通常フォームへ遷移している。**推奨修正**：開始条件を編集画面上の同一オーバーレイで扱う。',
  RUxNf: '開始確認窓は出るが、設計の配信概要・確認項目・警告・ボタン配置が不足する。**推奨修正**：対象、開始条件、終了条件、戻せない影響を設計順に表示する。',
  NrBkW: '設計は開始完了後の一覧と完了案内だが、実装は編集画面の保存完了状態。**推奨修正**：配信開始後の状態・次の行動を設計どおり表示する。',
  g2UNV: '設計はテスト送信確認の小窓だが、実装は宛先一覧を持つ大きな送信窓。**推奨修正**：対象選択、確認内容、窓寸法を設計へそろえる。',
  M2b2B: '結果画面の骨組みは近いが、配信指標、ステップ別反応、プレビュー、状態別表示が設計と異なる。**推奨修正**：通常・読込・失敗の各状態で同じ指標構成を保つ。',
...
for (const screen of SCREENS) {
  ...
  const issue210Note = ISSUE_210_REVIEW[screen.node]
  if ([2, 3, 4, 5].includes(screen.feature) && issue210Note) {
