...
export const SCREENS = [
  ...
  */
  ...
  },
  {
    ...
    */
    ...
    verdict: 'needs_fix',
    verdictNote: '**#510 で予約完了の画面が入り、未実装ではなくなった。** 見込み人数の注記は設計より丁寧で、「配信対象は、送信を始める直前に同じ条件でもう一度数えます。上の人数は現在の見込みなので、友だちやタグの変化によって予約時刻までに増減します。」と、その数が何なのかを言う。除外見込みが「—人」なのも正しい。P1 設計の「次にできること」4つ（配信内容を編集／テスト送信する／複製して別配信を作る／**予約を取り消す**）が無い。とくに予約を取り消す導線が無いのは重い。P2 STEP1〜5の進み表示が無い。「配信開始・完了・エラーはSlackの同じスレッドへ通知します。」の案内が無い。予約した内容に「状態 予約中」の行が無い',
    verdictSource: 'broadcasts-v6/bPF0s-1920.png',
    verdictHead: 'ab1841bc',
  },
  ...
  */
  ...
  },
  {
    ...
    gap: 'api',
    gapNote: '設定だけを画面側で計算せず、`POST /api/reminders/{id}/preview` で各通の予定日時、過去で作られない通、条件で外れる通、取消時に止まる通を返す必要がある',
    status: 'unimplemented',
    why: '現行APIにpreviewが無い。ブラウザだけで同じ時刻計算を再実装すると、実行Workerと食い違って「画面では届くのに送られない」が起きるため、通常実装だけでは完成しない',
  },
  {
    ...REMINDER, node: 'W98zZQ', name: '7-1-E テスト送信確認', route: '/reminders/edit?id=reminder-3',
    gap: 'api',
    gapNote: 'リマインダ専用のdry-run／テスト送信APIが要る。実登録、予定、送信後アクション、履歴、回数制限を更新せずにプレビューと送信結果を返す',
    status: 'unimplemented',
    why: '一斉配信・シナリオのテスト口は別の保存形式と実行経路で、リマインダの基準日・版・予定時刻を検証できない。現行reminders routeにテスト／dry-run APIは無い',
  },
  {
    ...REMINDER, node: 's6Vvp', name: '7-1-F 最終確認', route: '/reminders/new',
    gap: 'api',
    gapNote: '公開版、`validate`、`preview`、`publish` が要る。基準日・通・テスト・対象・終了条件をサーバーで確認し、公開する版を固定してから有効化する',
    status: 'unimplemented', why: '現行 `/reminders/new` は「作成→必要なら停止→1通目追加」を順に実行し、確認前に有効な定義が作られる。設計の有効化前チェック、対象人数、予定通数、テスト済み、版固定を保証する口が無いので `ConfirmDialog` だけでは作れない',
  },
  {
    ...REMINDER, node: 'PSmHo', name: '7-1-G 有効化完了', route: '/reminders/new',
    gap: 'api',
    gapNote: '`publish` の返事（公開版、対象、予定件数、最初の実行、監視先）を受けて完了画面を出す。最終確認のAPI契約と同時に作る',
    status: 'unimplemented', why: '現行create/updateは公開版や公開結果を返さず、完了画面へ出す確定値が無い。7-1-Fのvalidate/publish実装が前提',
  },
  {
    /*
