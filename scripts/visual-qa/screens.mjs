...
export const SCREENS = [
  ...
  */
  ...
  { ...BOOKING_SET, node: 'tksPc',
    ...
    }, name: '28-1-A 受付枠と休業日', route: '/booking/staff/shifts', verdict: 'match', verdictNote: '**2026-09-07 Issue #433 で実値を再判定。** #1198/#1188の固定応答を読み、曜日ごとの上限（3/3/休/3/4/2/2件）、顧客向け○・△・×・休、時間帯の「残り1/2・0/2」、休業日、設備A/Bを画面で確認した。通常・読込中・失敗を1440/1920で再撮影し、全8枚で横はみ出し0。', verdictSource: 'booking-settings-v6/tksPc-{normal,loading,error}.txt + tksPc-{normal,loading,error}-{1440,1920}.png', verdictHead: '58989eeaa' },
  { ...BOOKING_SET, node: 'GhOb3', name: '28-1-B 予約メニューをつくる', route: '/booking/menus/new',
    ...
    verdict: 'match', verdictNote: '**2026-09-07 Issue #433 で入力例を入力後に再判定。** #1184の`variant="v6"`骨格で番号付き5節、右側プレビュー・注意、下部固定アクションを確認した。撮影時にメニュー名「トリミング（小型犬）」、105分、¥8,400、分類「トリミング」、説明文を実際に入力し、1440/1920の変種画像で一致・横はみ出し0を確認した。', verdictSource: 'booking-settings-v6/GhOb3.txt + GhOb3-pencil-input-{1440,1920}.png + 2026-09-07 visual comparison', verdictHead: '58989eeaa' },
  {
...
const FEATURE_17_REVIEW = {
  ...
  N46cQ: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #429 / UI HEAD 65390c132 で再判定。** 並び替えと `sortOrder` の保存、公開版の中身を見る導線、V6下書きの利用対象条件を実装し、3102/8789で1440/1920pxを撮影した。両幅とも横はみ出し0。設計の操作と一覧構造はそろった。**残る口**：現在の `/api/mileage/earning-rules` は行別の付与回数だけで、設計上段の「この30日で付いたマイル」「1人あたり平均」を返さない。Issue #418がこの集計契約を実装中のため、値を作らず `structure_match_data_pending` とする。',
  ...
  BmoGY: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #429 / UI HEAD 65390c132 で再判定。** 一斉配信と同じ15軸の条件ビルダーを接続し、有効期限・取消時の減算・LINEプレビューとともにV6下書きへ保存する。3102/8789で1440/1920pxを撮影し、両幅とも横はみ出し0。**残る口**：取消でマイルを引いたときの友だち向け自動通知はIssue #418で実装中であり、現画面も未接続と明記している。条件や通知結果を偽らず、通知口の統合まで `structure_match_data_pending` とする。',
  ...
  k8VCU: {
    verdict: 'match',
    verdictNote: '**2026-09-07 Issue #429 / UI HEAD 65390c132 で一致。** N46cQと同じ一覧に、通常・読込中・0件・取得失敗を分けて実装した。3102/8789で全4状態を1440/1920px撮影し、状態名、案内、再読込導線、件数表示をPencil正本と目視比較した。全画像で横はみ出し0。並び順保存・公開版確認・利用対象条件も通常状態から到達できる。',
