...
export const SCREENS = [
  ...
  */
    ...
    verdictSource: 'analytics-v6/QQ1SR.txt + QQ1SR-loading.txt + QQ1SR-empty.txt + QQ1SR-error.txt + 1440/1920px screenshots', verdictHead: 'd7fe26794' },
  {
    ...
    verdict: 'unjudged',
    verdictNote: '**設計画像なし。** `analytics-v6/URqOA.txt` を正本として、入れる内容、送る間隔・曜日・時刻・期間、ログインユーザーとメールの宛先、LINE要約、変化通知、右側の到着見本・参照元・接続先・注意、下部の3操作があることを照合する。APIは設定・実行時スナップショット・配信結果を保存し、通常・読込・空・失敗を別表示する。撮影後に最終判定する。',
    verdictSource: 'analytics-v6/URqOA.txt',
  },
