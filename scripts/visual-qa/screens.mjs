...
export const SCREENS = [
  ...
  */
  ...
  },
  {
    ...
    verdict: 'needs_fix', verdictNote: 'P1 一覧の状態に共通部品（ListState）を使っておらず、読込・空・失敗の言い分けが設計とそろわない。ほかの機能は同じ部品へ寄せている **ルート**：`/inflow-links?tab=links`（空・読込・失敗）。**取得元**：`inflow-v6/design-qa.md`。**推奨修正**：**共通部品（`ListState`）へ寄せるだけ**。`bzDn6` `q5G45` `zCQXe` `Vdbv5` `f8SBSh` はすべて同じ部品で束1・束4を満たしているので、**同じ形に替えれば済む**。新しい口は要らない。', verdictSource: 'inflow-v6/design-qa.md',
