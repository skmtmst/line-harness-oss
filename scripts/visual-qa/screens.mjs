  ...
  */
  ...
  },
  {
    ...
    */
    ...
    verdict: 'match', verdictNote: '**2026-09-07 PR #1041（head `16e2331cb`）で再実装・再撮影し一致。** Pencil実ノード `s6MBc` と `/mileage/score-rules` を同じ1920px入力で左右に並べた。点を付ける7行、編集・削除・追加、帯の境目、マイルとの違い、つながる先、版状態と固定フッターを照合した。通常・読込・0件・失敗の4状態を1440・1920で撮り、10枚すべて横スクロール0。設計例と固定データの点数差は構造差ではない。過去を再計算しない安全な契約と「スコアのルールを公開」の文言は現行V6要件を優先した。取得元：`mileage-v6/s6MBc.txt`。', verdictSource: 'mileage-v6/s6MBc.txt', verdictHead: '16e2331cb',
    states: {
  ...
  */
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `168a3f23b` を3105/8792で再撮影。構造一致・顧客連携データ待ち。** 左にお客様・予約内容・要望・通知予定、右にLINEプレビュー・注意・関連先・空き再確認を配置し、実際の枠を使って10:00〜11:45の105分を表示した。1440px・1920pxとも横はみ出し0。友だち一覧APIは電話・ペット・店舗・来店回数を返さず、撮影時計では前日と2時間前の通知時刻が過去になるため、存在しない値や過ぎた送信予定を作らず一致扱いにしない。',
  ...
  },
  {
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `168a3f23b` を3105/8792で再撮影。構造一致・通知実績データ待ち。** PR #1088の固定応答で実際に登録完了まで進み、返された予約ID・カレンダー状態、お客様・日時・メニュー・担当・受付方法、LINE処理開始、同じ予約台帳への記録、次の操作、LINEプレビューと関連先を表示した。1440px・1920pxとも横はみ出し0。登録結果APIは送信・開封実績、登録者、予定通知の実行結果、成果計上結果を返さないため、送信済みと作らず一致扱いにしない。',
  ...
  },
  {
    ...
    */
    ...
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07 Issue #304 / PR #TBD / UI HEAD `168a3f23b` を3105/8792で再撮影。構造一致・重複詳細データ待ち。** 実際のHTTP 409から赤い要約、保持した入力、選択日だけの空き候補、別担当の確認案内、注意、関連先を表示し、選び直した後は登録完了まで到達した。競合・回復を各1440px・1920pxで撮影し、全4枚とも横はみ出し0。エラーAPIは機械コードだけで、重複時間の範囲・件数・別担当の候補を返さないため、入力へ戻って確認と表示し一致扱いにしない。',
...
const FEATURE_17_REVIEW = {
  s98Vfw: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07、Issue #230 / PR #1041 / HEAD 16e2331cb で再実装・再撮影。** Pencil実ノード `s98Vfw` と実装を同じ1920px入力で左右に並べ、1440pxも確認した。4つの集計帯、案内、検索・絞り込み札、固定幅の一覧、明細と手動調整、件数・ページ送りをV6構造へ揃え、横スクロール0。残る未接続はランク、今月の増減、失効予定のAPI値。設計サンプル値を作らず `— 未取得` と理由を表示する。',
...
const ISSUE_297_REVIEW = {
  ...
  dKlkz: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・本番API待ち。固定一覧にある「NEN会員」で削除窓を開き、削除対象、5行の影響、取り消せない注意、確認入力、取消・削除を設計順に表示した。128人とマイル値は確認済み。固定一覧は設計名「NEN会員（定期）」ではなく「NEN会員」を返し、本番のタグ一覧も連動アクションと自動参照先の内訳をまだ返さないため、名称と内訳を未照合とした。2幅とも横はみ出し0。',
  },
  H374MR: {
    verdict: 'match',
  ...
  HBTk0: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・本番API待ち。4指標、検索・種類絞り込み、項目・差し込み名・種類・入力済み・フォーム・表示先・操作の一覧、安全確認を設計順に表示した。固定データの4行から利用人数、回答フォーム数、表示先を読み、設計値で撮影済み。本番APIは回答フォーム数と表示先をまだ省略できる契約のため、その場合は「—」へ安全に戻す。2幅とも横はみ出し0。',
  },
  yKEdO: {
    verdict: 'match',
  ...
  rIhbN: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・本番API待ち。4指標、検索・利用状態、順番・マーク・人数・初期値・自動変更・使用先・操作、安全確認を設計順に表示した。固定データの4行から23人・19人・186人・3人と自動変更名を読み、設計値で撮影済み。本番APIは画面ごとの表示先を返さず、現在は削除判定に使う参照先を表示しているため、その列だけ未照合。2幅とも横はみ出し0。',
  },
  QKx8Q: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・集計API待ち。カード表示を設計の一覧表へ戻し、条件名・要約・該当人数・共有・使用先・作成者日時・操作の7列、3絞り込み、保存条件からコピーを表示した。固定データの5行と今月84回を実値から表示済み。設計の総数12件・配信使用中5件を返す一覧総数の口がまだ無く、取得した5行からは5件・2件になるため、2指標だけ未照合。2幅とも横はみ出し0。',
  },
  l25rlp: {
    verdict: 'match',
  ...
  ee0sk: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・保存API待ち。既存タグの4区画、右側要約、マイル・倍率、3つの連動アクション、遡及設定、保存操作を設計値で表示した。遡及反映は事故を避けるため初期OFFを維持し、設計のON状態は保存確認時だけ再現する。本番APIは連動アクションの保存先をまだ持たないため、その永続化だけ未照合。2幅とも横はみ出し0。',
  },
  VjXGX: {
    verdict: 'match',
    note: '一致。設計と同じNEN会員（定期）の対象128人、紹介者34人、本人1,280mile、紹介者170mile、合計1,450mile、倍率、アクション、取り消せない注意、確認チェック、2つの保存方法を表示した。固定データ取り込み後に2幅で再比較し、横はみ出し0。',
  },
  byqIW: {
    verdict: 'structure_match_data_pending',
  ...
  zGZMA: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・本番API待ち。固定データの「保留」3人で窓を開き、対象マーク、影響人数、初期値「未対応」への置換、履歴保持、取消・保管を表示した。設計の物理削除ではなく要件どおり保管する安全側の差を維持するため、その操作差だけ未照合。2幅とも横はみ出し0。',
  },
  XBkiQ: {
    verdict: 'structure_match_data_pending',
    note: '構造一致・内訳API待ち。固定ID ss-1 を読み、名前・説明・共有範囲、AND・OR条件、右側の該当18人・使用先・一覧表示、削除・取消・保存を設計値で表示した。人数は一覧で取得済みのmatchCountを初期表示へ使うよう直した。本番APIはLINE15人・MAIL3人のチャネル内訳を返さないため、その1行だけ未照合。2幅とも横はみ出し0。',
  },
}
