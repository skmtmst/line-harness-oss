...
const FEATURE_15_REVIEW = {
  g89Tc: {
    verdict: 'match',
    verdictNote: '**2026-09-07、Issue #392 / HEAD `0cc67ed91d` を固定ポート web 3105・mock 8792 で1440px・1920px再撮影し、一致。** 2幅とも横はみ出し0。容量APIの実値から2.4GB / 10GB・残り7.6GB・24%帯を表示し、「上限に近い」もファイル種別ごとのLINE上限80%以上だけを絞る。3フォルダ＋未分類、検索、6つの絞り込み、格子/一覧、並び順、表示件数、カード、一括削除を設計画像と目視比較した。固定データは代表10件のため、設計の186件は作り物で埋めない。',
    verdictSource: 'media-v6/g89Tc.txt + g89Tc-1440.png + g89Tc-1920.png',
    verdictHead: '0cc67ed91d',
  },
  voJtX: {
    verdict: 'match',
    verdictNote: '**2026-09-07、Issue #392 / HEAD `0cc67ed91d` を1440px・1920px再撮影し、一致。** 2幅とも横はみ出し0。大きなプレビュー、ファイル情報、名前付き使用先3件、ダウンロード、同じ種類のファイル選択、送信進捗、安全確認、変更理由、新版追加を設計と同じ全面詳細で表示する。新版は名前と管理用URLを保ち、現在の固定版を勝手に切り替えないことも明示した。署名URLへの直接PUT、完了確認、差し替え下見、版追加の実API契約を接続し、契約テストで確認した。',
    verdictSource: 'media-v6/voJtX.txt + voJtX-1440.png + voJtX-1920.png',
    verdictHead: '0cc67ed91d',
  },
  eXAJP: {
    verdict: 'structure_match_data_pending',
    verdictNote: '**2026-09-07、Issue #392 / HEAD `0cc67ed91d` を1440px・1920px再撮影し、構造一致・ファイル投入状態のみ画像未確認。** 2幅とも横はみ出し0。設計どおり全面の窓、20件選択、画像10MB・音声/動画200MB・PDF20MB、フォルダ、選択件数、固定操作欄を表示する。実装は署名URLへ直接PUTし、ファイル別の準備中・送信率・確認中・完了・失敗・1件再試行を持ち、25件の契約テストで上限と経路を確認した。**残る理由:** 現行の撮影器はローカルファイル投入を表現できず、設計画像の「完了2件・上限超過1件」を同じ画像に作れない。初期状態の構造と2幅は目視済み。',
    verdictSource: 'media-v6/eXAJP.txt + eXAJP-1440.png + eXAJP-1920.png',
    verdictHead: '0cc67ed91d',
  },
  YfTfJ: {
    verdict: 'match',
    verdictNote: '**2026-09-07、Issue #392 / HEAD `0cc67ed91d` で使用中・削除可能を1440px・1920px再撮影し、一致。** 4枚とも横はみ出し0。使用中は3か所の種類・名前・安全な導線を出して削除を止め、別メディアへの一括差し替えを案内する。未使用は「どこでも使っていません」、元に戻せない説明、削除操作を表示する。影響確認と差し替え実行は版番号つき実API契約へ接続済みで、409時は影響を読み直す。削除可能状態の撮影対象も固定データの未使用PDFへ修正した。',
...
export const CAPTURED_AT = {
  ...
  15: [
    { pr: 0, head: '0cc67ed91d', on: '2026-09-07', screens: ['g89Tc', 'voJtX', 'eXAJP', 'YfTfJ'], note: 'Issue #392。統合済みの容量・直接アップロード・版追加・使用先差し替え契約へ接続し、固定ポート3105/8792で1440px・1920pxを再撮影。対象10枚は横はみ出し0。一覧・詳細・削除確認を一致へ更新し、ファイル投入を撮影器で表現できない登録窓だけ理由付き構造一致を維持した。' },
