...
function bodyFor(pathname, query = new URLSearchParams()) {
  ...
  */
  ...
  /*
    支払い（`njLGA`）。**Codexが実装中で、この口は「想定」。**
    PRのheadが届いたら実装のコードで確かめる。**推測を正本にしない。**
  */
  if (pathname === '/api/affiliate-payments') return { success: true, data: AFFILIATE_PAYMENTS }
