...
function petsToCsv(items: NenPetRow[]): string {
  const header = ['ペット名', '種類', '品種', '誕生日', '年齢', '体重(kg)', '避妊去勢', '運動量', '主食', '1日の目安(g)', '1日の必要カロリー(kcal)', '体重の更新日', '飼い主', 'EC会員ID']
  const lines = items.map((p) => [
    p.name, p.animalType === 'cat' ? '猫' : '犬', p.breed, p.birthday ?? '', p.ageLabel, p.weightKg ?? '', NEUTERED_LABEL[p.neutered], p.activityLabel,
    p.productName ?? '', p.feeding?.dailyGrams ?? '', p.feeding?.dailyKcal ?? '', p.updatedAt.slice(0, 10), p.owner.name, p.owner.customerId ?? '',
  ].map(csvCell).join(','))
