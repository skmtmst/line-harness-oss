...
export default function SupportInboxPage() {
  ...
  const counts = useMemo(() => ({
    total: items.length,
    line: items.filter((item) => item.channel === 'line').length,
    email: items.filter((item) => item.channel === 'email').length,
    urgent: items.filter((item) => Date.now() - new Date(item.lastIncomingAt).getTime() >= 30 * 60_000).length,
  }), [items])
  ...
  return (
    <div className="space-y-5">
      <Header title="お問い合わせ" description="LINEとメールを一つの受信箱で即時対応" />

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <SummaryCard label="未対応" value={counts.total} accent="rose" />
        <SummaryCard label="LINE" value={counts.line} accent="green" />
        <SummaryCard label="メール" value={counts.email} accent="sky" />
        <SummaryCard label="30分以上待ち" value={counts.urgent} accent="amber" />
      </div>

      {error && <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">{error}</div>}

      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:grid lg:h-[calc(100vh-260px)] lg:min-h-[620px] lg:grid-cols-[390px_1fr]">
        <aside className="border-b border-gray-200 lg:border-b-0 lg:border-r">
          <div className="space-y-3 border-b border-gray-200 bg-gray-50/70 p-4">
            <div className="flex gap-2">
              {(['all', 'line', 'email'] as Channel[]).map((value) => (
                <button key={value} onClick={() => setChannel(value)} className={`flex-1 rounded-lg px-3 py-2 text-xs font-bold ${channel === value ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 ring-1 ring-gray-200'}`}>
                  {value === 'all' ? 'すべて' : value === 'line' ? 'LINE' : 'メール'}
                </button>
              ))}
            </div>
            <div className="flex gap-2">
        ...
        <main className="flex min-h-[560px] flex-col bg-[#f4f6f5]">
          {!selected ? (
            ...
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center">
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#06C755] text-2xl font-bold text-white">LINE</div>
              ...
              <Link href={`/chats?friend=${encodeURIComponent(selected.threadId)}&unanswered=1`} className="mt-6 rounded-xl bg-[#06C755] px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-emerald-600">LINEで返信する →</Link>
            </div>
          ) : detail ? (
            <>
              ...
              <div className="border-t border-gray-200 bg-white p-4">
                ...
                <div className="mt-3 flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => void sendReply()} disabled={!reply.trim() || sending} className="rounded-xl bg-[#06C755] px-6 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-40">{sending ? '送信中…' : 'メールで返信'}</button>
                </div>
              </div>
            </>
...
function SummaryCard({ label, value, accent }: { label: string; value: number; accent: 'rose' | 'green' | 'sky' | 'amber' }) {
  const styles = { rose: 'bg-rose-50 text-rose-700 ring-rose-100', green: 'bg-emerald-50 text-emerald-700 ring-emerald-100', sky: 'bg-sky-50 text-sky-700 ring-sky-100', amber: 'bg-amber-50 text-amber-700 ring-amber-100' }
  return <div className={`rounded-xl p-4 ring-1 ${styles[accent]}`}><p className="text-xs font-semibold opacity-70">{label}</p><p className="mt-1 text-2xl font-bold tabular-nums">{value}</p></div>
}
...
function ChannelAvatar({ item }: { item: InboxItem }) {
  ...
  return <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-xs font-black text-white ${item.channel === 'line' ? 'bg-[#06C755]' : 'bg-sky-500'}`}>{item.channel === 'line' ? 'L' : '✉'}</div>
}
