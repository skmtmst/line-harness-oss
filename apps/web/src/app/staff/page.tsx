...
function Kpi({ label, value, unit, note }: { label: string; value: string; unit?: string; note: string }) { return <div className="flex h-[105px] flex-col gap-[5px] rounded-[18px] border border-hairline bg-canvas p-[15px]"><p className="text-xs font-semibold leading-[1.45] text-ink-faint">{label}</p><div className="flex h-[29px] items-start gap-1"><p className="text-xl font-bold leading-[1.45] tabular-nums text-ink">{value}</p>{unit && value !== '—' && <span className="mt-3 text-xs font-medium leading-[1.45] text-ink-faint">{unit}</span>}</div><p className="text-[11px] leading-[1.45] text-ink-faint">{note}</p></div> }
    ...
    {sessions === null ? <p className="mt-3 text-xs text-ink-faint">読み込んでいます…</p> : sessions.length === 0 ? <p className="mt-3 text-xs text-ink-faint">ログイン中の端末はありません。</p> : (
      <ul className="mt-3 divide-y divide-hairline">
        {sessions.map((session) => <li key={session.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
          <div className="min-w-0"><p className="truncate text-sm font-medium text-ink" title={session.userAgent ?? undefined}>{deviceLabel(session.userAgent)}{session.current && <span className="ml-2 rounded-full bg-accent-soft px-2 py-0.5 text-xs font-semibold text-accent-deep">この端末</span>}</p>
...
function StaffPageHost() {
  ...
  return <div data-design-node="e3jz3" className="flex flex-col gap-4"><div><MergedTabs basePath="/staff" tabs={staffTabs} active={tab} defaultKey="members" actions={tabAction} /></div>
    ...
    */}
    ...
    {missing > 0 && <div className="flex items-center rounded-control bg-warning-bg px-4 py-3 text-sm"><KeyRound aria-hidden="true" className="text-status-warn-deep mr-2 size-4 shrink-0" /><p>二段階認証が未設定のユーザーが <b>{missing}人</b> います。高い権限の人から設定してください。</p></div>}
    {/*
      ★V7 `x63W5x`：取れない KPI は「—」。読み込み中は「読み込んでいます」、
      失敗は「読み込めませんでした」と言い分け、0（本当に0人）と混ぜない。
    */}
    <div data-design="KPIs" className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Kpi label="いまいる人" value={summaryReady ? `${accessSummary.active}` : '—'} unit="人" note={error ? '読み込めませんでした' : summaryReady ? `管理者 ${accessSummary.roleCounts.administrator}・運用 ${accessSummary.roleCounts.operations}・見るだけ ${accessSummary.roleCounts.view_only}` : '読み込んでいます'} /><Kpi label="招待して返事がない" value={summaryReady ? `${accessSummary.invited}` : '—'} unit="人" note={error ? '読み込めませんでした' : summaryReady ? `期限切れ ${accessSummary.expiredInvitations}人` : '読み込んでいます'} /><Kpi label="90日 入っていない" value={summaryReady ? `${accessSummary.unused90Days}` : '—'} unit="人" note={error ? '読み込めませんでした' : summaryReady ? '最終ログインから90日以上' : '読み込んでいます'} /><Kpi label="2段階の確認" value={summaryReady ? `${accessSummary.mfaEnabled} / ${accessSummary.active}` : '—'} unit="人" note={error ? '読み込めませんでした' : summaryReady ? `管理者は必ず入れてください${accessSummary.mfaRate === null ? '' : `（${accessSummary.mfaRate}%）`}` : '読み込んでいます'} /></div>
    <div className="flex items-center justify-between gap-3 rounded-control bg-info-bg px-4 py-3 text-xs text-ink-secondary"><span>「見せる範囲」は、画面ごとに決められます。電話番号や住所など、必要な情報だけを見せると事故が減ります。</span></div>
    {/*
      ★V7 `x63W5x`：一覧の失敗でページ上の帯は出さない。表の中の
      TableStateRow error（読み直す口つき）だけにまとめる。
    */}
    {resendNotice && <div className="rounded-control bg-info-bg px-4 py-3 text-sm font-medium text-info" role="status"><p>{resendNotice}</p></div>}
    {resendError && <div className="rounded-control bg-danger-bg p-3 text-sm text-danger" role="alert"><p>{resendError}</p></div>}
    {permissionSaveNotice && <div className="rounded-control bg-info-bg px-4 py-3 text-sm font-medium text-info" role="status"><p>{permissionSaveNotice}</p></div>}
