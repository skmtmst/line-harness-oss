...
export default function TemplateConsole({ type }: { type: TemplateType }) {
  ...
  return <div className={styles.console} data-design-node={NODES[stage]} aria-busy={busy}>
    ...
    {stage === 'accounts' && <>
      ...
      <footer className={styles.footer}><Button disabled={busy} onClick={toList}>戻る</Button><Button aria-label={`${selected.length}店舗の重複を確認`} variant="primary" disabled={busy || !selected.length} onClick={() => checkStores(selected)}>{selected.length === 1 ? '選択した1店舗へ配布する前に重複を確認' : `選択した${selected.length}店舗へ一括配布する前に重複を確認`}</Button></footer>
    </>}
