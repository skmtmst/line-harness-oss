  ...
  return (
    <main className="min-h-screen bg-canvas-sunken p-10">
      ...
      <div className="mt-8">
        ...
        {state === 'overlays-notices' ? <section data-qa-overlay="notices" className="grid gap-4" style={{ width: 844 }}>{noticeVisible ? <><Notice tone="info" message="この画面の数は日本時間で数えます。" onClose={noop} /><Notice tone="success" message="保存しました" onClose={() => setNoticeVisible(false)} /><Notice tone="warn" message="入力内容を確認してください" onClose={noop} /><Notice tone="danger" message="処理に失敗しました。通信を確かめて、もう一度お試しください。" onClose={noop} /></> : <button type="button" onClick={() => setNoticeVisible(true)}>通知を戻す</button>}</section> : null}
