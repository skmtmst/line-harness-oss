      ...
      <div className="flex flex-col items-start gap-4 xl:flex-row">
        <div className="min-w-0 flex-1 space-y-4">
          ...
          <section className="border-hairline rounded-card border bg-canvas p-4">
            <div className="flex flex-wrap items-center justify-between gap-4"><div className="min-w-0 flex-1"><h2 className="text-base font-bold text-ink">復旧</h2><div className="mt-2">{impactFailed ? <NoteBar tone="warn">停止状態を確認できないため、停止・復旧を実行できません。</NoteBar> : <NoteBar>{isStopped ? '停止前に動いていたものだけを戻します。期限を過ぎた予約配信は安全のため再開しません。' : `いまは止めていません。復旧できるものはありません。${calculatedAt ? `${formatOperationDate(calculatedAt)}に確認しました。` : ''}`}</NoteBar>}</div></div>{isStopped && <Button variant="secondary" size="field" onClick={() => { setConfirmWord(''); setStepUpCode(''); setRequestKey(crypto.randomUUID()); setConfirmMode('restore') }} disabled={mutationLocked || !canControl}>復旧する</Button>}</div>
          </section>
