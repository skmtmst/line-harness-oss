...
export default function MileageRewardsTab({ accountId }: { accountId: string }) {
  ...
  return (
    <div data-design-node="qlVLJ" data-mileage-rewards-state="ready">
      {actionError ? <div className="mb-4 rounded-v6-control border border-v6-danger-border bg-v6-danger-bg p-3 text-sm text-v6-danger" role="alert">{actionError}</div> : null}
      ...
      <div className="mb-5 flex items-start gap-3 rounded-v6-card border border-v6-action/25 bg-v6-action-soft p-4 text-sm text-v6-ink">
        <Gift className="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" />
        <div>
          <p className="font-semibold">使い道を1つ以上公開してください</p>
          <p className="mt-1 text-xs text-v6-ink-secondary">ためる理由だけでなく、何に交換できるかを友だちへ伝えると、次の行動につながります。</p>
        </div>
      </div>
      ...
      {rewards.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-v6-card border border-hairline bg-canvas">
          <div className="mileage-reward-row-grid grid items-center gap-3 border-b border-v6-divider bg-v6-surface px-4 py-3 text-xs font-medium text-v6-ink-faint">
            <span>使い道</span><span>必要マイル</span><span>渡すもの</span><span>今月</span><span>状態</span><span className="text-right">操作</span>
          </div>
          <div className="divide-y divide-v6-divider">
            {rewards.map((reward, index) => (
              <div key={reward.id} className="mileage-reward-row-grid grid items-center gap-3 px-4 py-4 text-sm">
                <div className="min-w-0">
                  <p className="truncate font-semibold text-v6-ink" title={reward.name}>{reward.name}</p>
                  <p className="mt-1 truncate text-xs text-v6-ink-faint" title={reward.description ?? ''}>{reward.description || '説明はありません'}</p>
                </div>
                <span className="font-semibold text-v6-purple">{number(reward.currentVersion?.requiredMiles ?? null)}</span>
                <div className="min-w-0">
                  <p className="truncate text-v6-ink-secondary">{KIND_LABELS[reward.rewardKind]}</p>
                  {reward.rewardKind === 'coupon' ? <p className="mt-1 text-xs text-v6-ink-faint">在庫 {number(reward.availableCodeCount)}件</p> : null}
                </div>
                <span>{number(reward.exchangedThisMonth)}件</span>
                <span className={`w-fit rounded-full px-2 py-1 text-xs font-medium ${reward.status === 'published' ? 'bg-v6-accent-soft text-v6-accent-hover' : reward.status === 'stopped' ? 'bg-v6-warning-bg text-v6-warning' : 'bg-v6-surface-strong text-v6-ink-secondary'}`}>{STATUS_LABELS[reward.status]}</span>
      ...
      <style jsx>{`.mileage-reward-row-grid { grid-template-columns: minmax(220px, 1.7fr) 90px minmax(170px, 1.1fr) 100px 90px 210px; }`}</style>
    </div>
  )
}
