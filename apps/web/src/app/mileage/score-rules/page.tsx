        ...
        <div className="grid gap-3 2xl:grid-cols-4">
              ...
              <DataTable>
                ...
                */}
                ...
                <tbody>{bundle.rules.map((rule, index) => (
                  <Tr key={rule.id}>
                    <Td>
                      <Toggle checked={rule.enabled} disabled={!canEdit} label={`${rule.name}を動かす`} onChange={canEdit ? (enabled) => updateRule(index, { enabled }) : undefined} />
                      <p className="mt-1 whitespace-nowrap text-xs text-v6-ink-faint">{rule.enabled ? '動かす' : '止める'}</p>
                    </Td>
                    <Td>
          ...
          <aside className="grid gap-3 md:grid-cols-3 2xl:grid-cols-1">
            ...
            <section className="rounded-v6-card border border-v6-warning/30 bg-v6-warning-bg p-4 text-xs text-v6-ink-secondary">
              ...
              <p className="mt-2">顧客には表示しません。対応や配信の優先順位を考える目安で、顧客の価値を表す点数ではありません。</p>
