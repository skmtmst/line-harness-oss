      ...
      <main className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
        ...
        {tab === 'flow' && <section className="space-y-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:flex sm:items-center sm:justify-between">
            ...
            <SelectField value={testFriendId} onChange={(e) => setTestFriendId(e.target.value)} aria-label="テスト送信先" className="mt-3 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm sm:mt-0 sm:w-72" options={[{ value: '', label: '未設定' }, ...friends.map((friend) => ({ value: friend.id, label: friend.displayName || '名前未取得' }))]} />
          </div>
        ...
        {tab === 'pets' && <section className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
          <div className="space-y-4"><div className="rounded-2xl border border-gray-200 bg-white p-5"><h2 className="text-lg font-bold text-gray-900">ペット情報を登録</h2><p className="mt-1 text-sm text-gray-500">LINEログイン会員が任意入力した情報も、ここに自動保存されます。</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><SelectField value={petDraft.friendId} onChange={(e) => setPetDraft({ ...petDraft, friendId: e.target.value })} aria-label="ペット情報を登録するLINEユーザー" className="rounded-xl border border-gray-200 px-3 py-2.5" options={[{ value: '', label: 'LINEユーザーを選択' }, ...friends.map((friend) => ({ value: friend.id, label: friend.displayName || '名前未取得' }))]} /><input placeholder="わんちゃん・ねこちゃんのお名前" value={petDraft.name} onChange={(e) => setPetDraft({ ...petDraft, name: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2.5" /><SelectField value={petDraft.animalType} onChange={(e) => setPetDraft({ ...petDraft, animalType: e.target.value })} aria-label="ペットの種類" options={[{ value: "dog", label: "わんちゃん" }, { value: "cat", label: "ねこちゃん" }, { value: "other", label: "その他" }]} className="rounded-xl border border-gray-200 px-3 py-2.5" /><SelectField value={petDraft.gender} onChange={(e) => setPetDraft({ ...petDraft, gender: e.target.value })} aria-label="ペットの性別" options={[{ value: "unknown", label: "回答しない" }, { value: "male", label: "男の子" }, { value: "female", label: "女の子" }]} className="rounded-xl border border-gray-200 px-3 py-2.5" /><input type="date" value={petDraft.birthday} onChange={(e) => setPetDraft({ ...petDraft, birthday: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2.5" /><button onClick={() => void addPet()} className="rounded-xl bg-emerald-600 px-4 py-2.5 font-semibold text-white">登録する</button></div></div>
