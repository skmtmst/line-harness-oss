    ...
    <div data-design-node="P2J0Te" className="space-y-4 pb-8">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-card border border-hairline bg-canvas px-4 py-3">
        <label className="grid gap-1 text-xs font-semibold text-ink-secondary">
          追加の種類
          <select className="v6-select min-w-44" value={kind} onChange={(event) => setKind(event.target.value as KindFilter)}>
            <option value="all">すべての追加</option>
            <option value="first_time">はじめて</option>
            <option value="returning">再追加・ブロック解除</option>
          </select>
        </label>
        <label className="grid gap-1 text-xs font-semibold text-ink-secondary">
          流入経路
          <select className="v6-select min-w-44" value={attribution} onChange={(event) => setAttribution(event.target.value as AttributionFilter)}>
            <option value="all">すべての経路</option>
            <option value="captured">経路を取得できた</option>
            <option value="unavailable">経路を取得できない</option>
          </select>
        </label>
        <label className="grid gap-1 text-xs font-semibold text-ink-secondary">
          配信・処理
          <select className="v6-select min-w-44" value={routing} onChange={(event) => setRouting(event.target.value as RoutingFilter)}>
            <option value="all">すべての結果</option>
            <option value="completed">動きました</option>
            <option value="pending">処理中です</option>
            <option value="failed">確認が必要です</option>
            <option value="suppressed">配信しませんでした</option>
          </select>
        </label>
