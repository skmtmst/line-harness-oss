...
export default function NewRestaurantStorePage() {
  ...
  return <div>
    <Header title="店舗を追加" description="LINE公式アカウントの準備から接続まで、4つの手順で進めます。" action={<Link href="/restaurant-test/stores" className="text-sm font-semibold text-action">店舗一覧へ戻る</Link>} />
    <div className="grid items-start gap-5 xl:grid-cols-[220px_minmax(0,1fr)]">
      <ol className="rounded-card border border-hairline bg-canvas p-4">{steps.map(([title, description], index) => {
        ...
        const complete = number < step || Boolean(created)
        ...
        return <li key={title} className="relative flex gap-3 pb-6 last:pb-0">
          ...
          <div><p className={`text-sm font-semibold ${complete ? 'text-success' : current ? 'text-ink' : 'text-ink-faint'}`}>{title}</p><p className="mt-1 text-xs leading-5 text-ink-faint">{description}</p></div>
        </li>
      })}</ol>

      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
        <main className="rounded-card border border-hairline bg-canvas p-5 sm:p-7">
          <p className="text-xs font-semibold text-ink-faint">ステップ {step} / 4</p>
          ...
          {step === 2 && <div className="mt-7 space-y-5">
        ...
        <aside className="rounded-card border border-hairline bg-canvas p-5">
          ...
          <div className="mt-4 space-y-3">{step === 1 ? <p className="text-xs leading-5 text-ink-secondary">店舗名はあとから店舗設定で変更できます。</p>
            : step === 2 ? <ManualLink href={MANUAL_LINKS.createOfficialAccount}>LINE公式アカウントを作る</ManualLink>
