          ...
          <section className="border-hairline rounded-card border bg-canvas p-5">
            ...
            <div className="mt-4 space-y-4">
              <label className="text-ink-secondary block text-sm">
                共通アクション名
                <input value={name} onChange={(event) => setName(event.target.value)} maxLength={120} className={`${inputClass} mt-1`} placeholder="例：来店後のお礼を送る" />
              </label>
              <label className="text-ink-secondary block text-sm">
                説明
                <textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={3} className={`${inputClass} mt-1 py-2`} placeholder="使う場面や目的を書きます" />
              </label>
            </div>
          </section>
