  ...
  return (
    <div data-design-node="JupxW" data-design="Body">
      ...
      {deleteOpen && route && <div className="fixed inset-0 z-70 flex items-center justify-center bg-ink/35 p-4" data-design-node="UIaM7" role="dialog" aria-modal="true">
        <div className="w-full max-w-[840px] overflow-hidden rounded-card bg-canvas shadow-2xl">
          <div className="flex min-h-[96px] items-start gap-3 border-b border-hairline px-6 py-5"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-danger-bg text-xl font-bold text-danger">!</span><div><h2 className="text-xl font-bold text-ink">「{route.name}」を削除しますか？</h2><p className="mt-1 text-sm text-ink-faint">このURLは {route.createdAt.slice(5, 10).replace('-', '/')} から使われています。消すと同じURLは開けなくなります。</p></div></div>
