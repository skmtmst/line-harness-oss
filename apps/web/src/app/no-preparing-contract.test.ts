...
describe('画面に「準備中」を置かない', () => {
  it('全画面を読めている（数え漏れの見張り）', () => {
    ...
    expect(FILES.filter((f) => /^app\/.*\/page\.tsx$/.test(f.p) || f.p === 'app/page.tsx')).toHaveLength(134)  // 2026-09-04: 自動応答の公開（`auto-replies/publish`）と 34 の画面（`getting-started`・`recipes`・`recipes/clone`）で 134。
  })
