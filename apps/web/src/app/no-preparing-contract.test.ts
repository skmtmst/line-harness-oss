...
describe('画面に「準備中」を置かない', () => {
  it('全画面を読めている（数え漏れの見張り）', () => {
    ...
    expect(FILES.filter((f) => /^app\/.*\/page\.tsx$/.test(f.p) || f.p === 'app/page.tsx')).toHaveLength(184)  // 情報欄の編集画面と運営専用ナレッジ（37-11）、★V7見本（v7-progress-filedrop・v7-combobox）を含む実測値。2026-09-25 に比較用 /tags-v2・/tags-v3 の2画面を撤去。
  })
