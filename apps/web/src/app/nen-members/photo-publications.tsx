...
export function PhotoPublications({ accountId, onBack }: { accountId: string; onBack: () => void }) {
  ...
  if (state === 'loading') return <main><ListState kind="loading" title="出している写真を読み込んでいます" /></main>
  if (state === 'forbidden') return <main><ListState kind="forbidden" /></main>
  if (state === 'error') return <main><ListState kind="error" title="出している写真を読み込めませんでした" onRetry={() => void load()} /></main>
  ...
  if (!data || (items.length === 0 && pendingWithdrawals.length === 0 && withdrawnItems.length === 0)) {
    return <main><Button onClick={onBack}>写真審査へ戻る</Button><ListState kind="empty" title="出している写真はありません" description="同意のある写真を掲載すると、使っている場所と表示回数がここに出ます。" /></main>
  }

  const top = data.summary.topPhoto
  return <><main data-photo-view="publications">
