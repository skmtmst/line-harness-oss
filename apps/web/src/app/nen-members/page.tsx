...
export default function PhotoReviewsPage() {
  ...
  return <>
    <div data-design="KPIs" className="mx-auto mb-4 grid max-w-7xl grid-cols-1 gap-4 px-4 sm:grid-cols-2 sm:px-6 xl:grid-cols-4">
      <div className="bg-canvas rounded-tile border-hairline border p-4">
    ...
    <main className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      {notice && <div className="rounded-control border border-v6-accent-border bg-v6-accent-soft px-4 py-3 text-sm text-v6-accent-hover">{notice}</div>}
      ...
      void review(text(rejectingPhoto.id), 'rejected', { reasonCode, reasonNote: reasonNote.trim() })
        ...
        <div className="grid gap-6 md:grid-cols-[220px_1fr]">
          ...
          <div className="space-y-4">
            <div className="rounded-control bg-v6-surface px-3 py-2 text-sm text-v6-ink-secondary">
              <p className="font-semibold text-v6-ink">
              ...
              <p className="mt-1 text-xs text-v6-ink-faint">この方を前に戻した回数は未取得です</p>
            </div>
            <fieldset className="space-y-2">
              <legend className="text-sm font-semibold text-v6-ink">戻す理由</legend>
              {REVIEW_REASONS.map((reason) => <label key={reason.value} className="flex cursor-pointer items-start gap-2 rounded-control border border-hairline px-3 py-2.5 text-sm text-v6-ink-secondary"><input type="radio" name="photo-review-reason" value={reason.value} checked={reasonCode === reason.value} onChange={() => { setReasonCode(reason.value); setReasonError('') }} className="mt-0.5" /><span><span className="font-medium text-v6-ink">{reason.label}</span><span className="mt-1 block text-xs text-v6-ink-faint">投稿者へ：今回は「{reason.label}」のため、掲載を見送らせていただきました。</span></span></label>)}
            </fieldset>
            <label className="block text-sm font-semibold text-v6-ink">投稿者に届く補足（直せます）<textarea value={reasonNote} onChange={(event) => { setReasonNote(event.target.value.slice(0, 500)); setReasonError('') }} rows={3} placeholder={reasonCode === 'other' ? '理由を入力してください' : '必要な場合だけ入力します'} className="mt-2 w-full rounded-control border border-hairline bg-canvas px-3 py-2 text-sm font-normal text-v6-ink" /></label>
            <div className="rounded-control border border-v6-accent-border bg-v6-accent-soft p-3 text-sm text-v6-ink-secondary"><p className="font-semibold text-v6-ink">投稿者に届く内容</p><p className="mt-1 whitespace-pre-line">お写真をご投稿いただきありがとうございます。{`\n`}今回は「{selectedReasonLabel}」のため、掲載を見送らせていただきました。{reasonNote && `\n${reasonNote}`}{`\n`}内容をご確認のうえ、よろしければ別のお写真をご投稿ください。</p></div>
          </div>
        </div>
    </Dialog>}
  </>
