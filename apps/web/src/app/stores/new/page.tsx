...
function NewStoreFlow() {
  ...
  return <div className="min-h-screen bg-shell pb-28 text-ink">
    ...
    <footer className="fixed inset-x-0 bottom-0 border-t border-hairline bg-canvas"><div className="mx-auto flex min-h-20 max-w-5xl items-center justify-center gap-6 px-6"><p className="flex-1 text-sm text-ink-secondary">{connected ? '接続情報を保存しました' : '入力内容はこの画面で保持されています'}</p><div className="flex shrink-0 gap-3">{step !== 'choose' && step !== 'done' ? <Button variant="secondary" disabled={saving || Boolean(connected)} onClick={back}>戻る</Button> : null}{step === 'verify' ? connected ? <Button variant="primary" onClick={() => move('done')}>次へ</Button> : <Button variant="primary" disabled={saving} onClick={() => void connect()}>{saving ? '接続中…' : '接続して保存'}</Button> : step === 'done' ? <Button variant="primary" onClick={() => router.replace('/hq')}>統括の店舗一覧へ</Button> : <Button variant="primary" disabled={(step === 'choose' && !choice) || (step === 'create' && !createdOfficial) || (step === 'api' && !apiEnabled)} onClick={next}>次へ</Button>}</div><div className="flex-1" aria-hidden="true" /></div></footer>
  </div>
}
