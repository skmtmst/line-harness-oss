      ...
      <aside className="relative h-full w-full overflow-y-auto bg-canvas-sunken shadow-xl">
        <div className="border-hairline sticky top-0 z-10 flex min-h-16 items-center justify-between gap-3 border-b bg-canvas px-6 py-3">
          <div className="min-w-0">
            <p className="text-accent-deep text-xs font-semibold">予約管理　›　今日　›　{formatJpTime(b.starts_at)} {b.friend_name ?? 'お客様'}さま</p>
        ...
        <div data-design="Body" className="grid gap-4 px-6 py-4 xl:grid-cols-4">
          ...
          <section className="bg-canvas rounded-card border-hairline mb-4 border p-5">
            ...
            {b.friend_id ? <Link href={`/friends/detail?id=${encodeURIComponent(b.friend_id)}`} className="text-accent-deep text-xs font-semibold">顧客カルテで以前の予約を見る →</Link> : null}
          </section>
          ...
          <section className="bg-canvas rounded-card border-hairline border p-5">
            ...
            <div className="mt-3 space-y-2 text-xs"><p><Link href="/booking/menus" className="text-accent-deep font-semibold">→ 予約設定</Link>　メニューと受付枠</p><p><Link href="/reminders" className="text-accent-deep font-semibold">→ リマインダ</Link>　前日・開始前のお知らせ</p>{b.friend_id ? <p><Link href={`/chats?friend=${b.friend_id}`} className="text-accent-deep font-semibold">→ 受信箱</Link>　この方とのやりとり</p> : null}<p><Link href="/mileage" className="text-accent-deep font-semibold">→ マイル</Link>　来店時の付与</p></div>
          </section>
          <div className="bg-canvas rounded-card border-hairline border p-5">
