...
function ArchiveMarkDialog({ mark, impact, replacementMarkId, loading, saving, error, onReplacement, onCancel, onConfirm }: {
  ...
  return (
    <div ref={dialogRef} className="fixed inset-0 z-[90] flex items-center justify-center bg-ink/45 p-4">
      <section className="w-full max-w-[680px] rounded-card border border-hairline bg-canvas p-7 shadow-2xl" role="alertdialog" aria-modal="true">
        ...
        {loading ? <p className="mt-5 rounded-control bg-surface-soft p-4 text-sm text-ink-faint">影響を確認しています…</p> : impact ? (
          <div className="mt-5 space-y-4">
            <dl className="divide-y divide-hairline overflow-hidden rounded-control border border-hairline text-sm">
              ...
              <div className="flex justify-between px-4 py-3"><dt className="text-ink-secondary">表示先</dt><dd className="max-w-[420px] text-right font-bold">{impact.displayTargets.map((target) => DISPLAY_TARGET_LABELS[target]).join('・')}</dd></div>
            </dl>
        ...
        <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={saving} className="rounded-control border border-hairline px-4 py-2.5 text-sm font-medium text-ink-secondary">やめる</button><button type="button" onClick={onConfirm} disabled={loading || saving || !impact?.canArchive || !replacementMarkId} className="rounded-control bg-danger px-4 py-2.5 text-sm font-bold text-on-accent disabled:opacity-40">{saving ? '保管中…' : '置き換えて保管する'}</button></div>
      </section>
    </div>
  )
}
