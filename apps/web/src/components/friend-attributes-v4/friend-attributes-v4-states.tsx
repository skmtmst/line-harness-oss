...
const screenData: Record<Exclude<FriendAttributesV4State, 'csv'>, {
  nodeId: string
  description: string
  primary: string
  metrics: Metric[]
  ...
  fields: {
    nodeId: 'ZAFby',
    description: '友だちごとに記録する情報項目を、回答フォーム・友だち詳細・差し込みで共通管理します。',
    primary: '＋ 項目を追加',
    metrics: [
    ...
    guide: '作成した項目は、回答フォームの登録先・友だち詳細・テンプレートの差し込みで同じ内容を利用します。',
    ...
    safetyTitle: '削除・種類変更の安全確認',
    safetyText: '使用中の項目を削除・種類変更するときは、影響する友だち数と回答フォームを表示します。値が失われる操作は専用確認ダイアログで確認します。',
  },
  marks: {
    nodeId: 'yTPY6',
    ...
    guide: '受信箱・友だち一覧・友だち詳細で共通利用します。新着時の初期値、自動変更ルール、担当者の対応状況を揃えます。',
    ...
    safetyTitle: '削除・初期値変更の安全確認',
    safetyText: '使用中のマークを削除するときは、影響人数と置き換え先を表示します。ブラウザ標準アラートではなく、内容が分かる専用確認ダイアログを使います。',
  },
  searches: {
    nodeId: 'cxtem',
    description: 'よく使う絞り込みに名前を付け、一覧・配信・自動処理で再利用します。',
    primary: '＋ 条件を作成',
    metrics: [
    ...
    guide: 'AND（すべて満たす）とOR（いずれか満たす）を組み合わせ、友だち一覧・配信先・オートメーションから同じ条件を呼び出します。',
...
function Table({ headers, rows, wide }: { headers: string[]; rows: Row[]; wide?: boolean }) {
  return <div className={`${styles.table} ${wide ? styles.searchTable : ''}`}>
    ...
    {rows.map((row, index) => <div className={styles.tableRow} key={`${row[0]?.text}-${index}`}>{row.map((cell, cellIndex) => (
      <span key={`${cell.text}-${cellIndex}`} title={cell.text}><b className={cell.tone ? styles[cell.tone] : ''}>{cell.text}</b></span>
    ))}</div>)}
  </div>
}
...
function CsvDialog() {
  return <div className={styles.overlay} role="presentation"><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="csv-title">
    <header><h2 id="csv-title">CSVでタグを一括登録</h2><p>1列目にタグ名、2列目にフォルダ名を入れます。登録前に内容と重複を確認できます。</p></header>
    <div className={styles.flow}><strong>登録の流れ</strong><span>1. テンプレートを確認　 2. CSVを選択　 3. 重複・形式エラーを確認　 4. 登録</span></div>
...
export default function FriendAttributesV4States({ state }: { state: FriendAttributesV4State }) {
  ...
  const data = state === 'csv' ? {
    ...
    guide: 'タグ名・用途で検索し、付与元と使用先を確認できます。',
  ...
  return <div className={styles.screen} data-design-node={data.nodeId} data-design-version="friend-attributes-v4-states">
    ...
    <div className={styles.toolbar}><div>{data.filters.map((filter) => <Button key={filter}>{filter}</Button>)}</div><div><Button>{state === 'searches' ? '並び替え' : 'ドラッグで並び替え'}</Button><Button primary>{data.primary}</Button></div></div>
    ...
    {state === 'searches' && <div className={styles.pagination}><Button>20件表示</Button><span>1〜20 / 12件</span><b>前へ　1　2　3　…　次へ</b></div>}
    {state === 'csv' && <CsvDialog />}
  </div>
}
