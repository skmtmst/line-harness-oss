...
export default function BroadcastAssetManager({ kind }: { kind: BroadcastAssetKind }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-bold text-success">{meta.title}</span><h3 className="mt-3 truncate font-bold text-slate-900">{item.name}</h3><p className="mt-1 text-xs text-slate-500">更新 {new Date(item.updatedAt).toLocaleString('ja-JP')}</p></div></div><div className="mt-4 flex flex-wrap gap-2"><a href={`/broadcasts/new?contentTemplateId=${encodeURIComponent(item.id)}`} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-bold text-success">一斉配信で使う</a><button onClick={() => startEdit(item)} className="min-w-24 flex-1 rounded-lg border px-3 py-2 text-sm font-bold">編集</button><button onClick={() => { setDeleteError(''); setDeleteTarget({ item, accountId: selectedAccountId }) }} className="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-600">削除</button></div></article>)}{items.length === 0 && <div className="col-span-full rounded-2xl border border-dashed bg-white p-12 text-center text-sm text-slate-500">まだ{meta.singular}テンプレートがありません。「新規作成」から追加してください。</div>}</div>
