...
const LABELS: Record<BroadcastAssetKind, { title: string; description: string; singular: string }> = {
  rich_message: { title: 'リッチメッセージ', description: '画像とタップ領域を組み合わせた素材を設計します。', singular: 'リッチメッセージ' },
  card_message: { title: 'カードタイプ', description: '横にスワイプできるカードを最大9枚まで設計します。', singular: 'カードセット' },
  coupon: { title: 'クーポン', description: '配信時に選択できるクーポンを登録します。', singular: 'クーポン' },
  research: { title: 'リサーチ', description: '配信時に選択できるアンケートを登録します。', singular: 'リサーチ' },
}
...
export default function BroadcastAssetManager({ kind }: { kind: BroadcastAssetKind }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold text-slate-900">{meta.title} 設計</h2><p className="mt-1 text-sm text-slate-500">{meta.description}</p></div><button onClick={() => { reset(); setShowForm(true) }} className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white">＋ 新規作成</button></div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-bold text-emerald-700">{meta.title}</span><h3 className="mt-3 truncate font-bold text-slate-900">{item.name}</h3><p className="mt-1 text-xs text-slate-500">更新 {new Date(item.updatedAt).toLocaleString('ja-JP')}</p></div></div><div className="mt-4 flex gap-2"><button onClick={() => startEdit(item)} className="flex-1 rounded-lg border px-3 py-2 text-sm font-bold">編集</button><button onClick={async () => { if (!confirm(`「${item.name}」を削除しますか？`)) return; await api.broadcastMessageAssets.delete(item.id); await load() }} className="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-600">削除</button></div></article>)}{items.length === 0 && <div className="col-span-full rounded-2xl border border-dashed bg-white p-12 text-center text-sm text-slate-500">まだ{meta.singular}がありません。「新規作成」から追加してください。</div>}</div>
  </div>
}
