...
const LABELS: Record<BroadcastAssetKind, { title: string; description: string; singular: string }> = {
  ...
  card_message: { title: 'カードタイプ', description: '横にスワイプできるカードを最大9枚まで作成し、一斉配信から引用できます。', singular: 'カードセット' },
...
export default function BroadcastAssetManager({ kind }: { kind: BroadcastAssetKind }) {
  ...
  /*
    ...
    何が消えるのかを本文で読ませられず、画像比較にも写らない。
  */
  const [deleteTarget, setDeleteTarget] = useState<BroadcastMessageAsset | null>(null)
  const [deleting, setDeleting] = useState(false)
  const [deleteError, setDeleteError] = useState('')

  /*
    このテンプレートを参照する子テーブルは無い（外部キーが張られていない）。
    引用して作った配信は中身をコピー済みなので、消しても送信済み・予約済みの
    配信は変わらない。
  */
  const confirmDelete = async () => {
    ...
    setDeleteError('')
    try {
      const res = await api.broadcastMessageAssets.delete(deleteTarget.id)
      if (!res.success) throw new Error(res.error)
      setDeleteTarget(null)
      await load()
    } catch {
      setDeleteError('テンプレートを削除できませんでした。時間をおいて、もう一度お試しください。')
    } finally {
      setDeleting(false)
    }
  }
  ...
  return <div className="space-y-5">
    ...
    {showForm && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {kind === 'card_message' && <div className="space-y-3">{cards.map((card, index) => <div key={card.id} className="rounded-xl border bg-slate-50 p-4"><div className="mb-3 flex justify-between"><b className="text-sm">カード {index + 1}</b><button disabled={cards.length === 1} onClick={() => setCards((all) => all.filter((_, i) => i !== index))} className="text-xs text-rose-600 disabled:opacity-30">削除</button></div><div className="grid gap-3 md:grid-cols-2"><select value={card.template} onChange={(e) => setCards((all) => all.map((c,i) => i===index ? {...c,template:e.target.value}:c))} className="rounded-lg border px-3 py-2 text-sm"><option value="product">プロダクト</option><option value="location">ロケーション</option><option value="person">人物</option><option value="image">画像</option></select><label className="rounded-lg border border-dashed px-3 py-2 text-center text-sm">{card.imageUrl ? '画像設定済み' : '画像を選択'}<input type="file" accept="image/jpeg,image/png" className="hidden" onChange={(e) => { const f=e.target.files?.[0]; if(f) void upload(f,index) }}/></label><input value={card.title} onChange={(e) => setCards((all) => all.map((c,i) => i===index ? {...c,title:e.target.value}:c))} placeholder="タイトル" className="rounded-lg border px-3 py-2 text-sm"/><input value={card.description} onChange={(e) => setCards((all) => all.map((c,i) => i===index ? {...c,description:e.target.value}:c))} placeholder="説明" className="rounded-lg border px-3 py-2 text-sm"/><input value={card.actionLabel} onChange={(e) => setCards((all) => all.map((c,i) => i===index ? {...c,actionLabel:e.target.value}:c))} placeholder="ボタン名" className="rounded-lg border px-3 py-2 text-sm"/><input value={card.actionUrl} onChange={(e) => setCards((all) => all.map((c,i) => i===index ? {...c,actionUrl:e.target.value}:c))} placeholder="アクションURL" className="rounded-lg border px-3 py-2 text-sm"/></div></div>)}<button disabled={cards.length >= 9} onClick={() => setCards((all) => [...all,newCard()])} className="w-full rounded-xl border border-dashed py-3 text-sm font-bold text-emerald-700 disabled:text-slate-400">＋ カードを追加（{cards.length}/9）</button><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked readOnly />末尾に「もっと見る」カードを表示</label></div>}
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-bold text-emerald-700">{meta.title}</span><h3 className="mt-3 truncate font-bold text-slate-900">{item.name}</h3><p className="mt-1 text-xs text-slate-500">更新 {new Date(item.updatedAt).toLocaleString('ja-JP')}</p></div></div><div className="mt-4 flex flex-wrap gap-2"><a href={`/broadcasts/new?contentTemplateId=${encodeURIComponent(item.id)}`} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-bold text-emerald-700">一斉配信で使う</a><button onClick={() => startEdit(item)} className="min-w-24 flex-1 rounded-lg border px-3 py-2 text-sm font-bold">編集</button><button onClick={() => { setDeleteError(''); setDeleteTarget(item) }} className="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-600">削除</button></div></article>)}{items.length === 0 && <div className="col-span-full rounded-2xl border border-dashed bg-white p-12 text-center text-sm text-slate-500">まだ{meta.singular}テンプレートがありません。「新規作成」から追加してください。</div>}</div>
    <ConfirmDialog
      open={deleteTarget !== null}
      title={`${meta.title}「${deleteTarget?.name ?? ''}」を削除しますか？`}
      description={`この${meta.singular}テンプレートと、登録した中身を削除します。このテンプレートを引用して作った一斉配信は中身を写し取ってあるので、送信済み・予約中の配信は変わりません。以降このテンプレートを引用することはできなくなります。この操作は元に戻せません。`}
      confirmLabel="削除する"
      destructive
      busy={deleting}
      error={deleteError}
      onConfirm={() => void confirmDelete()}
      onCancel={() => { if (deleting) return; setDeleteError(''); setDeleteTarget(null) }}
    />
  </div>
}
