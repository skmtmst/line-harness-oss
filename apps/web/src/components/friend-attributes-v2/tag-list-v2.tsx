...
const SHADOW = '[box-shadow:1px_1px_2px_rgba(29,29,31,0.13)]'
...
function Pager({ page, pages, onChange }: { page: number; pages: number; onChange: (page: number) => void }) {
  ...
  return <div data-design="Pagination" className="flex justify-end gap-1 border-t border-hairline px-3 py-3 text-xs">
...
export default function FriendAttributesV2TagList({ fixture }: { fixture?: FriendAttributesV2Fixture }) {
  ...
  return <div data-design-node="xn98K" className="min-w-0 font-sans text-ink">
    <header data-design="Head" className="flex min-h-[58px] items-start justify-between gap-5"><div><h1 className="text-[30px] font-bold leading-tight tracking-[-0.02em]">友だち属性</h1><p className="mt-1 text-[13px] text-ink-secondary">タグ・情報欄・対応マーク・保存条件を、用途まで見ながら管理します。</p></div><div className="flex shrink-0 gap-2"><button type="button" onClick={() => setManualOpen(true)} className="h-9 rounded-control border border-hairline bg-canvas px-4 text-[13px]">マニュアル</button><button type="button" onClick={() => { setCsvOpen(true); setCsvRows([]); setCsvError('') }} className="h-9 rounded-control border border-hairline bg-canvas px-4 text-[13px]">CSVで一括登録</button></div></header>
    <nav data-design="Tabs" className="mt-3 flex h-8 items-center gap-2" aria-label="友だち属性の種類"><span className="inline-flex h-8 items-center rounded-pill bg-accent-soft px-4 text-[13px] font-semibold text-accent">タグ</span>{[['fields','友だち情報欄'],['marks','対応マーク'],['searches','保存した検索']].map(([tab,label]) => <Link key={tab} href={`/tags?tab=${tab}`} className="inline-flex h-8 items-center rounded-pill bg-canvas-sunken px-4 text-[13px] text-ink-secondary">{label}</Link>)}</nav>
    <section data-design="KPIs" className="mt-4 grid grid-cols-2 gap-3 xl:grid-cols-4">{kpis.map(([title,value,unit,detail]) => <article key={title} className={`h-[108px] rounded-[12px] border border-hairline bg-canvas px-5 py-4 ${SHADOW}`}><p className="text-[12px] font-medium text-ink-secondary">{title}</p><p className="mt-1 flex items-baseline gap-1"><span className="text-[26px] font-bold tabular-nums">{value.toLocaleString('ja-JP')}</span><span className="text-[12px] text-ink-secondary">{unit}</span></p><p className="mt-1 text-[11px] text-ink-faint">{detail}</p></article>)}</section>
    <div data-design="Actions" className="mt-4 flex gap-2"><Link href="/tags/folders/new" className="inline-flex h-9 items-center rounded-control border border-hairline bg-canvas px-4 text-[13px]">フォルダを追加</Link><Link href="/tags/new" className="inline-flex h-9 items-center rounded-control bg-accent px-4 text-[13px] font-semibold text-on-accent">＋ タグを追加</Link></div>
    {error && <p className="mt-3 rounded-control border border-danger/20 bg-danger-bg px-4 py-3 text-sm text-danger">{error}</p>}
    <div className="mt-3 grid min-w-0 gap-[14px] xl:grid-cols-[270px_minmax(0,1fr)]">
      <aside data-design="Folder" className={`h-fit overflow-hidden rounded-[12px] border border-hairline bg-canvas ${SHADOW}`}><div className="flex h-[46px] items-center justify-between border-b border-hairline px-4"><h2 className="text-[13px] font-bold">フォルダ</h2><span className="text-[11px] text-ink-faint">{fixture?.total ?? items.length}件</span></div><div className="p-2">{folderRows.map((row) => <button key={row.id} type="button" onClick={() => setFolder(row.id)} className={`flex h-9 w-full items-center gap-2 rounded-control px-3 text-[13px] ${folder === row.id ? 'bg-accent-soft font-semibold text-accent' : 'text-ink-secondary hover:bg-canvas-sunken'}`}><span className="h-2 w-2 rounded-full" style={{backgroundColor:row.color}} /><span className="flex-1 truncate text-left">{row.name}</span><span className="text-[11px] text-ink-faint">{row.count}</span></button>)}</div><p className="border-t border-hairline px-4 py-3 text-[10px] leading-4 text-ink-faint">フォルダを削除しても、中のタグは未分類として残ります。</p></aside>
      <main className="min-w-0"><div data-design="Toolbar" className="flex min-h-9 flex-wrap items-center gap-2"><input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="タグ名・用途で検索" className="h-9 w-[145px] rounded-control border border-hairline px-3 text-[12px] outline-none focus:border-accent" /><select value={usage} onChange={(e) => setUsage(e.target.value)} className="h-9 rounded-control border border-hairline px-3 text-[12px]"><option value="all">使用状態：すべて</option><option value="unused">未使用</option><option value="linked">連動あり</option></select><select value={source} onChange={(e) => setSource(e.target.value)} className="h-9 rounded-control border border-hairline px-3 text-[12px]"><option value="all">付与元：すべて</option>{[...new Set(items.map((tag) => getMeta(tag).source))].map((value) => <option key={value}>{value}</option>)}</select><select aria-label="表示件数" value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))} className="ml-auto h-9 rounded-control border border-hairline px-3 text-[12px]">{[20,30,40,50].map((size) => <option key={size} value={size}>{size}件表示</option>)}</select><span className="whitespace-nowrap text-[11px] text-ink-faint">{total ? (current-1)*pageSize+1 : 0}〜{Math.min(current*pageSize,total)}/{total}件</span></div>
        <div data-design="QuickFilters" className="mb-2 mt-2 flex min-h-7 flex-wrap items-center gap-2 text-[11px]"><span className="text-ink-faint">よく使う</span>{([['unused','未使用のタグ'],['recent','今月増えたタグ'],['automatic','自動付与あり'],['linked','連動あり'],['starred','★一覧表示']] as Array<[Quick,string]>).map(([key,label]) => <button key={key} type="button" onClick={() => {setQuickTouched(true);setQuick(quick===key?'':key)}} className={`rounded-pill border px-3 py-1.5 ${quick===key?'border-accent bg-accent-soft text-accent':'border-hairline bg-canvas text-ink-secondary'}`}>{label}</button>)}</div>
        <div data-design="Table" className={`overflow-hidden rounded-[12px] border border-hairline bg-canvas ${SHADOW}`}><table className="w-full table-fixed text-[12px]"><colgroup><col className="w-9"/><col className="w-[20%]"/><col className="w-[11%]"/><col className="w-[8%]"/><col className="w-[11%]"/><col className="w-[21%]"/><col className="hidden w-[11%] 2xl:table-column"/><col className="hidden w-[9%] 2xl:table-column"/><col className="hidden w-[7%] 2xl:table-column"/><col className="w-[12%]"/></colgroup><thead className="h-[46px] border-b border-hairline bg-canvas-sunken text-[10px] text-ink-faint"><tr><th/><th className="px-2 text-left">タグ</th><th className="px-2 text-left">フォルダ</th><th className="px-2 text-left">付与人数</th><th className="px-2 text-left">自動付与のもと</th><th className="px-2 text-left">連動（マイル・アクション）</th><th className="hidden px-2 text-left 2xl:table-cell">使用先</th><th className="hidden px-2 text-left 2xl:table-cell">登録日</th><th className="hidden px-2 text-left 2xl:table-cell">表示</th><th className="px-2 text-left">操作</th></tr></thead><tbody className="divide-y divide-hairline">{loading ? <tr><td colSpan={10} className="h-[184px] text-center text-ink-faint">読み込み中…</td></tr> : visible.length===0 ? <tr><td colSpan={10} className="h-[184px] text-center text-ink-faint">条件に合うタグはありません</td></tr> : visible.map((tag) => {const meta=getMeta(tag);return <tr key={tag.id} className="h-[46px] hover:bg-canvas-sunken"><td draggable={!fixture} onDragStart={()=>setDragId(tag.id)} onDragOver={(e)=>!fixture&&e.preventDefault()} onDrop={()=>void reorder(tag.id)} className="cursor-grab px-2 text-center text-ink-faint"><span className="tracking-[-2px]">⠿</span></td><td className="truncate px-2 font-medium" title={tag.name}>{tag.name}</td><td className="px-2"><select aria-label={`${tag.name}のフォルダ`} value={tag.groupId??''} disabled={Boolean(fixture)} onChange={async(e)=>{const result=await api.tags.setGroup(tag.id,e.target.value||null);if(!result.success)setError(result.error);else void load()}} className="h-7 w-full rounded-md border border-hairline bg-canvas px-2 text-[11px] disabled:opacity-100"><option value="">未分類</option>{groups.map((group)=><option key={group.id} value={group.id}>{group.name}</option>)}</select></td><td className="px-2 font-medium">{tag.friendCount??0}人</td><td className="truncate px-2 text-[11px] text-ink-secondary">{meta.source}</td><td className="px-2"><div className="flex gap-1 overflow-hidden">{meta.chips.length?meta.chips.map((chip,index)=><span key={chip} className={`whitespace-nowrap rounded-pill px-2 py-1 text-[9px] ${chip.includes('倍')?'bg-warning-bg text-warning':index>1?'bg-canvas-sunken text-ink-secondary':'bg-accent-soft text-accent'}`}>{chip}</span>):<span className="text-ink-faint">—</span>}</div></td><td className="hidden truncate px-2 text-[11px] text-ink-secondary 2xl:table-cell">{meta.usage}</td><td className="hidden px-2 text-[10px] text-ink-faint 2xl:table-cell">{meta.date}</td><td className="hidden px-2 text-[10px] 2xl:table-cell"><button type="button" disabled={Boolean(fixture)} onClick={async()=>{const result=await api.tags.update(tag.id,{isStarred:!tag.isStarred});if(!result.success)setError(result.error);else void load()}}>{tag.isStarred?'★ 一覧':'—'}</button></td><td className="px-2"><div className="flex gap-2 whitespace-nowrap text-[11px]"><Link href={`/tags/edit?id=${encodeURIComponent(tag.id)}`} className="text-accent">編集</Link><button type="button" onClick={()=>setDeleteTarget(tag)} className="text-danger">削除</button></div></td></tr>})}</tbody></table><Pager page={current} pages={pages} onChange={setPage}/></div>
      </main>
    </div>
