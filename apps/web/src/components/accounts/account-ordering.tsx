...
export default function AccountOrdering() {
  ...
  return <section className="mt-6 border-t border-hairline pt-6" aria-labelledby="account-ordering-title">
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
      ...
      <button onClick={() => void save()} disabled={changed.length === 0 || saving} className="cursor-pointer whitespace-nowrap rounded-control bg-accent px-4 py-2 text-sm font-medium text-on-accent disabled:cursor-not-allowed disabled:bg-hairline">▣ {saving ? '保存中…' : '並びを保存'}</button>
    </div>
