...
export const restaurantTestApi = {
  ...
  connectStore: (accountId: string | null, body: { name: string; alias: string; channelId: string; channelSecret: string }) => fetchApi<{ success: true; data: { store: { id: string; name: string }; lineAccountName: string; basicId: string | null; friendCount: number | null; webhook: 'ok' | 'inactive' | 'failed' } }>(withOptionalAccount('/api/restaurant-test/stores/connect', accountId), { method: 'POST', body: JSON.stringify(body) }),
