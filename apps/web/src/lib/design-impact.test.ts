...
describe('共通部品の影響範囲', () => {
  ...
  it('共通Buttonを直接importする83ファイルを利用先に数える', () => {
    // 2026-09-04: 一覧の失敗状態（`shared/list-state.tsx`）に「再読み込み」の
    // 押し口を足したので 82 → 83。共通 Button を使うので数に入る。
    expect(directImporters(files, button)).toHaveLength(83)
  })
