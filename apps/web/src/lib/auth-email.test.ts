...
describe('ログイン前に開ける画面', () => {
  it('ログイン・招待・登録 3 画面・再設定 2 画面・運営ログイン。末尾の / があっても同じ', () => {
    expect([...PUBLIC_AUTH_PATHS]).toEqual(['/login', '/login/two-factor', '/staff/invite', '/register', '/register/sent', '/register/complete', '/password/forgot', '/password/reset', '/ops/login'])
