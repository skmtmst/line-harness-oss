...
// 取りこぼしがあればここでコンパイルが落ちるように、型から鍵を起こす。
// （db 側の定数をそのまま使わないのは、この経路を試験するとき db 全体を
//   差し替えることがあり、実行時の値が消えるため）
const INTENT_KEYS: Record<RichMenuAreaIntent, true> = {
  url: true,
  tel: true,
  text: true,
  template: true,
...
const VALID_INTENTS = new Set<string>(Object.keys(INTENT_KEYS));
...
function parseAreaInput(raw: unknown): Parsed<RichMenuAreaInput> {
  ...
  return {
    ...
    value: {
      ...
      intent: (r.intent as RichMenuAreaInput['intent']) ?? null,
