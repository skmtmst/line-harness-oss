...
import { createTag, createTagsBulk } from '@line-crm/db';
...
describe('legacy tags after migration 382: real SQLite and HTTP', () => {
  ...
  test('versioned PATCH rejects a scoped collision and bumps the version on success', async () => {
    ...
    expect((await request('/api/tags/scope-b', 'PATCH', { name: 'ＶＩＰ', lineAccountId: 'a1', expectedVersion: 1 })).status).toBe(409);
    ...
    expect((await request('/api/tags/scope-b', 'PATCH', { name: 'Ｓｔａｆｆ', lineAccountId: 'a1', expectedVersion: 1 })).status).toBe(200);
  ...
  test('old scoped NULL-normalized names reject duplicate POST and versioned PATCH', async () => {
    for (const id of ['a1', 'a2']) sql.prepare("INSERT INTO line_accounts(id,name,channel_id,channel_access_token,channel_secret) VALUES (?,?,?,'fixture','fixture')").run(id,id,`fixture-${id}`);
