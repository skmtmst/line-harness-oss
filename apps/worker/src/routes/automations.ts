...
import type { AutomationExecutionRun, AutomationExecutionRunsResponse, ExecutionRunStatus } from '@line-crm/shared';
...
/** V6 25-1-C: 実行まで接続済みの処理だけを含む、サーバー管理の見本。 */
automations.get('/api/automation-templates', requireRole('owner', 'admin', 'staff'), async (c) => {
  const accountId = await requireDraftAccount(c);
  if (typeof accountId !== 'string') return accountId;
  return draftEndpoint(c, () => listAutomationTemplates());
});

automations.get('/api/automation-draft-resources', requireRole('owner', 'admin'), async (c) => {
  const accountId = await requireDraftAccount(c);
  if (typeof accountId !== 'string') return accountId;
  return draftEndpoint(c, () => listAutomationDraftResources(c.env.DB, accountId));
});

automations.post('/api/automation-templates/:key/drafts', requireRole('owner', 'admin'), async (c) => {
  const accountId = await requireDraftAccount(c);
  if (typeof accountId !== 'string') return accountId;
...
automations.get('/api/automation-drafts/:id', requireRole('owner', 'admin'), async (c) => {
  const accountId = await requireDraftAccount(c);
  if (typeof accountId !== 'string') return accountId;
...
automations.put('/api/automation-drafts/:id', requireRole('owner', 'admin'), async (c) => {
  const accountId = await requireDraftAccount(c);
  if (typeof accountId !== 'string') return accountId;
  type DraftBody = {
...
automations.get('/api/automation-runs', async (c) => {
  try {
    const scope = await getVisibleLineAccountScope(c.env.DB, c.get('staff'));
    const requestedAccountId = c.req.query('lineAccountId')?.trim();
