...
beforeEach(() => {
  ...
  mocks.retryRun.mockResolvedValue({ kind: 'scheduled', run: { ...run, status: 'planned' } })
})
...
describe('リマインダ実行記録のアカウント範囲', () => {
  ...
  it('同じ再試行キーの再送を、二重登録せず受付済みとして返す', async () => {
    mocks.canAccess.mockResolvedValue(true)
    mocks.retryRun.mockResolvedValue({ kind: 'replay', run: { ...run, status: 'planned' } })
    ...
    expect(response.status).toBe(200)
    expect(body.data).toEqual({ id: 'run-1', status: 'planned', replayed: true })
  })
})
