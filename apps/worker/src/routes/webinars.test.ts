...
function staffReq(path: string, init?: RequestInit) {
  return staffApp.request(path, init, env, execCtx);
}
...
describe('webinar notification settings', () => {
  ...
  test('古い版の保存競合は409で返す', async () => {
    ...
    expect(res.status).toBe(409);
    expect(await res.json()).toEqual({ success: false, error: 'version_conflict' });
  });

  test.each([
    ['PUT', '/api/webinars/w1/notifications'],
    ['POST', '/api/webinars/w1/notifications/test'],
  ])('staff の %s %s を403で止める', async (method, path) => {
    dbMocks.getWebinarById.mockResolvedValue(makeWebinar({ account_id: 'account-a' }));
    const res = await staffReq(path, {
      method,
      headers: { 'Content-Type': 'application/json' },
