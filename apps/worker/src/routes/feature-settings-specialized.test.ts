...
describe('専用機能目録 API', () => {
  ...
  it('目録だけの保存では版付き機能設定を書き換えない', async () => {
    const settings = await app().request('/api/settings/features?account_id=default-account', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        expectedVersion: 0,
  ...
  it('目録と版付き機能設定の同時保存を拒否し、どちらも書き込まない', async () => {
