...
function commonStatus(status: AutoReplyEvaluationStatus): ExecutionRunStatus {
  ...
  if (status === 'reply_failed' || status === 'failed') return 'failed';
...
function serializeRun(
  ...
  return {
    ...
    inputPreview: row.input_preview_masked,
...
autoReplyRuns.get(
  ...
  async (c) => {
    try {
      ...
      const requestedRuleId = c.req.query('ruleId') || undefined;
      ...
      const selectedRule = requestedRuleId
        ...
        : visibleRules.find((rule) => rule.is_active === 1) ?? visibleRules[0];
      ...
      const data: AutoReplyRunsResponse = {
        ...
        items: runs.items.map((row) => serializeRun(row, selectedRule)),
