...
restaurantTest.get('/api/restaurant-test/snapshot', requireRole('owner', 'admin', 'staff'), async (c) => {
  ...
  const [stores, memberships, approvals, reservations, tables, inventory, menuItems, connectors, reviews, posts, lineFlows] = await Promise.all([
    c.env.DB.prepare('SELECT * FROM rt_stores WHERE organization_id = ? ORDER BY code').bind(orgId).all(),
    c.env.DB.prepare('SELECT * FROM rt_memberships WHERE organization_id = ? ORDER BY role, staff_name').bind(orgId).all(),
    c.env.DB.prepare('SELECT * FROM rt_approval_requests WHERE organization_id = ? ORDER BY CASE status WHEN \'pending\' THEN 0 WHEN \'returned\' THEN 1 ELSE 2 END, created_at DESC LIMIT 100').bind(orgId).all(),
    c.env.DB.prepare(`SELECT r.*, s.name AS store_name, t.label AS table_label, m.name AS course_name
    ...
    c.env.DB.prepare('SELECT t.* FROM rt_tables t JOIN rt_stores s ON s.id = t.store_id WHERE s.organization_id = ? ORDER BY t.store_id, t.code').bind(orgId).all(),
    c.env.DB.prepare('SELECT i.* FROM rt_inventory_slots i JOIN rt_stores s ON s.id = i.store_id WHERE s.organization_id = ? ORDER BY i.starts_at LIMIT 300').bind(orgId).all(),
    c.env.DB.prepare('SELECT m.* FROM rt_menu_items m JOIN rt_stores s ON s.id = m.store_id WHERE s.organization_id = ? ORDER BY m.kind, m.name').bind(orgId).all(),
    c.env.DB.prepare('SELECT x.* FROM rt_connector_status x JOIN rt_stores s ON s.id = x.store_id WHERE s.organization_id = ? ORDER BY x.store_id, x.provider').bind(orgId).all(),
    c.env.DB.prepare('SELECT g.* FROM rt_gbp_reviews g JOIN rt_stores s ON s.id = g.store_id WHERE s.organization_id = ? ORDER BY g.reviewed_at DESC LIMIT 100').bind(orgId).all(),
    c.env.DB.prepare('SELECT p.* FROM rt_gbp_posts p JOIN rt_stores s ON s.id = p.store_id WHERE s.organization_id = ? ORDER BY p.created_at DESC LIMIT 100').bind(orgId).all(),
    c.env.DB.prepare('SELECT * FROM rt_line_flows WHERE organization_id = ? ORDER BY flow_type').bind(orgId).all(),
  ]);
...
restaurantTest.post('/api/restaurant-test/bootstrap', requireRole('owner', 'admin'), async (c) => {
  ...
  const statements: D1PreparedStatement[] = [
    c.env.DB.prepare('INSERT INTO rt_organizations (id, account_id, name) VALUES (?, ?, ?)').bind(orgId, selectedAccount, body.organizationName?.trim() || '然-NEN RESTAURANT LAB'),
    c.env.DB.prepare('INSERT INTO rt_stores (id, organization_id, name, code, area, capacity, line_status, google_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(mainId, orgId, '銀座店', 'GINZA', '東京', 32, 'connected', 'connected'),
    c.env.DB.prepare('INSERT INTO rt_stores (id, organization_id, name, code, area, capacity, line_status, google_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(secondId, orgId, '横浜店', 'YOKOHAMA', '神奈川', 24, 'warning', 'unconfigured'),
    c.env.DB.prepare('INSERT INTO rt_memberships (id, organization_id, staff_name, email, role, status) VALUES (?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), orgId, '本部 管理者', 'admin@example.test', 'super_admin', 'active'),
    c.env.DB.prepare('INSERT INTO rt_memberships (id, organization_id, store_id, staff_name, email, role, status) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), orgId, mainId, '銀座 店長', 'ginza@example.test', 'store_manager', 'active'),
    c.env.DB.prepare('INSERT INTO rt_memberships (id, organization_id, store_id, staff_name, role, status) VALUES (?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), orgId, mainId, 'ホール スタッフ', 'staff', 'active'),
  ];
  ...
  for (const [id, code, label, min, max, x, y, join] of tableSeed) {
    statements.push(c.env.DB.prepare('INSERT INTO rt_tables (id, store_id, code, label, seat_type, min_capacity, max_capacity, floor_x, floor_y, join_group) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id, mainId, code, label, code.startsWith('P') ? 'private_room' : code.startsWith('C') ? 'counter' : 'table', min, max, x, y, join));
  }
  statements.push(
    c.env.DB.prepare('INSERT INTO rt_menu_items (id, store_id, kind, name, price, allergens_json, service_periods_json, duration_minutes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(menuIds[0], mainId, 'course', '季節のおまかせコース', 8800, '["卵","小麦"]', '["dinner"]', 120),
    c.env.DB.prepare('INSERT INTO rt_menu_items (id, store_id, kind, name, price, allergens_json, service_periods_json, duration_minutes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(menuIds[1], mainId, 'course', 'ランチ会席', 4800, '["小麦"]', '["lunch"]', 90),
    c.env.DB.prepare('INSERT INTO rt_menu_items (id, store_id, kind, name, price, allergens_json, service_periods_json) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(menuIds[2], mainId, 'a_la_carte', '旬魚の炭火焼', 2600, '["魚"]', '["dinner"]'),
  );
  for (const [hour, reserved] of [[17, 4], [17.5, 8], [18, 12], [18.5, 18], [19, 22], [19.5, 16], [20, 10]] as const) {
    const h = Math.floor(hour);
    const minute = hour % 1 ? 30 : 0;
    statements.push(c.env.DB.prepare('INSERT INTO rt_inventory_slots (id, store_id, starts_at, total_capacity, ota_capacity, line_capacity, walk_in_capacity, reserved_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), mainId, at(1, h, minute), 32, 18, 10, 4, reserved));
  }
  ...
  for (const [external, customer, guests, start, end, source] of reservations) {
    ...
    statements.push(c.env.DB.prepare('INSERT INTO rt_reservations (id, store_id, source, external_id, hub_source, customer_name, customer_phone, guest_count, starts_at, ends_at, table_id, course_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), mainId, source, external, source === 'restaurant_board' ? 'restaurant_board' : null, customer, '090-0000-0000', guests, start, end, table, menuIds[0], 'confirmed'));
  }
  for (const provider of ['restaurant_board', 'reszaiko', 'hotpepper', 'tabelog', 'gurunavi', 'ikyu', 'retty', 'google_business_profile', 'line']) {
    ...
    statements.push(c.env.DB.prepare('INSERT INTO rt_connector_status (id, store_id, provider, mode, status, last_synced_at) VALUES (?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), mainId, provider, enabled && provider !== 'google_business_profile' && provider !== 'line' ? 'inbound_only' : 'disabled', enabled ? 'connected' : 'unconfigured', enabled ? now.toISOString() : null));
  }
  statements.push(
    c.env.DB.prepare('INSERT INTO rt_approval_requests (id, organization_id, store_id, kind, title, status, requested_by, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), orgId, mainId, 'gbp_post', '秋の限定コースのご案内', 'pending', 'ホール スタッフ', '{"postType":"standard"}'),
    c.env.DB.prepare('INSERT INTO rt_approval_requests (id, organization_id, store_id, kind, title, status, requested_by, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), orgId, mainId, 'menu_change', 'ランチ会席 価格改定', 'pending', '銀座 店長', '{"price":5200}'),
    c.env.DB.prepare('INSERT INTO rt_gbp_reviews (id, store_id, external_review_id, author_name, rating, comment, reviewed_at, sentiment) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), mainId, 'GBP-DEMO-1', 'ご来店者様', 5, '季節のお料理と落ち着いた接客が素晴らしかったです。', at(-1, 12), 'positive'),
  );
  ...
  for (const [type, title, timing] of flowSeed) {
    statements.push(c.env.DB.prepare('INSERT INTO rt_line_flows (id, organization_id, store_id, flow_type, title, body, timing_minutes, is_enabled, delivery_mode) VALUES (?, ?, ?, ?, ?, ?, ?, 0, \'preview_only\')').bind(crypto.randomUUID(), orgId, mainId, type, title, `${title}のカードメッセージを準備しています。`, timing));
  }
...
restaurantTest.patch('/api/restaurant-test/approvals/:id', requireRole('owner', 'admin'), async (c) => {
  ...
  const result = await c.env.DB.prepare(`UPDATE rt_approval_requests
...
restaurantTest.post('/api/restaurant-test/reservations/manual', requireRole('owner', 'admin', 'staff'), async (c) => {
  ...
  if (!await acquireLock(c.env.DB, lockKey, lockOwner)) return c.json({ success: false, error: '同じ時間帯を別の担当者が更新中です' }, 409);
  try {
    const tables = await c.env.DB.prepare('SELECT id, min_capacity, max_capacity, is_active FROM rt_tables WHERE store_id = ?').bind(storeId).all<{ id: string; min_capacity: number; max_capacity: number; is_active: number }>();
    ...
    await c.env.DB.prepare(`INSERT INTO rt_reservations
    ...
    await releaseLock(c.env.DB, lockKey, lockOwner);
  }
});
...
restaurantTest.post('/api/restaurant-test/inbound/reservations', requireRole('owner', 'admin'), async (c) => {
  ...
  const inserted = await c.env.DB.prepare(`INSERT INTO rt_sync_events
  ...
  try {
    ...
    await c.env.DB.prepare("UPDATE rt_sync_events SET status = 'processed', processed_at = datetime('now') WHERE id = ?").bind(eventDbId).run();
    return c.json({ success: true, data: { duplicate: false, direction: 'inbound', outboundWrites: 0 } }, 201);
  } catch (error) {
    await c.env.DB.prepare("UPDATE rt_sync_events SET status = 'failed', error_message = ? WHERE id = ?").bind(error instanceof Error ? error.message.slice(0, 500) : 'unknown', eventDbId).run();
    throw error;
  }
});
...
restaurantTest.post('/api/restaurant-test/tables', requireRole('owner', 'admin'), async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare('INSERT INTO rt_tables (id, store_id, code, label, seat_type, min_capacity, max_capacity) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, body.storeId, body.code.trim(), body.label.trim(), body.seatType, min, max).run();
...
restaurantTest.post('/api/restaurant-test/memberships', requireRole('owner', 'admin'), async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare(`INSERT INTO rt_memberships
...
restaurantTest.put('/api/restaurant-test/inventory/:id', requireRole('owner', 'admin'), async (c) => {
  ...
  const result = await c.env.DB.prepare(`UPDATE rt_inventory_slots SET
...
restaurantTest.post('/api/restaurant-test/menu', requireRole('owner', 'admin'), async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare('INSERT INTO rt_menu_items (id, store_id, kind, name, price, allergens_json, service_periods_json) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, body.storeId, body.kind, body.name.trim(), price, JSON.stringify(body.allergens || []), JSON.stringify(body.servicePeriods || ['dinner'])).run();
  return c.json({ success: true, data: { id } }, 201);
});
...
restaurantTest.post('/api/restaurant-test/gbp/posts', requireRole('owner', 'admin'), async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`INSERT INTO rt_gbp_posts
    ...
    c.env.DB.prepare(`INSERT INTO rt_approval_requests
...
restaurantTest.put('/api/restaurant-test/gbp/reviews/:id/draft', requireRole('owner', 'admin'), async (c) => {
  ...
  const result = await c.env.DB.prepare(`UPDATE rt_gbp_reviews SET reply_draft = ?, reply_status = 'draft', updated_at = datetime('now')
...
restaurantTest.put('/api/restaurant-test/line-flows/:id', requireRole('owner', 'admin'), async (c) => {
  ...
  const result = await c.env.DB.prepare(`UPDATE rt_line_flows SET title = ?, body = ?, timing_minutes = ?, is_enabled = ?,
