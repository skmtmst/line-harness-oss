...
describe('GET /api/auto-reply-runs', () => {
  it('担当者が見えるLINEアカウントだけをDB取得条件へ渡す', async () => {
    const response = await app().request('/api/auto-reply-runs?ruleId=rule-a&limit=20', {}, { DB: db } as Env['Bindings']);
    ...
    const json = await response.json() as { success: boolean; data: { items: Array<{ status: string }> } };
