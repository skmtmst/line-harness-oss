...
describe('fireEvent — V6分析イベント', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    actionScoreMocks.applyActionScoreEvent.mockResolvedValue({ configured: false, status: 'legacy', applications: [] });
  });
...
describe('fireEvent — V6行動スコア', () => {
  ...
  it('元イベントの識別子がそろえばアカウント別の公開版を使い、旧ルールへ二重加点しない', async () => {
    const dbModule = await import('@line-crm/db');
    actionScoreMocks.applyActionScoreEvent.mockResolvedValue({ configured: true, status: 'published', applications: [] });
    const db = fakeDb({ capturedInserts: [] });
    ...
    expect(actionScoreMocks.applyActionScoreEvent).toHaveBeenCalledWith(db, expect.objectContaining({
      ...
      source: 'line_webhook', sourceEventId: 'webhook-1',
    }));
    expect(dbModule.applyScoring).not.toHaveBeenCalled();
  });

  it('元イベントを識別できない旧経路だけは、移行前のルールを維持する', async () => {
    const dbModule = await import('@line-crm/db');
    const db = fakeDb({ capturedInserts: [] });
