...
describe('リマインダ配信の実行記録', () => {
  it('未来の通を送らずplannedで先に記録し、予定件数と次回日時を出せる', async () => {
  ...
  it('一時失敗は5分後に同じLINE再送キーで送り直し、成功後は止まる', async () => {
    const { db, raw } = createTestD1()
    seedReminder(raw)
    ...
    const tooEarly = await processReminderDeliveries(db, client, {
      now: new Date('2026-08-28T09:04:59.000Z'),
      pause: noPause,
      resolveClient: async () => client,
    })
  ...
  it('自動再試行を5回で止め、手動確認が必要な言葉へ変える', async () => {
    const { db, raw } = createTestD1()
    seedReminder(raw)
    const client = makeClient(async () => {
