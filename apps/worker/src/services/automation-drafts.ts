...
async function requireResource(
  ...
  const row = await db.prepare(
    `SELECT id FROM ${table} WHERE id = ? AND line_account_id = ? LIMIT 1`,
...
export async function updateAutomationDraft(
  ...
  const triggerConfig = input.triggerConfig !== null && typeof input.triggerConfig === 'object' && !Array.isArray(input.triggerConfig)
