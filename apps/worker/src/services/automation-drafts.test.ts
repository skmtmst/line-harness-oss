...
describe('V6オートメーションの見本と下書き', () => {
  ...
  beforeEach(() => {
    ...
    testDb.raw.prepare(`INSERT INTO tags (id, name, line_account_id) VALUES ('tag-1', '予約', 'account-1')`).run();
    testDb.raw.prepare(`INSERT INTO tags (id, name, line_account_id) VALUES ('tag-2', '別店舗', 'account-2')`).run();
    testDb.raw.prepare(
  ...
  it('実行まで接続済みの見本だけを返し、空の見本は一覧へ出さない', () => {
    ...
    expect(listAutomationTemplates().some((item) => item.key === 'blank')).toBe(false);
  });
  ...
  it('選び直した同一アカウントの資源だけを下書きへ保存する', async () => {
    const created = await createAutomationDraftFromTemplate(testDb.db, {
  ...
  it('別アカウントの参照先と古い版での保存を拒否する', async () => {
    const created = await createAutomationDraftFromTemplate(testDb.db, {
      templateKey: 'received-message-tag',
      lineAccountId: 'account-1',
    });
    await expect(updateAutomationDraft(testDb.db, {
      ...
      triggerConfig: {},
      actions: [{ id: 'step-1', type: 'add_tag', params: { tagId: 'missing' }, onFailure: 'stop' }],
