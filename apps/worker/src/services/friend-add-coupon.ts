...
function parseSetting(value: string | null): FriendAddCouponSetting | null {
  ...
  try {
    ...
    return {
      ...
      couponName: setting.couponName.trim(),
    };
  } catch {
    return null;
  }
...
export function buildFriendAddCouponMessage(issue: {
  ...
  return [
    '友だち追加ありがとうございます🌿',
    '',
    `初回のお買い物に使える${issue.discountRate}%OFFクーポンをプレゼントします。`,
    `クーポンコード：${issue.code}`,
    `有効期限：${issue.expiresAt.slice(0, 10)}`,
    '※お一人様1回限りです。',
  ].join('\n');
}
...
export async function issueFriendAddCoupon(
  ...
  const expires = new Date(now.getTime() + setting.validityDays * 86_400_000);
  const expiresAt = jstIsoDate(expires);
  const createdAt = jstNow();
  const generatedCode = `${setting.codePrefix}-${crypto.randomUUID().replace(/-/g, '').slice(0, 12).toUpperCase()}`;
  await db.prepare(
    ...
    crypto.randomUUID(), input.lineAccountId, input.friendId, generatedCode,
    setting.discountRate, validFrom, expiresAt, createdAt, createdAt,
  ).run();
