...
async function matchRichReference(b:R2RuntimeBinding,ref:RichReference,account:string,execution=false):Promise<RichReferenceMatch> {
  ...
  if(ref.kind==='form') {
    ...
    const targetId=await plannedTargetId(b,account,ref),form:DbRow={...source!,id:targetId,is_active:0,status:'active',archived_at:null,revision:1,content_revision:1,current_published_version_id:null,submit_count:0};delete form.created_at;delete form.updated_at;
