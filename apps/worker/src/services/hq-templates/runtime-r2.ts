...
export async function executeR2RuntimeStore(options: R2StoreOptions): Promise<R2StoreOutcome> {
  ...
  let batchStarted = false, putUncertain = false;
  ...
  try {
    ...
    for (const object of plan.stage) {
      ...
      putUncertain=true;
      const written=await options.bucket.put(object.key,object.bytes,{onlyIf:{etagDoesNotMatch:'*'},customMetadata:{ownerToken:object.ownerToken,contentHash},httpMetadata:{contentType:object.contentType}});
      putUncertain=false;
    ...
    if (batchStarted || putUncertain || !code) return { status: 'staged', reused: false, cleanupPending: !!plan?.stage.length };
    ...
    const cleanupPending = plan ? !(await cleanupFailedOwnedImages(binding,runId,context.targetAccountId,plan)) : false;
