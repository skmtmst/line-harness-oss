...
function parseRule(row: RuleRow): SupportMarkAutomationRule | null {
  ...
  let actions: unknown;
  try { actions = JSON.parse(row.action_config); } catch { return null; }
...
function validateInput(input: SaveSupportMarkAutomationRule): void {
  ...
  if (!Number.isInteger(input.manualProtectionMinutes)
    || input.manualProtectionMinutes < 0 || input.manualProtectionMinutes > 10080) {
  ...
  await db.batch([
    db.prepare(
      `INSERT INTO automation_definitions
       ...
       VALUES (?, ?, ?, '対応マークの自動変更', ?, ?, ?, ?, ?)`,
    ).bind(id, scope.lineAccountId, input.name.trim(), input.isActive ? 'active' : 'stopped', input.priority, actorId, now, now),
    db.prepare(
    ...
    ).bind(versionId, id, payload.triggerConfig, payload.conditionConfig, payload.actionConfig, actorId, now, now),
...
export async function updateSupportMarkAutomationRule(
  ...
  const results = await db.batch([
    ...
    ).bind(versionId, nextVersion, payload.triggerConfig, payload.conditionConfig, payload.actionConfig,
      actorId, now, now, ruleId, scope.lineAccountId, ruleId, expectedVersion),
    db.prepare(
    ...
    ).bind(input.name.trim(), input.isActive ? 'active' : 'stopped', input.priority,
      versionId, now, ruleId, scope.lineAccountId, versionId, ruleId),
  ]);
...
export async function archiveSupportMarkAutomationRule(
  ...
  const result = await db.prepare(
    `UPDATE automation_definitions SET status = 'archived', archived_at = ?, updated_at = ?
      WHERE id = ? AND line_account_id = ? AND current_published_version_id =
        (SELECT id FROM automation_versions WHERE automation_id = ? AND version_number = ?)`,
  ).bind(jstNow(), jstNow(), ruleId, scope.lineAccountId, ruleId, expectedVersion).run();
  return Number(result.meta?.changes ?? 0) === 1 ? 'archived' : 'conflict';
}
