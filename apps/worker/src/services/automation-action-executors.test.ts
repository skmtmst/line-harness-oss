...
describe('V6オートメーションの既存処理接続', () => {
  ...
  it('対応マークを変更し、変更前後と自動変更の根拠を監査へ残す', async () => {
    ...
    const tenant = testDb.raw.prepare(
    ...
    testDb.raw.prepare(
      `INSERT INTO support_mark_scopes (mark_id, tenant_id, line_account_id, created_at)
       VALUES ('mark-working', ?, 'account-1', datetime('now'))`,
  ...
  it('手で変更した直後は保護時間が切れるまで自動変更で上書きしない', async () => {
    ...
    testDb.raw.prepare(
      `INSERT INTO support_mark_scopes (mark_id, tenant_id, line_account_id, created_at)
       VALUES ('mark-protected', ?, 'account-1', datetime('now'))`,
    ).run(tenant.tenant_id);
    testDb.raw.prepare(
      `INSERT INTO operation_audit
         (id, target_kind, target_id, action, actor_id, friend_id, created_at)
       VALUES ('audit-manual', 'support_mark', NULL, 'changed', 'staff-1', 'friend-1', datetime('now'))`,
    ).run();
