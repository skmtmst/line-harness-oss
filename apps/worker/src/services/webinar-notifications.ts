import { getAccountSetting, resolveLineCredential, jstNow } from '@line-crm/db';
...
import { classifyReminderDeliveryError } from './reminder-delivery.js';
    ...
    const result = await db.prepare(
      `INSERT INTO webinar_notification_jobs
         (id, webinar_id, registration_id, friend_id, session_start_at,
          ...
          next_attempt_at, retry_key, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0, ?, ?, ?, ?)
...
export async function saveWebinarNotificationSettings(
  ...
  if ((previous?.version ?? 0) !== input.expectedVersion) throw new Error('version_conflict');
  ...
  const saved = await db.prepare(
    `INSERT INTO webinar_notification_settings
     ...
     ON CONFLICT(webinar_id) DO UPDATE SET
       ...
       updated_at=excluded.updated_at
     WHERE webinar_notification_settings.version=?`,
  ).bind(
    webinarId,
    version,
  ...
  if ((saved.meta.changes ?? 0) !== 1) throw new Error('version_conflict');

  const cancelledResult = await db.prepare(
    `UPDATE webinar_notification_jobs
        SET status='cancelled', cancelled_at=?, updated_at=?
      WHERE webinar_id=? AND status IN ('pending','failed')`,
...
export async function registerWebinarSession(
  ...
  if (existing) {
    ...
    await db.prepare(
      `UPDATE webinar_notification_jobs
          SET status='cancelled', cancelled_at=?, updated_at=?
        WHERE registration_id=? AND status IN ('pending','failed')`,
...
export async function processWebinarNotificationJobs(
  ...
  const due = await db.prepare(
    `SELECT j.id, j.webinar_id, j.registration_id, j.friend_id,
            j.session_start_at, j.kind, j.attempt_count, j.retry_key,
      ...
      WHERE (
          ...
          OR (j.status='sending' AND COALESCE(j.lease_expires_at, 0) <= ?)
        )
        AND j.scheduled_at <= ?
        AND COALESCE(j.next_attempt_at, j.scheduled_at) <= ?
        AND r.status='active'
        AND w.status='active'
      ORDER BY j.scheduled_at ASC
      LIMIT 100`,
  ).bind(nowEpoch, nowEpoch, nowEpoch).all<DueJobRow>();
  let sent = 0;
  let failed = 0;
  let skipped = 0;
  for (const row of due.results ?? []) {
    const claimed = await db.prepare(
      `UPDATE webinar_notification_jobs
        ...
        WHERE id=? AND (
          ...
          OR (status='sending' AND COALESCE(lease_expires_at, 0) <= ?)
        )`,
    ...
    try {
      ...
      if (
        ...
        !row.is_following ||
        !row.line_account_id ||
        row.account_id !== row.line_account_id ||
        !row.line_account_active ||
        !featureEnabled(featureRaw)
      ) {
        await db.prepare(
          `UPDATE webinar_notification_jobs
              SET status='skipped', lease_expires_at=NULL, last_error=NULL, updated_at=?
            WHERE id=?`,
      ...
      ).bind(row.line_account_id).first<{ liff_id: string | null }>();
      const liffId = account?.liff_id ?? options.defaultLiffId;
      if (!liffId) throw new Error('missing_liff_id');
      const accessToken = await resolveLineCredential(
      ...
      await db.prepare(
        `UPDATE webinar_notification_jobs
            SET status='sent', sent_at=?, lease_expires_at=NULL, last_error=NULL, updated_at=?
      ...
      const nextAttempt = safe.retryable
        ? nextDeliveryRetryAt(now, attempt)
        : null;
      await db.prepare(
        `UPDATE webinar_notification_jobs
            SET status='failed', attempt_count=?, next_attempt_at=?, lease_expires_at=NULL,
                last_error=?, updated_at=?
          WHERE id=? AND status='sending'`,
      ).bind(
        attempt,
        nextAttempt ? Math.floor(nextAttempt.getTime() / 1000) : null,
        safe.message,
        now.toISOString(),
        row.id,
      ).run();
