...
describe('webinar notification jobs', () => {
  test('設定の版を上げ、古い未送信予定を取り消して新しい予定を作る', async () => {
    ...
    const second = await saveWebinarNotificationSettings(
      db,
      'webinar-1',
      { ...SETTINGS, expectedVersion: 1, hourBeforeMinutes: 180 },
    ...
    expect(raw.prepare(
      `SELECT COUNT(*) AS count FROM webinar_notification_jobs WHERE status='pending'`,
    ).get()).toEqual({ count: 4 });
  });

  test('古い版の設定保存は予定を変えず競合として止める', async () => {
    const { db, raw } = createTestD1();
    seedBase(raw);
    await registerWebinarSession(db, 'webinar-1', 'friend-1', SESSION, NOW);
    await saveWebinarNotificationSettings(db, 'webinar-1', SETTINGS, NOW);

    await expect(saveWebinarNotificationSettings(
      db,
      'webinar-1',
    ...
    expect(raw.prepare(
      `SELECT version, hour_before_minutes FROM webinar_notification_settings WHERE webinar_id='webinar-1'`,
    ).get()).toEqual({ version: 1, hour_before_minutes: 60 });
    expect(raw.prepare(
      `SELECT COUNT(*) AS count FROM webinar_notification_jobs WHERE status='cancelled'`,
    ).get()).toEqual({ count: 0 });
  });
  ...
  test('期限の来た通知をHarness Proxyへ一度だけ渡し、retry keyを維持する', async () => {
    ...
    const job = raw.prepare(
      `SELECT id, retry_key FROM webinar_notification_jobs WHERE kind='day_before'`,
    ).get() as { id: string; retry_key: string };
    raw.prepare(
      `UPDATE webinar_notification_jobs SET scheduled_at=?, next_attempt_at=? WHERE id=?`,
    ...
    expect(request.headers.get('X-Line-Retry-Key')).toBe(job.retry_key);
  ...
  test('共通の再試行上限で止まり、秘密を含む送信エラーを保存しない', async () => {
    const { db, raw } = createTestD1();
    seedBase(raw);
    await saveWebinarNotificationSettings(db, 'webinar-1', SETTINGS, NOW);
    await registerWebinarSession(db, 'webinar-1', 'friend-1', SESSION, NOW);
    ...
    raw.prepare(
      `UPDATE webinar_notification_jobs
          SET scheduled_at=?, next_attempt_at=?, attempt_count=3 WHERE id=?`,
    ).run(Math.floor(NOW.getTime() / 1000), Math.floor(NOW.getTime() / 1000), job.id);
    const dispatch = vi.fn(async () => new Response('token=private-customer-data', { status: 500 }));
    ...
    expect(raw.prepare(
      ...
      last_error: 'LINEへの送信に一時的に失敗しました。自動で再試行します。',
    });
    ...
    expect(JSON.stringify(raw.prepare(
      `SELECT last_error FROM webinar_notification_jobs WHERE id=?`,
    ).get(job.id))).not.toContain('private-customer-data');
  });

  test('ウェビナーと友だちのLINEアカウントが違えば実行時に送らない', async () => {
    const { db, raw } = createTestD1();
    seedBase(raw);
    raw.prepare(
      `INSERT INTO line_accounts
         (id, channel_id, name, channel_access_token, channel_secret, is_active, liff_id)
       VALUES ('account-2', 'channel-2', '別店舗', 'token-2', 'secret-2', 1, 'liff-2')`,
    ).run();
    await saveWebinarNotificationSettings(db, 'webinar-1', SETTINGS, NOW);
    await registerWebinarSession(db, 'webinar-1', 'friend-1', SESSION, NOW);
    raw.prepare(`UPDATE friends SET line_account_id='account-2' WHERE id='friend-1'`).run();
    const job = raw.prepare(
      `SELECT id FROM webinar_notification_jobs WHERE kind='day_before'`,
    ).get() as { id: string };
    raw.prepare(
